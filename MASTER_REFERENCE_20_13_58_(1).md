<!-- ============================================================================ -->
<!-- ***** READ THIS FIRST - ACTIVE STATE (Master 20.13.58, 2026-09-18) ***** -->
<!-- *** THE TURNING POINT: architecture was INVERTED all week. Now corrected. *** -->
<!-- CORE REFRAME (operator's insight, confirmed by full audit + research): -->
<!--   LiDAR was only ever meant to MEASURE the room (scale/structure/skeleton). -->
<!--   CAMERA was meant to ILLUSTRATE it (surface/texture/detail). -->
<!--   We ran it BACKWARDS all week: meshed the LiDAR (blobs) + decorated w/ camera. -->
<!--   RIGHT: mesh the PHOTOGRAMMETRY (camera=geometry) + LiDAR for SCALE only. -->
<!-- -->
<!-- THE ROOT CAUSE, PROVEN (not tau, not pipeline, not calibration): -->
<!--   The L2 is a ROBOTICS-CLASS LiDAR: ~5.7mm surface thickness static, 9.8mm moving -->
<!--   (measured this session vs dense_test GOOD-ref 3.3mm). This is a HARDWARE FLOOR - -->
<!--   no meshing (Poisson/TSDF/MLS all tried), no denoise, no setting produces clean -->
<!--   set-design geometry from the L2 cloud. Meshing the LiDAR = permanent blobs. -->
<!-- -->
<!-- TAU - TESTED AND ELIMINATED as the problem (big finding, closes a week-long thread): -->
<!--   * tau_pan_v3 CANNOT solve tau (no /aft_mapped_to_init odometry topic in that bag). -->
<!--   * tau_solve on fusioncap_163005 (has full odometry): -293ms, but WEAK (peak width -->
<!--     275ms fails <100 gate; half-splits agree 0.1ms so a real but broad signal). -->
<!--   * Added --tau flag to pointlio_pose_matcher.py (subtracts from camera stamps). -->
<!--   * Built tau_consistency.py (cross-frame color agreement = un-circular tau metric). -->
<!--   * SWEPT tau -0.2..+0.4 on 163005: cross-frame std is FLAT ~45 (min 45.12 @ +0.15, -->
<!--     vs 45.27 @ 0 = 0.3% improvement). -293ms was WORSE (47.5). -->
<!--   VERDICT: tau is real (~+0.1s) but NEGLIGIBLE (0.3% of the fusion scatter). The -->
<!--   ~45 disagreement is DOMINATED by the L2 geometry noise, NOT timing. Do NOT chase tau. -->
<!-- -->
<!-- THE CORRECTED PIPELINE (photogrammetry-primary) - what to run: -->
<!--   1. Capture: SLOW 360deg tipping walk (floor->ceiling), MUCH slower than 163005's -->
<!--      ~1m/s for 60-80% photo overlap. Same style operator already uses, just slower. -->
<!--      This ALSO serves LiDAR (translation keeps odometry alive). No sensor conflict. -->
<!--   2. Use ALL frames (2106), NOT decimated 422 - photogrammetry wants dense overlap. -->
<!--   3. make_xmp_v2.py bridge (PROVEN, reused) -> RealityScan Folder import -> Align. -->
<!--   4. Calculate Model HIGH DETAIL from the PHOTOS (not the LiDAR) = the geometry. -->
<!--   5. LiDAR for SCALE ONLY via control-points/distance (NOT full merge - sidesteps -->
<!--      the component-merge wall entirely; just need 1 known distance from the cloud). -->
<!--   6. Texture F9 from photos -> export FBX -> Unreal Nanite. -->
<!-- -->
<!-- OPERATOR'S FRAMING (carry into tomorrow, exact words): 'use the LiDAR to MAP the -->
<!--   photogrammetry, instead of using the photogrammetry to map the LiDAR.' The LiDAR is -->
<!--   the MEASURING REFERENCE the photo-geometry gets fitted TO. Correct dependency dir. -->
<!-- -->
<!-- TOMORROW'S JOB: RealityScan COMPOSITING / MELDING the two. Mechanisms to pin down -->
<!--   (documented, not yet run end-to-end): (a) Grouping Factor = the dial weighting -->
<!--   LiDAR-vs-photo for meshing (higher=LiDAR drives geometry/scale, photos add surface); -->
<!--   (b) Control Points / Distance Constraints = lock photogrammetry to LiDAR's true -->
<!--   measurements WITHOUT the full component merge (sidesteps the merge wall); -->
<!--   (c) the composite SEQUENCE: align photos -> LiDAR as metric reference -> mesh melded -->
<!--   -> texture. PLAN: research exact RS 2.1 meld workflow -> sandbox/conflict-check -> run. -->
<!-- -->
<!-- THIS SESSION'S TEST (163005, 422 frames, photogrammetry-primary, High Detail): -->
<!--   Result: COHERENT-BUT-ROUGH photo mesh (real planes/walls, textured) - MUCH better -->
<!--   STRUCTURE than the LiDAR confetti, but not clean set-design detail. -->
<!--   READ: confirms (a) photogrammetry-primary is the RIGHT architecture, (b) the limit -->
<!--   is now CAPTURE PACE not the sensor floor. 422 fast-walk frames undersupply photo -->
<!--   baselines. Fix = slower/denser capture (CRAFT, comes with reps) + all 2106 frames. -->
<!-- -->
<!-- CONFLICTS AUDITED (corrected pipeline): #1 capture motion (walk->slow tipping orbit; -->
<!--   craft, not blocker). #2 use all 2106 not 422 (trivial). #3 LiDAR-camera merge wall -->
<!--   (SIDESTEPPED: don't merge, borrow scale via 1 distance/control-point). -->
<!-- -->
<!-- HARDWARE VERDICT (researched, costs banked in artifacts): L2 CANNOT be upgraded to -->
<!--   clean surfaces by settings. Sub-$2k modules (Livox Avia/Mid-360) are SAME or WORSE -->
<!--   noise floor - NOT an upgrade. Clean (<=3mm) starts at BLK360 G2 (~$18-25k tripod) or -->
<!--   NavVis VLX4 (~$50k, only clean mobile). BUT: you likely DON'T need new hardware - -->
<!--   the L2 is FINE as the MEASUREMENT tool; clean surface comes from PHOTOGRAMMETRY. -->
<!--   Honest best value = $0: photos for geometry, L2 for scale. Qualifying spec for any -->
<!--   future LiDAR: independently-measured 1-sigma plane-fit range noise <=3mm at 5m. -->
<!-- -->
<!-- REUSABLE (all proven, zero rebuild): rig/capture, Point-LIO, pointlio_pose_matcher -->
<!--   (+tau flag), make_xmp_v2.py, calibration, colorize_cloud.py, tsdf_mesh.py, -->
<!--   tau_consistency.py. On Shadow: fusioncap_163005.ply, frames_163005_key(422+xmp), -->
<!--   COLOR_tau0/tau293.ply, sweep_tau*.npz. RealityScan 2.1.1.119. -->
<!-- ============================================================================ -->
...
═══════════════════════════════════════════════════════════════════════════
## 0-PRE. DOCUMENT LINEAGE — WHICH DOCUMENT TO TRUST (added 2026-09-03)
═══════════════════════════════════════════════════════════════════════════
THIS LINEAGE (MASTER_REFERENCE_20_x) IS AUTHORITATIVE. Any document named
PROJECT_MASTER_RECORD (or similar) that says "calibration is not finished" is a
FOSSIL from BEFORE sessions 6-8 — SUPERSEDED as of 2026-08-16. Do not merge it;
archive it with a supersession header. The timeline is consistent: intrinsics
solved 2026-08-13 (session 6), extrinsic solved 2026-08-16 (session 8), and the
Point-LIO pivot of 2026-08-20/21 sits DOWNSTREAM of completed calibration.

READ "VETTED" WITH ITS STATED METHOD ATTACHED — three items, three standards:
  - INTRINSICS:  vetted NUMERICALLY (RMS 0.297 px, triple-vet). A number.
  - EXTRINSIC:   vetted VISUALLY (session 8: colour lands on geometry). A verdict,
                 not yet a number — §8I's "quality never quantified" is the SAME
                 fact at a stricter standard, NOT a contradiction. The §8X
                 consistency SCORER exists precisely to turn this verdict into a
                 measured number (run it on the archive pans first).
  - TEMPORAL (tau): NOT SOLVED — the one item every document agrees is open
                 (~150-200 ms, sign nailed, affects COLOUR only, not geometry).
                 Its blocker was recorder dropouts on the slow SD — the 1 TB SSD
                 (543 MB/s, §8U) has UNBLOCKED the clean capture it needs.
═══════════════════════════════════════════════════════════════════════════
...
| rig_stop.sh / rig_start.sh | rig teardown/bringup | PATCHED 2026-09-03 (+pointlio_mapping,+rviz2); backups .bak_prezombiefix; REPO STALE |
| point_lio_capture.sh | lean capture | PATCHED 2026-09-03 rviz:=false; backup .bak_prezombiefix; REPO STALE |
| ~/Desktop/three.min.js | local three.js r128 (603445 B) | NEW 2026-09-03 — kills cloud.html's internet dependency |
| rig_kiosk.html (V3) | kiosk: buttons+bars+EMBEDDED 3D coverage panel | 8edf3eab; deploy paused at D1 (repo upload) |
| cloud.html (V3) | 3D viewer + ?embed=1&mode= params, local three.js | 325576a0; deploy paused at D1 |
| ~/rig_kiosk_launch.sh | RIG KIOSK icon launcher | edited to open /cloud 2026-09-03 am; UNDER V3 THIS IS OBSOLETE -> revert to :8080 next session (v3 kiosk contains the view). Desktop copy = stale draft |
| topic_silence_check_9_4.sh | outputs + (ship) | 3-layer proof (nodes/pgrep/hz) that sensor topics are SILENT before a bag replay/re-registration. Prevents live-vs-replay topic collision. PROVEN 2026-09-04 |
| camera_unwedge_9_4.sh | outputs + (ship) | clears the recurring GSCAM HANG (node up, device+topic present, 0 frames). kill -9 gscam + relaunch w/ exact rig_start.sh GSCAM_CONFIG. PROVEN 25.9 Hz. Root cause still OPEN |
| STARTUP_UNWEDGE_TODO_9_5.md | outputs | TODO: build camera-verify + Mode-1 auto-unwedge into rig_start.sh (cold, sandboxed). Two camera failure modes documented; Mode-2 USB-rebind untested-as-script (flag, don't auto-add yet) |
| JETSON_BAKE_MEMORY_WALL_9_5.md | outputs | the full finding: Jetson OOM-locks on full texture bake -> station task. Milestone bag 180551 details |
| pointlio_to_texture.py + per_shot_texture.py | Jetson ~/anchor_test | the projection/bake toolchain (baker 4 selftests PASS). WARNING: full-cloud run OOM-locks the Jetson — station only, or pre-downsample |
| rig_kiosk_bench_9_4.html | Jetson ~/Desktop + outputs | BENCH kiosk variant: relaxes Waveshare 1024x600 fixed-size to fill a normal window. Identical logic to v3.1. md5 df117808 on-Jetson. SERVE via localhost:8080/kiosk, NEVER file:// (dead panel) |
| run_point_lio.sh (re-registration use) | Jetson ~/ | launches Point-LIO with NO driver -> feed it a bag replay (cloud+imu) to REGENERATE poses offline. Exclude /aft_mapped_to_init from the replay (--topics /unilidar/cloud /unilidar/imu) so fresh poses don't collide with old |
...
═══════════════════════════════════════════════════════════════════════════
## 4B. FIELD CHAIN DOCTRINE (banked 2026-09-03, operator-ratified — "doctrine til we change it")
The complete field sequence, zero keyboard/mouse: every touch is a finger on the Waveshare or a physical switch.
1. POWER THE JETSON -> boots to desktop (prereq once: auto-login verified so no password prompt can strand the field)
2. DOUBLE-TAP RIG KIOSK icon -> launcher sources ROS, starts server if absent, waits for it, opens fullscreen kiosk.
   Screen: RIG OFF, red bars, dark 3D panel. Honest cold. (Requires icon revert to :8080 — pending.)
3. POWER THE L2 -> ~15 s spin-up. Screen unchanged — correct.
4. TAP START RIG -> LiDAR 12 Hz, IMU ~251 Hz, L2 temp appears. Green bars = ready. (Camera red until fixed; non-blocking.)
5. RIG LEAVES STAND -> in hand, braced. TAU-CRITICAL: tau rides hand tremor (6-12 Hz, synth-proven); a stand-started
   capture is tau-blind AND odometry-starved (7U). The stand is for transport and waiting, NEVER for capturing.
6. HOLD DEAD-STILL, TAP CAPTURE, STAY STILL ~10 s -> the init ritual, held in hand. Banner red CAPTURING.
7. WALK THE SPACE -> the panel paints the room red->green (red = walk here again), voxel counter climbs.
8. TAP CAPTURE TO STOP -> bag closes clean on the SSD; MESH CHECK verdict overlays the panel by itself seconds later.
9. STOP RIG -> L2 OFF -> rig back on stand.
Two switches, four taps, one lift, one walk. Everything between taps is automatic.
ADDENDUM (8AF, learned live): CAPTURE IS NOT A TOGGLE — every tap spawns a NEW capture script (whose clean-slate
kills the prior Point-LIO = false stop-edge, and ORPHANS the prior bag recorder). THE STOP IS Ctrl-C IN THE CAPTURE
TERMINAL, once. Never tap CAPTURE twice in one take. (v4 makes the button a true toggle; until then this is law.)
ALSO: the kiosk buttons and the Desktop icons are parallel doorways to the same scripts — doctrine uses the KIOSK ONLY.

## 4C. CAMERA / ODOMETRY SYNTHESIS  (ATTACHMENT: SYNTHESIS_RESEARCH_ROADMAP_9_4.md)
The photoreal (texture) half of the product = projecting camera color onto LiDAR geometry, placed by
odometry poses. Full four-paper research review is in the attachment SYNTHESIS_RESEARCH_ROADMAP_9_4.md
(md5 1b437367). Summary:

THREE ARCHITECTURAL STANCES:
1. OURS NOW — camera downstream, texture-only, poses trusted, tau must be corrected. Jetson/station.
2. V-LOAM (2015) — camera IN odometry loop, tau dissolved at source. Would re-architect away from Point-LIO.
3. Pose-free JOINT reconstruction (MUP NeurIPS'25; DVLO'24) — poses self-correct via geometry-color
   consistency. THE NORTH STAR — but needs a 24GB GPU (MUP: RTX 3090, 60K iters/~33-frame scene),
   PERMANENTLY off-rig. Our Jetson (8GB shared) hard-locked on a non-neural bake; MUP is orders beyond.

*** KEY INSIGHT (do not over-reach): MUP et al. are engineered to rescue poses off by 20deg/3m.
OUR Point-LIO poses are cm-level. We are in the EASY regime -> the SIMPLE path (project with our
good poses + tau timestamp-shift if smear appears) likely suffices. Frontier methods rescue BAD poses;
ours are good. This ARGUES FOR the simple projection proof, not the GPU pipeline. ***

ACTIONABLE NOW (our architecture, no GPU):
- TAU FIX (V-LOAM principle): interpolate pose to (image_timestamp - tau). We already interpolate to
  image timestamps (matcher, self-tested 5e-16); tau is just a shift in the lookup time. No new hardware.
- CAPTURE DOCTRINE (V-LOAM Table II): translate SLOWLY (~0.7 m/s). Fast motion BLURS geometry + breaks
  tracking. Speed window: enough to feed odom, slow enough to avoid blur. Add to the "rig leaves stand" rule.
- SUCCESS METRIC (MUP Eq.9): the projection-proof test = do ADJACENT frames' projected colors AGREE on
  shared surfaces (photometric consistency across parallax). Use this, NOT eyeballed "looks right."
- LATER (DVLO): cluster image pixels around each projected LiDAR point for better per-face sampling (v2).

*** CONVERGENCE (sandbox-proven 9-5): 'onboard processing' + 'synthesis proof' are THE SAME BUILD — a STREAMING BAKER. The bake locked the Jetson only because it held all 2200 images+score+mesh at once (~21GB). Chunked (8-20 imgs/chunk, free between) = bounded ~100-150MB peak, FLAT, fits 8GB easily. So a streaming rewrite of pointlio_to_texture.py (a) makes onboard baking viable AND (b) IS the memory-safe projection proof. ONE build, both payoffs. Design budget: chunk ~50-100k faces, ~8-20 images resident, peak <~4GB. NEXT BUILD. ***

STAGED LADDER: Capture (Jetson, DONE 180551) -> Anchoring (Jetson, DONE) -> SIMPLE PROJECTION PROOF
(Jetson small-data, NEXT/un-run, success=adjacent-frame color consistency) -> Texture bake (STATION,
memory-walled off Jetson) -> Pose-free joint recon (24GB GPU, north star, likely NOT NEEDED given cm poses).

## 4D. THE COVERAGE HUD — "did I get it?"  (ATTACHMENT: HUD_AND_EYE_PRINCIPLE_9_5.md)
THE THESIS: the product is TIME saved on set. Previz (crane heights, light placement, night-look, coverage
certainty) decided BEFORE the shooting day. Cheap rig-time bought against expensive set-time.

GOVERNING PRINCIPLE: THE HUMAN EYE IS THE ARBITER. Instruments advise, never decide. "Looks good" > "numbers
add up." Corollary: no numeric gates / auto "you got it" verdicts — build instruments that POINT THE EYE.
(Every project-long failure of this — 100%-green HUD, false "you got it", adaptive-threshold holes — came from
letting a number do the eye's job.)

THE HUD (true purpose of camera/odom synthesis, NOT the texture bake):
- LIVE ACCUMULATION METER registered to the real scene (camera + odometry + LiDAR).
  red = little accumulated here -> green = lots. RELATIVE, not an absolute count / threshold.
- Answers "have I spent enough TIME here?" DETAIL = TIME, self-scaling: operator dwells longer on cluttered
  surfaces, glances at mass. NO shot-type detection, NO adaptive threshold. "Enough" = operator's learned call.
- THREE SCENARIOS the project must serve: LOW = Ext Barn establishing (minutes, mass/scale/placement);
  MED = Int Office (scope, whole-space + minor detail); HIGH = Int Barn cluttered (dense everything, most time).
- TIME two faces: live control (dwell till green) + production budget (allocate by shot complexity; greening
  RATE can flag a bad time-budget before the truck leaves).

BUILD: projection foundation PROVEN (hud_projection_test_9_4.py: LiDAR lands on the real room, 180551 f1100,
50.8% on-screen). Per-frame density judged = WRONG question (a frame almost always covers what it points at;
hud_coverage_9_4.py gave 100%-green -> the threshold + wrong-question lessons). NEXT BRICK: accumulation-over-
TIME display on the projection foundation. VALIDATION needs a deliberately-VARIED capture (dwell/rush) — 180551
is too uniform to differentiate reds/greens by time-spent.

*** 8AI UPDATE (2026-09-05, BUILD DAY) ***
- *** CAMERA DIES ~14s INTO EVERY CAPTURE (silent) *** — found via same-instant lidar+cam test. Every camera/HUD/
  synthesis result to date used only the first ~14s of frames; camera-blind after. LIKELY CPU starvation (fusion
  nodes + Point-LIO + recorder starve gscam). FIX: drop fusion nodes during capture, verify camera-Hz mid-capture.
  This must be fixed BEFORE any HUD/synthesis is trustworthy — no capture yet has full camera coverage.
  (CAMERA_DIES_MIDCAPTURE_9_5.md)
- READABLE FORM = MESHED COVERAGE SURFACE (or grid-tint on the real photo). Abstract point cloud is UNREADABLE
  (operator AND Claude couldn't tell mantel from not — a HUD you must interrogate has failed). A meshed coverage
  surface of 121219 was LEGIBLE: mantel identified, green=push-in dwell, red=thin edges. THIS is the form. Cloud dead.
- PER-FRAME density = WRONG UNIT (re-confirmed): a single frame covers whatever it points at -> all-green. Judgment
  must be ACCUMULATION over the whole capture (which surfaces got TIME). Still unbuilt.
- THREE DISTINCT CAPTURES (checksummed, NOT duplicates): 180551 walk b45589bc, 112949 dwell 7899816a, 121219
  push-in 65b2caba. Room identical across them -> EYE can't tell apart, only md5 can. 121219 = cleanest, HUD base.
- FILE-DISCIPLINE LESSON: afternoon confusion = cross-file mislabeling (180551 overlays discussed as today's).
  Operator caught it twice; checksums resolved it. RULE: verify WHICH capture before interpreting any render.
- OPEN BUG: 180551 DUMPED frames noisy/blue-green while LIVE rqt feed is clean+sharp -> frame/colorspace bug in
  the dump path (captured frames should match the live view). Run down before trusting dumped-frame overlays.

## 4E. CAMERA CAPTURE ERROR (SOLVED 2026-09-06)  (ATTACHMENT: CAMERA_FIXED_9_6.md)
THE BUG: the camera died ~14s into EVERY capture, silently — the bag "succeeded" but camera frames covered
only the first ~14s; LiDAR ran full duration. Undermined every camera/HUD/synthesis result in the project
(all built on first-14s-only camera data).

ROOT CAUSE (CONFIRMED): the two fusion nodes (overlay_check_node.py, colorized_fusion_node.py) subscribe to
/image_raw via message_filters.ApproximateTimeSynchronizer. The synchronizer BUFFERS both streams waiting to
match image+cloud timestamp pairs; it can't keep up (27Hz cam + 12Hz cloud + per-frame projection), its queues
fill, and the back-pressure propagates to gscam, stalling it ~14s in. NOT resource starvation — tegrastats
during a capture showed RAM 3.6/7.6GB, CPU 20-60% (never pinned), GPU 0-62%, ~53C: full headroom. NOT RViz
(deployed capture is rviz:=false).

FIX (PROVEN): kill overlay_check_node.py + colorized_fusion_node.py before capture. Result: fusioncap_115614 =
camera survived the FULL 66.1s, 1809 frames @ 27.4Hz, matcher 100% (1809/1809) across the whole span. First
full-duration-camera capture in project history. The fusion nodes are LIVE-ONLY (rqt overlay + an unrecorded
/fusion/colorized_cloud) — killing them costs only a live monitor view; the colorized deliverable is made
post-capture by per_shot_texture, unaffected.

DOWNSTREAM EFFECTS:
- Every PRIOR capture is camera-truncated (~first 14s). All prior camera-based results (incl. any texture/
  fusion beyond 14s) used truncated data -> must be re-validated on 115614-class full captures.
- HUD DESIGN RULE (proven, must-not-violate): the live camera+LiDAR HUD is the SAME operation that killed the
  camera (overlay_check_node WAS a live camera+overlay publisher). Build it NON-blocking: best-effort QoS
  depth=1 (newest frame, drop the rest), grab latest cloud unsynced. ~15fps view (== operator's viewing
  instinct) is both smooth enough AND the exact non-back-pressure fix. NEVER use ApproximateTimeSynchronizer
  or a queuing/blocking subscription on /image_raw in the HUD.
- colorized_fusion_node.py = a reference implementation of the image->cloud projection (intrinsics/extrinsic
  loaders + projection) — mine it for the HUD/synthesis projection math.

PERMANENT-FIX TODO: bake "fusion nodes OFF during capture" into the workflow (remove from rig_start.sh OR kill
in point_lio_capture.sh clean-slate) + add a camera-Hz-during-capture guard that alarms on drop. Currently a
manual kill = fragile, easy to forget = silent 14s regression.

OPEN DECISION (banked 2026-09-06, to weigh next session): 15 fps — HUD-ONLY, or PROJECT-WIDE?
The ~15fps best-effort/depth-1 rule is PROVEN as the fix for the live HUD (the viewer that kills
the camera if it queues). The undecided question: does 15fps stay confined to the Waveshare HUD, or
should the whole capture run at a reduced rate?

Facts that bound the decision (do NOT re-derive):
- HARDWARE: at 1920x1200 the Arducam offers 30/60/80 fps ONLY — NO native 15fps at full res
  (v4l2-verified, §3). So "15fps everywhere" at full res is not a sensor mode; it is either
  frame-dropping (the HUD's method) or dropping resolution to get a native low rate.
- The camera bug was NEVER the frame RATE — it was the ApproximateTimeSynchronizer back-pressuring
  gscam. 115614 proved the camera records the FULL 27Hz cleanly once the synchronizer is gone.
  So lowering the capture rate is NOT required to fix the camera; killing the fusion nodes already did.
- HUD (drop-frames-freely viewer) and PIPELINE (lossless product path) are DIFFERENT jobs with
  different rate rules (operator drew this line explicitly). The HUD's 15fps is a display drop-rate;
  the recorded bag is untouched at 27Hz.

The two sides to weigh next time:
- HUD-ONLY 15fps (default reading of the proven rule): pipeline keeps full 27Hz recorded frames =
  maximum data for texture/synthesis; 15fps is purely the operator's live view. Cost: none to the
  product; two rates to reason about.
- PROJECT-WIDE 15fps: simpler single rate everywhere; lighter capture-time load; MORE hand-tremor
  spacing per frame. Cost: fewer frames for multi-view texture/densification (the product wants MORE
  views, not fewer — §7T densification + multi-view coverage arguments), and at full res it means
  frame-dropping anyway (no native 15). CONSEQUENCE TO CHECK: does halving frame count meaningfully
  thin multi-view coverage or the tau correlation's sampling?

NOT DECIDED. Weigh HUD-only vs project-wide against the product's appetite for views next session.
...
| fusioncap_094752 | /mnt/rigdata/ (135M bag + 4.8M pcd) | first SSD-era capture (30s static, LiDAR+IMU only, camera dead). Clean close. GEOMETRY DIVERGED (islands) — keep as gap-injection evidence, do NOT process |
| ~/miner/report_*.ply | Jetson ~/miner/ | mining-campaign meshes incl. THE CHAMPION report_121359.ply (11M) + .meshbin — the restore source. KEEP |
| ~/miner/session_094752/ | Jetson | tonight's diverged auto-check outputs (mesh.* + map_snapshot.npy), archived evidence |
| fusioncap_110644 | /mnt/rigdata/ (425M) | ACCIDENTAL bag (zombie-era 11:06-11:14, clean INT close). Junk map; raw /unilidar topics possibly minable. Deletable |
| fusioncap_170207 | /mnt/rigdata/ (+pcd 4.0M) | GOOD TAKE, 10.0x16.1x4.9m, 911 cloud / 19k imu. Clean doctrine-era walk |
| fusioncap_171814 | /mnt/rigdata/ (+pcd 3.4M) | GOOD TAKE, 9.4x12.9x2.8m (ceiling-true). The 'Nothing' walk — panel was module-dead, capture was CLEAN. KEEP |
| fusioncap_185352 | /mnt/rigdata/ (307M) | THE FINALE PAN. Bag closed clean (metadata 19:02); PCD FAIL->regenerable by replay (double-tap killed writer). Full map re-meshed 78,458v/163,056f from 23,883 voxels. Renders in chat outputs (finale_walk_COMPLETE_9_3.png) |
| champion courier | chat outputs + ~/miner/ | report_121359.meshbin verified BYTE-EXACT off-Jetson: md5 99a6bf2b, RGM1, 246,804v/549,948f. TRIPLE-SAFE |
| fusioncap_143849 | /mnt/rigdata/ (64M pcd) | 2026-09-04 LIVE 10deg IN-PLACE TIP test. Poses 0.25m spread, start≈end = pivot, not translation. PCD 64M (8x a pan) = more angles not positions. Evidence that in-place tip != translation |
| fusioncap_180551 | /mnt/rigdata/ (56M pcd, 81s) | *** THE MILESTONE *** First REAL translating capture (tip/tip/~1m walk, rig off stand). Poses span 2.65m. Camera live @28.9Hz, 2200 frames. 100% anchored WITH REAL CONTENT (validated). Seed bag for synthesis. DO full texture bake on the STATION only (Jetson OOM-locks) |
| posed_180551.npz + frames_180551/ | Jetson ~/anchor_test/ | 2200 anchored poses (303KB) + dumped frames. The first genuine pose-anchoring in the archive |
| reg_run1, reg_102338 | Jetson ~/anchor_test/ | RE-REGISTRATION outputs (Point-LIO re-run on 121658 fast / 102338 real-rate). Both near-static (0.63m / 0.16m) = confirmed absent-translation, not replay artifact. Keep as method evidence |
| ~/anchor_test/ | Jetson | re-registration workspace: matcher, posed_121658.npz, dumped frames (~6GB, deletable), the two tools |
...
- **"DIVERGENCE" IS DISPLACED-CLUSTER OUTLIERS, NOT ESTIMATOR RUNAWAY (2026-08-22)**: operator's RViz
  ...
  *** 2026-09-03 UPDATE (8AC): CANDIDATE MECHANISM FOUND. fusioncap_094752 (30s static stand) diverged as
  ISLANDS: 5-6 locally-coherent room-scale fragments stepping ~900m along ONE straight descending line with
  GROWING spacing — the signature of IMU dead-reckoning across odometry gaps (bias double-integration =
  straight line, quadratic steps), each odom revival depositing a coherent patch at the drifted pose.
  HYPOTHESIS: the 08-22 "92% room + 7% cluster" and this are the SAME failure at different severity (one gap
  vs all-gap static starvation). TEST on next walk bag: clusters should correlate with odom-gap timestamps.
  Also: robust-vs-full extent DISCRIMINATES the two failure modes (blob: robust collapses to room; smear/
  islands: robust stays huge) — tonight robust 179x915x367m ~ full = mass is spread. ***
- **MESH SERVE PATH IS /tmp + AUTO-CHECK OVERWRITES IT (2026-09-03, 8AC)**: rig_kiosk_server serves the
  viewer mesh from /tmp/rig_meshcheck/mesh.meshbin and the AUTO mesh check writes THE SAME FILE on every
  capture stop -> tonight it clobbered the staged champion. Worse: /tmp is wiped on reboot, so anything
  staged there is doomed regardless. RESTORE recipe (proven): cp ~/miner/report_121359.meshbin
  /tmp/rig_meshcheck/mesh.meshbin (viewer reads meta from the meshbin; stale mesh.json is ignored).
  SERVER REV REQUIREMENTS: persistent mesh dir, per-session filenames, protected champion slot, clean
  shutdown handler (server ignores SIGTERM — cause unknown; restart rule = kill -9 by PID, verify port
  freed, relaunch from ~/Desktop).
- **LAZY IMPORTS MAKE MISSING FILES INTO GHOST STORIES (2026-09-03 night, 8AF — THE lesson of the day)**: the
  server imports map_accumulator INSIDE get_map(), first called on a capture RISING EDGE -> a missing module
  crashes the reader thread MID-LOOP, silently (except:pass + daemon thread), and /data serves the LAST-BUILT
  DICT FOREVER: frozen bars, capturing:False against a 316 Hz wire, 'RIG ON' latched (ever[] never resets).
  This one fault impersonated executor starvation, QoS mismatch, DDS domains, SHM corruption and a display
  lock across two chats for ~4 hours. RULES: (1) any Desktop sweep = re-verify the server's import set
  (map_accumulator.py, mesh_check.py, check_capture.py are LOAD-BEARING); (2) reader-thread death must be
  LOUD (v4: log + respawn + a reader_alive field in /data); (3) a too-normal-looking frozen number (lidar
  12.0 with the rig off) is the tell — cross-check one value that MUST move.
- **JETSON FULL TEXTURE-BAKE HARD-LOCKS THE MACHINE (2026-09-05, 8AH — permanent)**: pointlio_to_texture.py
  on a real capture (mesh-from-full-~1M-pt-cloud + 2200 imgs + per-face projection) exhausts 8GB+swap and
  FREEZES the OS (needs reset). The mesh-build on the full cloud is the killer, BEFORE any projection, so
  --render-idx (one view) does NOT save you. FIX for a Jetson-side proof: pre-downsample the PCD first, OR
  point-projection (no mesh). Full bakes = PROCESSING STATION. Resolves §8I: geometry mesh ~3GB may fit;
  texture bake does not. NEVER run the full bake on the Jetson again.
- **CAMERA HAS TWO SILENT-FAILURE MODES (2026-09-05, 8AH)**: Mode 1 = gscam process wedge (node up, device
  fine, pipeline stuck) -> camera_unwedge_9_4.sh clears it. Mode 2 = device-state stall ("Could not get
  gstreamer sample" / "Opening in BLOCKING MODE"; device itself not delivering) -> software restart does NOT
  clear it -> PHYSICAL USB unplug/replug re-enumerates and fixes it (PROVEN, and survives the next Start Rig).
  DISCRIMINATOR: v4l2-ctl --stream-count=1 raw-grab; a 614400-byte frame = device delivering (Mode 1, gscam's
  fault); no frame = Mode 2 (device stalled, needs replug). TODO: build Mode-1 auto-unwedge into rig_start.sh.
- **CONSULT THE MASTER AT THE DECISION POINT, NOT JUST AT SESSION START (2026-09-04, 8AG — cost ~an afternoon)**:
  near-static poses on 121658 were treated as a novel mystery and a full re-registration investigation was built to
  explain them — when line 151 ALREADY stated (Aug 24) 'the cutoff was never a bug, it was absent translation; static-
  pivot RULED OUT.' The fact was banked; it just wasn't re-read when the surprising result appeared. RULE: a SURPRISING
  result is the trigger to search the Master FIRST (grep the symptom) before investigating. Reading the Master once at
  the top is necessary but NOT sufficient — it is a lookup to hit at every fork.
- **POSES ARE NEAR-STATIC ON BENCH CAPTURES — AND THAT'S CORRECT, NOT A BUG (2026-09-04, 8AG)**: /aft_mapped_to_init
  reports ~0 translation on pans/pivots/in-place-tips because the sensor doesn't translate. Verified cold on 121658
  (3cm/0.6deg), 083911 & 102338 (1-3cm), and a live 10deg in-place tip 143849 (0.25m, start≈end). Odometry needs LINEAR
  DISPLACEMENT of the sensor head; ANGLE is irrelevant (the proven 8in tip of line 150 worked because it TRANSLATED the
  head via a long lever, not because it tipped). To get real poses: TRANSLATE the rig through space (carry it / out-and-
  back), not pan/pivot/tip-in-place. This gates pose-based camera/odom texture synthesis entirely — it is a CAPTURE-
  TECHNIQUE prerequisite, unfixable in code, and the rig has been stand-committed so it's essentially never been done.
- **CAMERA 'SILENCED' MEMORY = LEAN MODE CUT THE FUSION NODES, NOT THE CAMERA (2026-09-04, 8AG)**: lean capture (8M)
  dropped RViz + the two fusion nodes to save CPU but KEPT gscam. The camera's absences are the involuntary GSCAM HANG
  (recurring, root cause open), never a deliberate mute. Clear with camera_unwedge_9_4.sh. Diagnosis ladder: gscam
  running? /dev/video0 present? lsusb 0c45:0578? — all yes + topic silent = wedge (run the tool); device/USB missing =
  hardware/cable (do not run the tool).
- **CAPTURE BUTTON != TOGGLE + v3.1 EARLY-SWITCH (8AF)**: second tap mid-take = clean-slate kills Point-LIO
  (false stop-edge: snapshot+mesh of a PARTIAL map, v3.1 flips panel to that partial mesh = the 'stuck
  champion' illusion) + orphans the first recorder. Sandbox gate (4 traces) reproduced it and caught the
  STALE-DONE RACE: a refused meshcheck (map<5000 vox) leaves meshstatus 'done' from an OLDER bake -> the
  panel switches to an OLD mesh with an OLD verdict. v4: process-derived capture state, bake-id in
  /meshstatus, switch only on fresh-done, refusal surfaced as error.
- **REPO STALE-NAME TRAP — CANONICAL NAMES ARE OLD (mapped 2026-09-03, 8AD)**: repo rig_kiosk.html =
  9ec7b1dc (CAMERA-era fossil); the real v2 hid as rig_kiosk(4).html = afbbd32d. Repo rig_kiosk_server.py
  = 8b07c7d6 (superseded); the real one hid as rig_kiosk_server(2).py = 447b5b64. Cause: browser
  download-counter names uploaded unrenamed. Until canonical names are replaced, the repo is NOT the
  single source of deployed truth. RULE (8AA, now twice-earned): RENAME BEFORE UPLOAD, verify md5.
- **STOP RIG WAS A FOSSIL (FIXED on-Jetson 2026-09-03)**: rig_stop.sh + rig_start.sh PATTERNS lacked
  pointlio_mapping + rviz2 -> every Stop since the Aug-20 pivot left the odometry stack alive (proven:
  zombie at 119% CPU + open recorder). PATCHED on the Jetson (backups .bak_prezombiefix); ALSO
  point_lio_capture.sh rviz:=true -> false (lean is now lean). REPO COPIES OF ALL THREE ARE STALE — ship.
- **'SCAN LIVE' LABEL LIES + RUN MESH CHECK IS ALWAYS ARMED**: /cloud's SCAN status means "connected",
  not "fresh" — the server re-serves its last buffered frames (13 = one decay buffer) indefinitely after
  the rig stops. And the server's in-memory map persists until a new capture, so RUN MESH CHECK (or the
  auto-check) at ANY time re-meshes stale voxels INTO THE SERVE PATH — it clobbered the champion twice
  on 2026-09-03. Treat the button as armed; restore recipe in the /tmp gotcha; server rev adds staleness
  display + per-session mesh files. The kiosk uptime clock (02:14) is DECORATIVE — no JS updates it.
...
## 8J. FIELD DATA ARCHITECTURE — capture strategy, storage, transmission (2026-08-26)
  >>> SEE §8T "THE DATA PATH" for the settled end-to-end chain: the rig SSD is a RELAY, not an endpoint; the laptop is never on the data path. <<<
═══════════════════════════════════════════════════════════════════════════
...
═══════════════════════════════════════════════════════════════════════════
## 8P. FIELD KIOSK COMPLETED — 4-button UI, real-scale thermal gauges, fill-screen, tap-exit (Stage 8, 2026-08-30)
═══════════════════════════════════════════════════════════════════════════
Took the 8O kiosk from mock to the finished field instrument. Method held throughout: one verified
command at a time, every file backed up before edit, every server change dry-run in the sandbox
(mock mode, no ROS) before touching the Jetson. Result is live on the Waveshare, filling the screen,
gauges reading real data, tap-to-exit confirmed working.

SERVER (~/Desktop/rig_kiosk_server.py) - 5 additive edits, backed up as .orig:
  - THERMAL spec table (module-level): Jetson floor 35 / ceiling 104.5 / green 68 / yellow 80 /
    ticks 68-70-99; L2 floor 30 / ceiling 85 / green 70 / yellow 78 / ticks 70-78.
  - gauge() helper: returns {temp, frac, state, ticks, floor, ceiling}; same color logic as band(),
    per-device thresholds (COLORING = Option B: 68/80 bands drive color, real trips are reference ticks).
  - Wired d["jetson"]/d["l2"] to gauge(); mock_reader updated to emit the gauge shape too
    (L2 mock baseline corrected 38->61 to match measured operating temp).
  - 4th action verb added: start_lean -> ~/rig_start_lean.sh.
  - Sandbox-verified BY EXECUTION: /data emits full gauge shape, tick positions in-range,
    all four routes dispatch, backward-compatible with the old page.

HTML (~/Desktop/rig_kiosk.html) - rebuilt, backed up as .bak*, verified md5 5295982e68dd7c030a81ff7a776f276d:
  - Four-button 2x2 grid (Start Rig / Start Lean / Capture / Stop Rig), fat-finger sized.
  - Two real-scale thermal gauges with tick marks + scale numbers (Jetson on kernel trips,
    L2 on manual-85 ceiling with 78 prevention line).
  - Camera-reference panel REMOVED (per the section-8 point-cloud-over-camera field-display finding).
  - Capture-drive slot (reads RECORD_PATH free space; drive-aware/mount logic DEFERRED to flash-drive arrival).
  - Tap-to-exit X (top-right) -> confirm overlay -> window.close(). CONFIRMED exits Firefox by tap on
    the Waveshare - no keyboard needed (solves the "--kiosk has no exit on a keyboard-less panel" trap).
  - FILL-THE-WINDOW: 1024x600 canvas scaled to viewport on BOTH axes via fit(); fills edge-to-edge on
    any resolution (no xrandr needed). Monitor panel grouped into 3 sections, space-between, to fill height.

LAUNCHER (~/Desktop/rig_kiosk_launch.sh): sources ROS -> starts server if not already running (pgrep
  guard) -> polls until /data answers -> firefox --kiosk. Confirmed working. Firefox = plain
  /usr/bin/firefox 154 (NOT Snap), so --kiosk behaves normally.
  *** GOTCHA BANKED: the pgrep guard means an OLD server keeps running after edits - must pkill -f
  rig_kiosk_server.py and relaunch for edits to load. First hot view showed empty gauges precisely
  because the pre-edit server (no frac/ticks) was still running. Restart fixed it. ***

JETSON THERMAL - REAL kernel trip points (measured off the board, replaces earlier guesses):
  - cpu/gpu/soc/cv zones: 70 (throttle onset) / 99 (hard) / 104.5 (shutdown).
  - tj-thermal (junction): 74 / 95 / 104.5.
  - Confirms the conservative 68/80 operator bands sit safely BELOW the real hardware trips.
  - The 80C "cool off" line is an EARLY-WARNING band, not the ceiling; true shutdown is 104.5.

POWER CORRECTION - supersedes section-8H "Jetson = straight USB-C PD on the 140W port, NO trigger":
  - Jetson confirmed = Orin Nano ENGINEERING REFERENCE Dev Kit (System>About).
  - The reference dev kit's USB-C is DATA-ONLY, NOT a power input (NVIDIA docs + forum answers).
  - Powered via DC BARREL JACK, 7-20V (stock adapter 19V/2.37A, barrel 5.5x2.5mm center-positive).
  - From the Ugreen this REQUIRES a PD trigger (USB-C -> fixed-V -> barrel). The standby "20V max"
    trigger (digital-readout, selectable) is the MAIN LINE, not a fallback.
  - Ugreen bank (25000mAh PD3.1, photo-confirmed): OUT1 USB-C 140W -> Jetson (via trigger);
    OUT2 USB-C 100W -> L2 (12V); OUT3 USB-A UNUSED (5V only, cannot source L2's 12V).
    Total-output table tops at 20V=10A; combined Jetson(~25W)+L2(~12W) draw ~35W = trivial for the bank.
  - A3 BENCH TEST STILL PENDING (run before mounting Jetson to rig): test 15V baseline FIRST, meter the
    trigger output (V + center-positive polarity) BEFORE connecting to the Jetson, cross-check on the
    Ugreen TFT, then dual-load with the L2, then hold-under-load IN MAXN SUPER (the real, burstier mode).
    Reversed polarity is the one thing that can kill the board - meter is the gate. 12V is RULED OUT
    (>3A on a standard cable under MAXN); 20V or 15V only.

STORAGE - investigated, NO crisis, one standing action:
  - SD card (mmcblk0p1): 234G total, 141G free, 38% used. Card is SEALED inside a case (disassembly to remove).
  - 48 GB in Desktop/"Fusioncap scans" = 8 tracked capture sessions (Aug 21-27), SINGLE-COPY on the sealed
    card. Biggest = fusioncap_175244 (22 GB). fusioncap_152121 metadata confirmed RICH (image_raw 1245 +
    aft_mapped_to_init 6267 + imu 14961; Point-LIO ran live) = real project data, not disposable.
  - Accumulated TESTS are NOT the space problem (~1-2 GB of actual capture artifacts); the Desktop bulk is.
  - NO FREEZE ON TESTS: 141 GB is ample for all Stage-8 validation + bounded captures.
  - STANDING ACTION: back up the capture sessions OFF the sealed card when a destination exists (insurance
    against single-card failure, NOT urgent). Offload = rsync (verifies + resumes); NOT GitHub (48 GB, 100MB/file cap).
  - OFFLOAD TARGET = USB SSD (NOT a flash stick - sustained write must clear the section-8J 70-100 MB/s fill rate;
    flash sticks collapse after cache = silent dropped/corrupt capture). Format EXT4 (no 4GB file cap,
    clean Linux perms). SECURE the drive to the rig body with a SLACK cable - Jetson has ONLY USB out, so a
    direct-plug stick on a moving rig = vibration/leverage risk on the port. Considered SSK 1TB "push-pull"
    (genuine TLC-NAND SSD - fine; confirm 1000 read AND write, not the 550-write sibling). Internal M.2 NVMe
    on the carrier board = the more robust alternative IF the case allows access (bolted down, no USB dangle).

FILE-TRANSFER LESSON (banked): getting the HTML onto the Jetson repeatedly failed via browser download
  (stale/numbered copies) and plain terminal paste (mangled a 16k-char base64 line mid-stream). What WORKED:
  gzip+base64 (one-third the size) piped through a single self-verifying command that decodes, writes, and
  checks md5, printing PASS/FAIL. For big files with a flaky clipboard, prefer GitHub pull or a checksummed
  gzip+base64 one-liner over raw paste. The checksum is the gate - never trust an unverified file.

OPEN (next session):
  - KIOSK COSMETIC (2, minor): gauge scale labels overlap where ticks are close (68/70; 99/104.5);
    L2 APD shows "??"/empty when the rig is OFF (L2 not publishing apd_temperature) - CONFIRM it fills on Start Rig.
  - Kiosk NOT yet a boot autostart (still launched manually) - deferred until stable + exit proven in the field.
  - *** PUSH TO GITHUB PENDING (highest-value first task) ***: rig_start_lean.sh, rig_stop.sh, new
    rig_kiosk.html, rig_kiosk_server.py, rig_kiosk_launch.sh, the .desktop launchers, and THIS Master -
    all unpushed; repo confirmed stale on these. Session work lives ONLY on the Jetson Desktop (backed up
    as .orig/.bak locally, but not off-machine).
  - A3 power test (Ugreen -> trigger -> Jetson) not yet run.
  - B1 flow-bar hot re-test / lean camera ~28Hz confirmation still owed.
═══════════════════════════════════════════════════════════════════════════

═══════════════════════════════════════════════════════════════════════════
## 8Q. DESKTOP BUTTON, LAUNCH PAGE, CAMERA RESTORED, WIFI/FOXGLOVE, FREEZE + EXIT FIXED (Stage 8 cont., 2026-08-30/31)
═══════════════════════════════════════════════════════════════════════════
A long, productive, and instructive session. Several real bugs found and fixed; two of them (the
Jetson freeze and the kiosk-won't-exit) were architectural, not cosmetic. Everything below was
verified by execution or checksum before being trusted. Method held: one step, verify, next step.

GITHUB REPO — VERIFIED COMPLETE (fixes the recurring missing-data problem):
  - Audited the live repo by cloning it. Found 3 gaps: rig_stop.sh MISSING, rig_start_lean.sh MISSING,
    rig_kiosk.html STALE (repo md5 2fd13fb2 vs working 5295982e). All three uploaded and RE-VERIFIED
    by re-clone: all 7 dependency files present + current, rig_start_lean.sh carries its 2 #LEAN: lines.
  - ROOT CAUSE OF PAST MISSING UPLOADS: there is NO local git clone of rig-files on the Jetson (only
    3 third-party repos: unilidar_sdk2, fast_calib, point_lio_ros2). All pushes were manual web-uploads,
    which only include hand-picked files. No GitHub auth (no credential helper, no SSH key) exists on
    the Jetson. STANDING ACTION: set up a token + local clone so pushes become git add -A (catches all).
  - Gather-before-upload trick: cp the needed files into one folder (~/repo_upload/) so the web picker
    can grab them all at once instead of hunting across ~/ and ~/Desktop/.

FILE-LOCATION SPLIT — THE BUG BEHIND "NOTHING HAPPENS" (root cause found + fixed):
  - The server's SCRIPTS dict and the .desktop launcher reference ~/ (home) paths, but rig_start_lean.sh,
    rig_stop.sh and rig_kiosk_launch.sh were living on ~/Desktop/ instead. Two of four scripts happened
    to be in home (start, capture worked); two weren't (stop, lean would fail); the launcher wasn't in
    home at all (so the icon did nothing). Pure path mismatch, no bad files.
  - FIX: consolidated all four scripts + launcher into ~/ (verified all executable). Desktop copies are
    now duplicates = decoys-in-waiting. RULE: launch scripts live in ~/, not ~/Desktop/.
  - Decoy files still on Desktop worth a cleanup someday: rig_kiosk(4).html, rig_kiosk_mock.html,
    rig_kiosk.html.bak*, rig_kiosk_server.py.orig, Desktop copies of the .sh scripts, fix_*.py helpers.

DESKTOP ICON — DEAD END (banked so we never fight this again):
  - GNOME will NOT make one desktop icon larger than the rest (global size only). And .desktop launchers
    need a per-file "Allow Launching" trust step, show "broken desktop file"/red-X until trusted, need a
    desktop refresh (no F5 on a touchscreen), and are one tiny icon among ~90. Four separate GNOME
    obstacles for one icon. Desktop ICONS are the wrong tool for a keyboard-less touchscreen.
  - The custom amber "RIG KIOSK" icon (rig_icon.png, md5 cd47614a) exists at ~/.local/share/icons/ and
    RigKiosk.desktop points at it — but it never cleared GNOME trust. Superseded; can be deleted.

DESKTOP BUTTON — THE ANSWER (permanent, autostarts, confirmed across logout/login):
  - ~/rig_button.py: a Tkinter always-on-top 420x420 amber "RIG KIOSK" button WINDOW on the desktop.
    Not an icon — a window, so it can be any size, needs no GNOME trust, no refresh. Tap -> runs
    ~/rig_kiosk_launch.sh. Size/position = the geometry line ("420x420+40+40").
  - ~/.config/autostart/rig_button.desktop: launches it at every login (autostart entries do NOT need
    the Allow-Launching trust step). Verified: logged out/in, button appears by itself.
  - PATTERN: "large button on the desktop" = an always-on-top window, never a desktop icon.

LAUNCH PAGE + ROUTES:
  - Server now serves launch.html (one giant "TAP TO START RIG" button, fill-window scaled) at "/",
    and the kiosk at "/kiosk". Tap -> location.href='/kiosk'. The launcher's firefox --kiosk
    http://localhost:8080 lands on the big button. Flow: desktop button -> big button -> kiosk.

CAMERA FEED RESTORED TO THE KIOSK (correcting my own regression):
  - I had removed the camera panel citing §8 "point cloud over camera". WRONG: RViz was the ideal
    coverage display but §8M proved it too heavy and it was cut, so the CAMERA FEED IS THE COVERAGE
    REFERENCE — without it the operator can't tell what's been scanned. The full-frame kiosk was
    enlarged precisely to give the camera room. Restored: compact 4-button row on top, BIG camera panel
    beneath (the left column), monitor on the right. Polls /camera.jpg at ~8fps; shows "NO FEED — start
    the rig" placeholder when the camera isn't publishing (server returns 503 gracefully).
  - Confirmed live on the Waveshare and, via http://192.168.0.204:8080/kiosk, on the PC. Size judged
    fine ("only a reference").

WIFI MONITORING — WORKING, TWO PATHS:
  - Jetson WiFi IP 192.168.0.204 (PC is 192.168.0.83). Kiosk server binds 0.0.0.0, so the PC gets the
    full kiosk (camera + bars + gauges) in a browser at http://192.168.0.204:8080/kiosk. Zero extra
    Jetson load. This is the "second pair of eyes" for coverage.
  - Foxglove: bridge (ros-humble-foxglove-bridge 3.4.3, already installed) on the Jetson, Foxglove
    desktop on the Windows PC, connect ws://192.168.0.204:8765 (WATCH THE PORT — 8576 typo bit us).
    Foxglove is the "RViz on the PC": rendering happens on the PC, the Jetson only serializes.
    Foxglove should NEVER run on the Jetson itself (it was found installed there — that's an RViz-class
    load right back on the board). Bridge lives in a terminal; if that terminal closes the bridge dies
    and the PC says "unreachable" (bit us once). Candidate: fold the bridge into the lean start script.
  - *** RAW /image_raw CANNOT CROSS WIFI: 1920x1200 x3B x ~28Hz ≈ 190 MB/s. Do NOT subscribe Foxglove
    to /image_raw. The camera reaches the PC only via the kiosk URL (downscaled JPEG). ***
  - BRIDGE IS NOW WHITELISTED (launch with ros2 run ... -p topic_whitelist:=[cloud, aft_mapped_to_init,
    imu, apd_temperature]) so /image_raw is physically not offered — a stray click can't request it.
    Advertised-channel lines only appear once the rig is running (advertised ≠ publishing).

THE JETSON FREEZE (hard reboot required) — DIAGNOSIS + SAFEGUARDS:
  - Logs from the frozen boot were LOST: no persistent journal existed. NOW FIXED: persistent journald
    enabled (/var/log/journal), so journalctl -b -1 will work after any future crash.
  - Leading cause (circumstantial, strong): MEMORY EXHAUSTION -> ZRAM THRASH. foxglove_bridge queues
    every subscribed topic for the WebSocket; a raw /image_raw subscription (190 MB/s into a WiFi link
    that drains a fraction) balloons that queue by gigabytes. Jetson has 7.4 GiB RAM + 3.7 GiB ZRAM
    (6x635M, compressed swap IN RAM) — zram costs CPU per page, so sustained pressure = grind-to-freeze,
    not clean shutdown. Signature matches "froze solid, hard reboot". Baseline at near-idle after
    reboot: 2.7 GiB used, ~4.6 GiB available for the ENTIRE stack (Firefox 1-2GB, kiosk server, ROS
    nodes, bridge, + Foxglove-on-Jetson if open). Never far from the edge.
  - Contributors: Foxglove open on the Jetson; Firefox kiosk; full ROS stack. Thermal ruled out (trips
    shut down, don't freeze); power ruled out (stock AC adapter).
  - SAFEGUARDS NOW IN PLACE: bridge whitelist (no /image_raw), persistent journal, never run Foxglove
    on the Jetson. BANKED FOR LATER: add a RAM gauge to the kiosk next to the thermal gauges — memory
    pressure is a known failure mode and should be watched like temperature.

THE KIOSK-WON'T-EXIT BUG — ROOT CAUSE + REAL FIX (architectural):
  - Symptom: ✕ -> "Exit?" overlay appears, tapping Exit does nothing; also pkill -f "firefox.*--kiosk"
    matched nothing; and pkill -f /usr/lib/firefox/firefox killed ALL Firefox (took the Claude chat
    window down) and session-restore brought the kiosk back.
  - ROOT CAUSE: Firefox MERGES any new launch into the already-running instance. "firefox --kiosk"
    did not start a separate kiosk process — it opened a kiosk WINDOW inside the same Firefox process
    as the user's normal browsing (one parent PID, e.g. 3819). Consequences: --kiosk vanished from
    the process cmdline (handed off), the kiosk had no separate process to target, window.close() was
    refused (page reached by navigation from launch.html, not script-opened), and killing the parent
    killed everything.
  - FIX: the kiosk is now its OWN Firefox process: launcher uses --no-remote --profile
    ~/.rigkiosk_profile --kiosk (separate profile = separate process + separate session; user.js seeded
    to suppress first-run nags and crash-restore). Launcher backgrounds it and RECORDS ITS PID to
    ~/.kiosk_firefox.pid. Server's exit_kiosk route reads that PID and os.kill(pid, SIGTERM) — kills
    exactly the kiosk, never the user's Firefox. Verified in sandbox (killed pid A, spared pid B) and
    CONFIRMED ON THE JETSON: Exit closes only the kiosk; the chat's Firefox stays open. "Worked
    beautifully."
  - $! footgun checked: the server is backgrounded earlier in the script, firefox is the LAST &, so
    $! = firefox. Nested heredoc (user.js inside the launcher paste) verified to write correctly.
  - Note: exit_kiosk is handled BEFORE the SCRIPTS lookup (it's not a script path). No pidfile ->
    honest error, not a fake success.

FILE-TRANSFER (reinforced): browser downloads to the Jetson landed stale/elsewhere or not at all
  (Desktop instead of Downloads; older numbered copies). Plain terminal paste of a 16k-char base64
  line mangled mid-stream. What works every time: gzip+base64 one-liner with a built-in md5 check
  (PASS/FAIL). Under ~8k chars is comfortable. For big binaries prefer GitHub pull. Small plain-text
  scripts can be pasted as a heredoc and checksummed.

CURRENT FILE CHECKSUMS (the working, verified versions on the Jetson):
  rig_kiosk_server.py  c730d0d7  (launch@/, kiosk@/kiosk, exit_kiosk via pidfile, gauges, start_lean)
  rig_kiosk.html       9ec7b1dc  (4-button row, big camera panel, real-scale gauges, server-exit)
  rig_kiosk_launch.sh  871a5443  (--no-remote kiosk profile, records PID)
  launch.html          3bca7afe  (big TAP TO START RIG button -> /kiosk)
  rig_button.py + ~/.config/autostart/rig_button.desktop (desktop button, autostart)

OPEN (next session):
  - *** PUSH THIS SESSION'S FILES TO GITHUB *** (server, kiosk.html, launch.html, launcher,
    rig_button.py, autostart entry, THIS Master). Repo is stale again on all of them.
  - Set up GitHub token + local clone on the Jetson (ends the manual-upload missing-file problem).
  - Point cloud on the PC via Foxglove: bridge whitelisted and PC connects, but the CLOUD RENDER +
    the §8M measurement (camera Hz on the kiosk while the PC pulls /unilidar/cloud) NOT YET DONE —
    rig wasn't hot, then the freeze intervened. This is the actual "RViz on the PC" prize; it was
    the camera detour that got tested. Do it with the rig in Start Lean, watch kiosk camera Hz
    before/after subscribing the cloud, and watch free -h for memory climb.
  - Add RAM gauge to kiosk. Consider folding the foxglove bridge into the lean start script.
  - Delete the desktop decoys + the superseded RigKiosk.desktop icon.
  - Still owed from before: A3 power test (Ugreen -> 15V trigger -> Jetson, meter before connect),
    B1 lean camera ~28Hz confirmation, kiosk cosmetics (gauge label overlap; L2 "??" when off).
═══════════════════════════════════════════════════════════════════════════

═══════════════════════════════════════════════════════════════════════════
## 8R. POINT CLOUD ON THE PC — SOLVED (browser viewer, no Foxglove); exit fixed for good; Foxglove root causes (2026-08-31 / 09-01)
═══════════════════════════════════════════════════════════════════════════
The "RViz on the PC" goal is DONE, and it took a browser tab. After three sessions of Foxglove
fumbling, the answer was the same pattern that already worked for the camera: the kiosk server serves
the data, the PC opens a URL. This section also records why Foxglove kept failing, the exit fix that
finally cannot trap the screen, and a hard lesson about diagnosing before checking the rig is alive.

THE WORKING SETUP (banked):
  - POINT CLOUD on the PC:  http://192.168.0.204:8080/cloud   (kiosk server + browser tab)
  - CAMERA on the PC:       http://192.168.0.204:8080/kiosk   (same server)
  - No Foxglove, no foxglove_bridge, no whitelist, no launch-order rule. Both feeds ride the kiosk server.
  - Testable COLD: `USE_MOCK=1 python3 rig_kiosk_server.py` serves a synthetic rotating room at /cloud,
    so the whole Jetson->PC pipeline can be proven with the rig off (it was: LIVE, 1800 pts, 6.3 Hz).
  - §8M MEASUREMENT (rig hot, PC pulling the cloud) vs baseline (rig hot, nothing pulling):
      LiDAR 12.0 -> 12.0 Hz | Camera 30.5 -> 29.0 Hz | IMU 254 -> 249 Hz | mem avail 3.2 -> 3.4 GiB | Jetson 57 -> 55C
    => the viewer costs the Jetson NOTHING measurable. Offloading rendering to the PC is free, as §8M hoped.

HOW IT WORKS (rig_kiosk_server.py, md5 aab58b69):
  - The server's existing /unilidar/cloud subscription now also parses each PointCloud2 using the
    message's OWN field offsets (x,y,z,intensity; ring/time/padding ignored) -> float32 [x,y,z,i]*N,
    NaN/inf dropped, capped at 4000 pts (ceil stride), throttled to 6 fps. Stashed in CLOUD{}.
  - New routes: /cloud.bin (binary, X-Points header) and /cloud (serves cloud.html next to the server).
  - Real L2 frame ≈ 1800 pts (21,600 pts/s / 12 Hz) -> no downsampling in practice, ~29 KB/frame,
    ~170 KB/s at 6 fps over WiFi.
  - Parser unit-tested on the L2's exact 32-byte layout incl. a reordered layout and NaN points.
  - NumPy-2 gotcha fixed (ndarray.ptp removed -> np.ptp). mock_reader emits a synthetic room.

THE VIEWER (cloud.html, md5 301fddda): three.js Points, orbit/zoom/pan (mouse + touch), floor grid,
  Z-up. HUD controls, REMEMBERED in the PC browser (localStorage):
  - point size 0.01–0.40 m; DECAY 0–10 s (keeps the last N seconds of frames -> a sparse 12 Hz sweep
    reads as a surface, and the trail shows where the rig just was — useful for coverage); FADE OLD
    (older frames dimmed to 25%); ATTITUDE roll X / pitch Y / yaw Z in 5° steps.
  - Decay retention + age-fade logic unit-tested (9 cases). Rotation convention verified with three.js:
    roll +90 stands up a LiDAR lying on its side (local +Y -> world +Z).
  - THE L2 IS MOUNTED ON ITS SIDE: the viewer's attitude sliders level it. Confirmed working; the exact
    angle lives in the PC browser's saved settings — WRITE THE VALUE HERE when next at the bench.
  - Field caveat: loads three.js r128 from cdnjs (needs internet on the PC). TODO: vendor three.min.js
    onto the Jetson and serve it locally so the viewer works with no connection.

FOXGLOVE — ROOT CAUSES (why three sessions failed), all verified from source/docs, not guessed:
  1. The 3D panel only draws a cloud if a transform path exists from the cloud's frame to the panel's
     DISPLAY FRAME (Foxglove docs). From the unilidar_sdk2 driver source: cloud frame = "unilidar_lidar",
     topic "unilidar/cloud", fields x,y,z,intensity,ring,time, published RELIABLE; the driver DOES
     broadcast TF (unilidar_imu_initial -> unilidar_imu -> unilidar_lidar) but our bridge whitelist
     dropped /tf. So unless Display frame == unilidar_lidar exactly, nothing renders — silently.
  2. The bridge only SENDS a topic once a panel SUBSCRIBES. The eye toggle never took; no subscribe line
     ever appeared in the bridge log; no Hz; no points. "No panels match that criteria" = the topic name
     went into Foxglove's add-panel search box, not a panel's topic field.
  3. Bridge whitelist syntax: the `-p topic_whitelist:="[...]"` form DOES parse as a proper string array
     (confirmed with `ros2 param get /foxglove_bridge topic_whitelist`). The anchors were never the problem.
  4. THE REAL REASON THE BRIDGE ADVERTISED NOTHING for an hour: the Jetson had REBOOTED (uptime -s showed
     boot 2 min before the first bridge launch). Server dead, rig nodes gone, L2 unpowered, kiosk.log
     untouched. The bridge was correctly advertising nothing because nothing existed. LESSON (banked as a
     rule): ALWAYS confirm the source is alive (rigstat / kiosk bars) BEFORE diagnosing the pipe.
  5. rig_start_lean.sh's clean-slate kill-list names "foxglove_bridge" explicitly (pkill -9). Start Lean
     kills the bridge. If the bridge is ever used again: launch it AFTER Start Lean, never before.
  6. Foxglove deliverables built + verified as far as possible, KEPT AS BACKUP (in repo/outputs):
     RIG_pointcloud_layout.json (3D panel pre-set: /unilidar/cloud visible, followTf=unilidar_lidar,
     color by intensity; panel configs validated against Foxglove's own Python SDK layout classes; the
     desktop-export wrapper is the one unverified part — import it once cold to prove it) and
     RIG_Foxglove.url (Windows shortcut: foxglove://open?ds=foxglove-websocket&ds.url=ws://192.168.0.204:8765/
     — verified against the official docs; opens the desktop app pre-connected).

KIOSK EXIT — FIXED FOR GOOD (no server dependency):
  - The exit stopped working when the server was dead (Exit asked the server to kill Firefox; no server,
    no exit; window.close() refused). Fix: ONE PREF in the kiosk's own Firefox profile:
      user_pref("dom.allow_scripts_to_close_windows", true);   -> appended to ~/.rigkiosk_profile/user.js
    Now the page's window.close() works regardless of the server. TESTED: server killed on purpose, tap
    ✕ -> Exit -> kiosk closed. Also confirmed the ✕ at the top of the screen closes it.
  - The escape hatch from the PC if ever needed (SSH): kill $(cat ~/.kiosk_firefox.pid) — if the pidfile
    is stale: pkill -f /usr/lib/firefox/firefox (kills all Firefox on the Jetson; fine in the field).
  - The reboot resurrected an old fullscreen kiosk window via Firefox session-restore (no launcher, no
    server, stale pidfile) — that was the trap. Hardened launcher (no-server guard + watchdog + no-restore
    prefs) was built and sandbox-tested but NOT deployed by choice (keep it simple). The pref fix alone
    means a zombie kiosk can always be closed with ✕. Hardened launcher kept in outputs if ever wanted.

DESKTOP BUTTON: was not running after the reboot (autostart entry present; crashed at login or got
  killed). Restarted by hand (`python3 ~/rig_button.py & disown`). Autostart survives; if it's ever
  missing after a login, that one line brings it back. Root cause of the login failure not chased.

FACTS CORRECTED THIS SESSION:
  - The L2 connects over WIRED ETHERNET (192.168.1.2), NOT USB/serial. `ls /dev/ttyUSB*` is the wrong
    check. The lean script's "Ethernet link detected / L2 responding" lines are the right tell.
  - `pgrep -af "unilidar|gscam"` FALSE-MATCHES the bridge (its cmdline contains "unilidar" in the
    whitelist). Match on binary paths, not topic words. rigstat.sh (md5 1d5d34d9) does this right.
  - "LiDAR launched but no data detected" from the lean script is usually the L2 spin-up race when you
    go hot AFTER the script starts (link came up at 8 s; the check ran at ~5 s). Wait ~15 s, then rigstat.
  - GO HOT is a separate, explicit step from Start Lean. Always say it, never assume it.

TOOLS ADDED: ~/rigstat.sh — one-shot rig status (rates from the kiosk's own /data, no pipe/timeout
  problems; memory; bridge running + whitelisted?). Run at every checkpoint.
  FILE TRANSFER: long pastes truncate/mangle on the Jetson terminal. RELIABLE METHOD (used all session):
  gzip+base64 split into ~1.4–2 KB heredoc chunks appended to /tmp/x.b64, then
  `tr -d '\n' < /tmp/x.b64 | base64 -d | gunzip > file` + md5 check. Never trust an unverified file.

OPEN (next session):
  - *** PUSH TO GITHUB ***: rig_kiosk_server.py (aab58b69), cloud.html (301fddda), rigstat.sh,
    RIG_pointcloud_layout.json, RIG_Foxglove.url, kiosk_exit.sh, the hardened launcher (optional),
    and THIS Master. Repo is stale on all of them.
  - Write the mount attitude angle (from the viewer's saved settings) into this Master.
  - Vendor three.min.js onto the Jetson so /cloud works without internet (field).
  - Set up GitHub token + local clone on the Jetson (still all web uploads).
  - Fold the cloud viewer's coverage value into the workflow: with Point-LIO running (Capture), the
    same viewer could show /cloud_registered (the accumulated map) downsampled — the real coverage map.
  - Still owed: A3 power test (Ugreen -> 15V trigger -> Jetson), B1 lean-camera confirmation, kiosk
    cosmetics (gauge label overlap), decoy cleanup on Desktop + repo, RAM gauge on the kiosk.
═══════════════════════════════════════════════════════════════════════════

═══════════════════════════════════════════════════════════════════════════
## 8S. BACK-HALF FORK RE-OPENED: set.a.light DIRECT IMPORT = CANDIDATE (not decided); Unreal remains the stated goal (2026-09-01)
═══════════════════════════════════════════════════════════════════════════
STATUS: A POSSIBLE / MAYBE. Unreal Engine has been the goal since the project began and stays the
goal. This section records a newly-found alternative that may be simpler WITHOUT losing image quality
— and the test that decides it. Nothing is switched until that test is passed.

THE OPEN QUESTION IN THE STRATEGIC BACKBONE IS NOW ANSWERED:
  The backbone asked: "can Set.A.Light import custom meshes? If yes, scanned mesh could feed it
  directly; if no, Unreal/Blender is the destination. Decides the back half."
  ANSWER (verified on elixxier's own docs, 2026-09-01): YES. set.a.light 3D (V3; 3D Importer, needs
  v2.5.9+) imports .fbx .obj .dae .3ds .lwo .PLY .stl .gltf .glb (they recommend fbx/obj/dae). The
  importer has a unit/size/scale step and auto-detects materials; V3 markets "wide outdoor
  environments", "import entire houses", physics-based lighting, built-in Day-for-Night, lifetime
  license (no subscription), Mac + Windows. Explicit constraint: polygon count is GPU-bound — keep it
  modest; elixxier themselves recommend Blender to fix/prepare meshes before import.

WHY THIS IS TEMPTING (the streamlined pipeline it would give):
  capture -> bake (textured mesh, §8I, already built) -> Blender prep -> set.a.light import ->
  relight / re-dress / re-camera -> photoreal stills.
  It removes: Unreal itself, the LiDAR Point Cloud plugin, the cloud game-engine learning curve, and
  three pipeline hacks the Unreal path needs (see below). The relighting happens in the tool that IS
  a lighting previs, with REAL film fixtures (SkyPanels etc.) instead of generic game lights. It is
  literally the backbone's stated core value: "Set.A.Light, but the set is the real scanned location."

WHY IT IS NOT YET THE DECISION (the gate):
  The four mandatory properties still rule: PHOTOREAL (indistinguishable from a photograph) is #1.
  set.a.light's renderer is a real-time physically-based previs renderer; Unreal (Lumen / path tracer)
  is a full production renderer. Whether set.a.light's stills meet the photoreal bar on a SCANNED mesh
  (not its own clean library geometry) is UNKNOWN and must be MEASURED, not assumed.
  *** THE GATE: an A/B on the SAME prepared mesh — render the identical hero view in set.a.light and in
  Unreal (or against the photoreal reference), same lights, same camera. If set.a.light's image is
  not visibly worse, it wins on simplicity. If it is worse, Unreal keeps the back half. ***
  Secondary unknowns to settle in the same test: importer polygon ceiling on a location-sized mesh;
  whether Day-for-Night behaves on scanned geometry; texture/vertex-colour fidelity through the importer.

WHAT IS TRUE OF THE UNREAL PATH REGARDLESS (researched 2026-09-01, keep even if Unreal wins):
  - UE 5.7 is current; the LiDAR Point Cloud plugin is built-in and still maintained (5.7 docs);
    enable under Edit > Plugins; import converts metres -> UU (1 UU = 1 cm) automatically.
  - CONFLICT 1 — MIRROR IMAGE: ROS is right-handed, Unreal is left-handed. Import as-is = mirrored
    scene (text reversed). Fix: negate Y on export, or Y-scale -1 on the actor. Must be deliberate.
  - CONFLICT 2 — points don't relight: the plugin renders points (normals-based + eye-dome lighting);
    the photoreal relight needs the MESH. And UNREAL DOES NOT IMPORT PLY -> mesh must go OBJ/FBX.
  - CONFLICT 3 — ASCII bloat: .xyz text is 5-10x bigger and slower than LAS; convert to LAS first.
  - Import mapping (x/y/z/r/g/b columns, RGB 0-255 vs 0-1) needs one deliberate check.
  - Machine: Shadow PC Power ($54.99/mo flat, RTX A4500-class, 20 GB VRAM, 28 GB RAM, persistent Win11,
    real support, explicitly markets UE5) beats per-minute Vagon DURING LEARNING (breakeven ~48 h/mo);
    Vagon/Paperspace per-minute wins later when usage is spiky. Vagon's greyed-out "+" is a documented
    Epic Launcher bug: Samples -> add any free sample -> back to Library -> "+" works. Do all account
    setup OFF the meter; install once (persistent); shortcut UnrealEditor.exe; STOP the machine.
  - Either destination still needs a real GPU (the Celeron cannot run UE5; set.a.light also wants a
    dedicated GPU) — Shadow serves both.

THE STEP THAT IS COMMON TO BOTH PATHS (so it is not wasted whichever wins):
  BLENDER PREP of the baked mesh: decimate to a polygon budget (~200-500k faces, colour preserved),
  SPLIT the fused scan into separable elements (walls / floor / ceiling / furniture) with their own
  materials so set-dressing can be swapped (replace furniture, change wallpaper) and holes patched,
  then export OBJ/FBX (and keep PLY). Blender is free, imports PLY natively, and is right-handed
  Z-up like ROS (nothing flips). This IS the "heavy art pass per hero shot" the backbone already
  budgets for. Build + test the decimate/split/export step in the sandbox before the first bake.

UNCHANGED BY ANY OF THIS: photoreal quality still hinges on MULTI-VIEW CAPTURE (§8I rough-mesh
  note: single-vantage lumps, 36% untextured). No back-half tool fixes that; capture strategy does.

DECISION OWNER: operator. RE-EXAMINE when the A/B is run. Until then Unreal = goal, set.a.light =
  candidate.
═══════════════════════════════════════════════════════════════════════════

═══════════════════════════════════════════════════════════════════════════
## 8T. A3 POWER TEST — IDLE HALF PASSED; the staged LOAD test defined; ***THE DATA PATH (SSD = RELAY, NOT ENDPOINT)***; laptop prepped (2026-09-01)
═══════════════════════════════════════════════════════════════════════════
A3 (Jetson on battery) — IDLE HALF: PASSED, with numbers.
  - Jetson trigger (YTADNETH 169P, DC OUT 5/9/12/15/20 V, OUT CURRENT 3 A MAX) set to 15 V, metered
    15.0 V, centre-positive. (At the 20 V setting it meters 20.1 V — within PD's ±5% band, harmless,
    kept as fallback only.) 3 A max => 45 W ceiling at 15 V = the stock adapter's rating. ADEQUATE.
  - Order that worked: connect BOTH triggers to the Ugreen first (Jetson 15.0 on the 100 W port,
    L2 12.0 on the 140 W port — port assignment does not matter electrically; both far exceed the
    loads), confirm both displays, THEN plug the barrels in. Reason: multi-port banks can renegotiate
    all ports when a second device is plugged in = a power blip that could reboot a running Jetson.
  - RESULT on the big monitor, no Waveshare: CLEAN BOOT. `nvpmodel -q` = MAXN_SUPER (no down-shift).
    tegrastats: VDD_IN 4.2 W idle at the board; CPU 729 MHz idle bursting to 1728 MHz (the Orin Nano
    Super max) = full clocks, NO throttling; ~50 C all zones; swap 0. Ugreen readout 4.5 W steady
    (0.3 W trigger loss) at 81%, bank estimating 21 h AT IDLE (optimistic).
  - RUNTIME PLANNING NUMBER: full rig load ≈ 35 W (Jetson ~20 + L2 ~10 + losses) => ~2 h from a full
    charge. Fine for §8J bounded passes; a second bank or a recharge break covers a long day.
  - VERDICT: 15 V is THE setting. 20 V unnecessary. The Jetson-on-battery unknown is resolved at idle.

A3 — LOAD HALF: DEFINED, NOT YET RUN (waits for the 1 TB SSD). Two stages, one variable at a time:
  STAGE 1 — full stack on battery, big monitor for visibility:
    Jetson 15 V + L2 12 V (both negotiated before barrels), camera on Jetson USB, 1 TB SSD on
    Jetson USB, monitor. Boot -> nvpmodel -q (still MAXN SUPER with the SSD drawing USB power?) ->
    Start Lean -> GO HOT -> rigstat at 5 min vs wall baseline (LiDAR 12 / Camera ~30 / IMU ~254 /
    Jetson ~57 C / mem ~3.2 GiB avail) -> then a REAL 2-3 min CAPTURE writing to the SSD (LiDAR +
    camera + Point-LIO + bag record = the true full load, never yet run on battery) -> rigstat during.
    Record Ugreen wattage at each step (idle 4.5 -> lean -> capturing) and battery % before/after.
  STAGE 2 — add the Waveshare, unplug the monitor = FULLY UNTETHERED, the field configuration:
    same run, watched only from the PC (kiosk URL, /cloud, SSH); desktop button -> kiosk -> Start
    Lean -> Capture by touch. Same measurements; the Waveshare's few watts show on the Ugreen.
  PREREQUISITES: SSD formatted EXT4 and RECORD_PATH repointed at it (else the capture writes to the
    sealed SD card and the drive isn't tested); both triggers metered at the start.
  FAIL SIGNATURES: camera/IMU Hz below baseline (throttle); Jetson temp climbing toward the 68 C
    line inside the enclosure; bag write errors or the SSD dropping out under vibration; any bank
    renegotiation blip. Wall adapter within reach throughout.

*** THE DATA PATH — READ THIS FIRST. THIS MUST BE CLEAR BEFORE ANY SHOOT. ***
  THE SSD ON THE RIG IS A RELAY, NOT AN ENDPOINT. It is the FIRST STOP, never the destination.
  The captured data moves DOWN a chain; the laptop only looks ACROSS it:

      [RIG]  L2 (Ethernet) + camera (USB) -> JETSON -> 1 TB SSD          <- data is BORN and CAUGHT here
                                                        |                   (70-100 MB/s: only the Jetson
                                                        v                    can catch the firehose)
      [BENCH] Jetson rsyncs SSD -> ARCHIVE HDD (two copies now)          <- data LIVES here, permanently
                                                        |
                                                        v
      [BENCH] Jetson BAKES from the SSD -> textured mesh (small)        <- the only thing that travels
                                                        |
                                                        v
      [CLOUD] Drive folder -> cloud lighting machine -> photoreal stills <- the MESH is lit here, never raw
                                                        |
                                                        v
      [RIG]   SSD verified-copied -> WIPED and reused on the next shoot   <- the relay is recycled

  THE LAPTOP IS NOT ON THIS CHAIN. At every stage it is a WINDOW (kiosk + /cloud tabs, a lightweight
  ~300 KB/s preview) and a KEYBOARD (SSH runs commands ON THE JETSON, e.g. check_capture.py — the script
  executes on the Jetson, reads the Jetson's SSD, and only the verdict text reaches the laptop).
  THE LAPTOP NEVER STORES, RECEIVES, OR REVIEWS RAW CAPTURE DATA. Not one byte.

  WHY THE SSD CANNOT MOVE TO THE LAPTOP (physics, not preference): the data is generated INTO the Jetson
  (LiDAR by Ethernet, camera by USB) at 4-6 GB/min. Putting the drive on the laptop would require the
  Jetson to ship that across WiFi, which carries under a tenth of it and DROPS PACKETS = CORRUPTION
  (§8J). Recording is WIRED and LOCAL; monitoring is WIRELESS and LIGHT. The drive lives where the
  firehose is. (Also: the laptop cannot read ext4 and has ~11 GB free.)

  WHERE "REVIEW" HAPPENS: the go/no-go verdict is computed ON THE RIG (check_capture.py on the Jetson,
  shown on the Waveshare via a CHECK button — planned) so no laptop is needed for the decision. Looking
  at the finished cloud/mesh happens on the Jetson (bake, viewer) or on the cloud machine after the mesh
  ships. Never on the laptop.

STORAGE ARCHITECTURE (settled; raw captures are large + irreplaceable, deliverables small + regenerable):
  capture (1 TB SSD on the rig, ext4, RECORD_PATH) -> FIELD MIRROR (second SSD, rsync at session end;
  RULE: no single copy leaves a shoot day) -> HOME ARCHIVE (4-8 TB HDD/NAS, permanent; the field SSD
  stays the 2nd copy until the next shoot wipes it) -> OFFSITE (cloud: deliverables always, raw for
  paying jobs). The cloud lighting machine holds DELIVERABLES ONLY, never raw. 1 TB ≈ 3-4 h of raw
  recording, but a real shoot day is ~20-60 GB (the 8 sessions = 48 GB). GitHub is NOT for data.
  LIVE RISK RIGHT NOW: the 48 GB of tracked sessions are SINGLE-COPY on the sealed SD card -> first
  rsync target when the SSD is in. The sealed card is for the OS, nothing irreplaceable.

POST-CAPTURE FLOW ("then what?") — THE JETSON IS THE INGEST HUB, NOT THE LAPTOP:
  (the laptop is Windows with ~11 GB free; it cannot read ext4 and cannot hold raw data — it never
  touches the bags)
  1. ON SITE, before leaving: check_capture.py on the new bags (GOOD/CAUTION/RESHOOT) — re-shoot a
     bad pass while still standing there.
  2. AT THE BENCH: archive drive plugged into the Jetson next to the SSD; rsync with checksums
     SSD -> archive (any format: Linux reads NTFS/exFAT, so a plain Windows HDD works and stays
     laptop-browsable). Two copies now exist.
  3. BAKE on the Jetson from the SSD -> textured_mesh.ply + .xyz. CAVEAT: §8I ran the bake IN THE
     SANDBOX (~3 GB RAM) and says "re-test on Jetson" — STILL OPEN. Fallback = bake on the cloud PC.
  4. Blender PREP (decimate, split into swappable elements, materials, OBJ/FBX) on the Jetson or the
     cloud PC; ship only the small mesh via the Drive folder to the cloud machine for lighting.
  5. Recycle the SSD after the archive copy is verified.

GENERIC PC (HP Celeron) — PREPPED AS A THIN CLIENT ONLY:
  - Its role: kiosk tabs, /cloud viewer, SSH, Drive relay, and a streaming client (<1 GB). NOTHING
    heavy is installed on it — Unreal/Blender/set.a.light all live on the cloud machine.
  - Disk: 58 GB total. Reclaimed 3.1 GB (hibernation off: `powercfg /h off` in an ADMIN PowerShell —
    the plain window refuses it). Epic Games Launcher uninstalled (had to Exit it from the tray first).
    C:\Windows\SoftwareDistribution holds 9.46 GB = a downloaded-but-not-installed Windows feature
    update; neither PowerShell deletion nor cleanmgr can release it -> LET THE UPDATE INSTALL (reboot)
    and it returns the ~9 GB on its own. ~11 GB free is already enough for the laptop's real job.
  - PowerShell gotchas banked: pasting text that includes the old prompt ("PS C:\Users\...>") makes
    PowerShell run "PS" as Get-Process; paste = right-click; clicking inside the window enters
    Select mode and pauses everything (title says "Select") — press Enter/Esc.
  - Cloud machine decision still open: Shadow PC Power recommended for the learning phase (§8S).

OPEN (next):
  - When the 1 TB SSD arrives: format ext4, mount, repoint RECORD_PATH, rsync the 48 GB off the SD
    card, then run A3 LOAD Stages 1 and 2 above.
  - Re-test the bake on the Jetson (open since §8I).
  - Buy the archive HDD (4-8 TB) and a second field SSD before the first real shoot day.
  - Push this session's Masters (20.13.7/8/9) + rigstat/cloud/server files to GitHub.
═══════════════════════════════════════════════════════════════════════════

═══════════════════════════════════════════════════════════════════════════
## 8U. THE 1 TB SSD IS LIVE: formatted, mounted boot-safe, 543 MB/s, capture repointed, the 48 GB RESCUED AND VERIFIED (2026-09-01, evening)
═══════════════════════════════════════════════════════════════════════════
The capture drive went from box to operational in one session, and the oldest risk on the books —
the single-copy sessions — is closed. All prerequisites for the A3 LOAD test (§8T) are now met.

THE DRIVE (on the Jetson, wall power for setup):
  - Samsung-class "Portable SSD", /dev/sda, 953.9 G. Factory partitions wiped (wipefs), GPT + one
    partition, ext4, label RIGDATA, UUID 6f04cd8c-d60a-4013-a83f-8be0adfb0a86.
  - MOUNT: /mnt/rigdata via fstab BY UUID (port/order independent) with **nofail** and
    x-systemd.device-timeout=5 — THE DRIVE IS OPTIONAL AT BOOT. Without nofail a missing drive would
    hang the boot, which on the keyboard-less rig looks like a dead Jetson. Owned by fasterbybaxter.
  - SPEED, measured (dd 4 GB, oflag=direct, cache bypassed): **543 MB/s sustained write** — 5x the
    worst-case 100 MB/s capture firehose, on the port/cable as installed. Write capacity settled.

CAPTURE REPOINTED (point_lio_capture.sh, backup at ~/point_lio_capture.sh.bak_pressd):
  - Bags + PCDs now land on /mnt/rigdata. WITH A GUARD: if /mnt/rigdata is not a mountpoint the
    script WARNS and falls back to ~/Desktop (SD card) rather than writing into an empty mount dir
    (which would silently fill the SD card while looking like the SSD). Lines 40-47.

THE 48 GB RESCUE — DONE AND VERIFIED:
  - The sessions were NOT in ~/Desktop/fusioncap_* — they live in ~/Desktop/"Fusioncap scans"/ (note
    the SPACE in the name; earlier greps missed it). 8 sessions there + tau_pan_v3 = 53.5 GB.
  - rsync COPY (not move) to /mnt/rigdata: 53.48 GB in 10m14s at 83 MB/s (SD-card read = the ceiling;
    incidentally proves the SD reads at capture rate). Then a full CHECKSUM VERIFY pass
    (rsync -rinc): ZERO differences. Every session now exists on TWO devices.
  - *** THE SINGLE-COPY RISK CARRIED SINCE §8J IS CLOSED. *** The SD card keeps its copy; the SSD
    carries the second until the home ARCHIVE HDD exists (still to buy), per the §8T data path.

PC <-> JETSON LINK, HONEST STATUS: the link itself has never failed — every "unreachable" traced to
  a dead endpoint (bridge not running, silent reboot, port typo, dead server). Four fragilities:
  1. DHCP: .204 is a lease. EVERYTHING hardcodes it. DO THE ROUTER RESERVATION (open, high value).
  2. Idle SSH drops ("client_loop: send disconnect"): fixed on the PC with ~/.ssh/config keepalive
     (ServerAliveInterval 30) + shortcut — connect is now just `ssh rig`.
  3. PC bookmarks for :8080/kiosk and :8080/cloud — saved.
  4. THE FIELD HAS NO ROUTER: hotspot/travel-router is the last untested field-kit piece. Dry-run it
     at home (note: hotspot WILL assign a different Jetson IP — argument for the travel router).

OPEN AFTER TONIGHT: run A3 LOAD Stages 1+2 (§8T — all prereqs now met); router reservation for .204;
  hotspot/travel-router dry run; buy archive HDD + second field SSD; re-test the bake on the Jetson;
  vendor three.min.js; record the mount attitude angle; GitHub push (Masters 7-11 + session files).
═══════════════════════════════════════════════════════════════════════════

═══════════════════════════════════════════════════════════════════════════
## 8V. RIG ASSEMBLED; RIG WATCH on the PC; FIRST BATTERY WALK — live chain PROVEN, bag lost to a power cut; STOP-BEFORE-POWER rule (2026-09-01, night)
═══════════════════════════════════════════════════════════════════════════
THE RIG IS ASSEMBLED. Everything on the chassis: L2 front, Jetson enclosed mid-rig, Waveshare on
the articulating arm, triggers + Ethernet aboard, antennas clear, on the C-stand. Cabling untidy
(acknowledged, deferred) EXCEPT two items flagged as pre-walk requirements:
  1. SECURE THE YTADNETH TRIGGER BODY to the frame (velcro/zip tie). Hanging by its cables = tug on
     the Jetson barrel under handheld movement = silent power cut. (See below: likely tonight's cause.)
  2. MOUNT THE SSD to the frame — a drive dangling from its USB plug is the dropout-under-vibration
     failure mode. Location on frame TBD.

RIG WATCH (PC integration — the hands-full problem solved):
  - Requirement: one icon clicked BEFORE walking to the rig; the PC waits, then opens everything
    itself when the rig comes up. Built: %USERPROFILE%\rig_watch.ps1 + a desktop shortcut.
  - Behaviour: polls http://192.168.0.204:8080/data every 3 s showing dots; on first 200 it opens
    the kiosk tab, the /cloud tab, and a PowerShell running `ssh rig`; window titled RIG WATCH.
  - GOTCHA BANKED: the PC's Desktop is REDIRECTED INTO ONEDRIVE (C:\Users\janes\OneDrive\Desktop).
    "$HOME\Desktop" does not exist -> DirectoryNotFoundException. Always resolve with
    [Environment]::GetFolderPath('Desktop').
  - STILL OWED: passwordless SSH key (the auto-opened SSH window times out at a password prompt when
    hands are full — the exact scenario RIG WATCH exists for); and the PowerShell paste hygiene rule
    (copy OUTPUT ONLY, never the prompt; a poisoned clipboard replays errors forever — when in doubt,
    close the window, open fresh, TYPE the short commands).

FIRST BATTERY WALK (~2 min, handheld) — WHAT IT PROVED AND WHAT IT COST:
  PROVEN, live and untethered, with a second observer on the PC:
    - Boot on battery at the rig (triggers negotiated first, 15.0/12.0), Waveshare desktop, RIG button.
    - RIG WATCH populated the PC when the rig came up.
    - THE LIVE CHAIN END TO END: L2 hot on battery -> kiosk server -> WiFi -> /cloud tab RENDERING THE
      MOVING SCAN during the walk (observer confirmed the LiDAR image changing). The monitoring system
      did its job on its first field-style outing.
    - Capture RAN on battery: 8.9 GB written to /mnt/rigdata during the walk (repointed script worked).
  COST: the rig LOST POWER at/near the end of the walk (found off afterwards; Ugreen still charged).
    The capture was STILL RECORDING when power died -> fusioncap_193652 had 8.9 GB of .db3 but NO
    metadata.yaml, and the db3 itself was TORN ("database disk image is malformed" — reindex failed
    even with -s sqlite3). DECISION: not worth salvaging (sqlite3 .recover was the path, abandoned by
    choice); folder deleted; a two-minute re-walk with a clean stop is a better test than a salvage.
    Root cause of the power cut NOT YET confirmed — the dangling trigger is suspect #1;
    `journalctl -b -N` (persistent journal) will show the abrupt end. Diagnose at next bench session.

*** RULES BANKED (write these on the rig if needed): ***
  1. STOP ORDER, ALWAYS: Capture STOP (button again) -> Stop Rig -> clean shutdown -> ONLY THEN
     remove power. EXITING THE KIOSK DOES NOT STOP A CAPTURE. A bag mid-write at 70+ MB/s when power
     dies is a torn, likely unrecoverable file.
  2. WHEN THE PC CANNOT REACH THE RIG: look at the UGREEN AND THE WAVESHARE FIRST (ten seconds,
     answers "is it even on") before any network diagnosis. Same §8R rule, PC edition — broken again
     tonight, ten minutes lost to SSH against a dark rig.
  3. THE STAGES EXIST FOR A REASON: tonight jumped to Stage 2 (walking) without Stage 1 (everything
     together, STATIC, short capture, clean stop, numbers). What broke is exactly what Stage 1
     catches on a stand. NEXT SESSION RUNS STAGE 1 FIRST, then the walk again. Operator's own call,
     seconded.

SSD state after cleanup: 50 G archive intact, 841 G free.

QUEUE (next session, in order):
  1. Secure trigger + SSD to frame; wiggle-test the barrel connections.
  2. journalctl on the walk's boot: confirm the power-cut signature and time.
  3. A3 STAGE 1 static: battery, stand, boot -> lean -> hot -> 2-3 min capture -> STOP -> 
     check_capture.py verdict on a CLEAN bag -> clean shutdown. Ugreen W + % at every step.
  4. Stage 2: the walk, repeated with the full stop order.
  5. SSH key for RIG WATCH's window; router reservation for .204 (both still open).
═══════════════════════════════════════════════════════════════════════════

═══════════════════════════════════════════════════════════════════════════
## 8W. MESH MOVED EARLY: the coverage map + MESH CHECK instrument (built, gated, re-engined; STAGED FOR DEPLOY) (2026-09-02/03)
═══════════════════════════════════════════════════════════════════════════
THE IDEA (operator's): the mesh has a SECOND JOB - an INSTRUMENT at capture time, not only a
deliverable at the bench. Points always look complete; only a SURFACE is honest about holes. §8I
proved the biggest quality lever is coverage at capture; this build puts a coverage-honest mesh
in the operator's hand ON SITE, before leaving. Two meshes, two standards: instrument = fast,
blocky, honest; deliverable = slow, beautiful, at the bench. The §8S back-half fork is untouched.

WHAT WAS BUILT (all sandbox-tested, mock-mode end-to-end, NOTHING deployed yet):
  1. LIVE COVERAGE MAP: the kiosk server subscribes to Point-LIO's /cloud_registered (topic name
     VERIFIED in point_lio_ros2 source; world frame "odom"; publishes only when odom_only=false -
     if absent live, check that flag). Points accumulate into a bounded voxel map (5 cm, hard cap
     2M voxels ~= 56 MB, capped flag exposed, per-voxel HIT COUNTS = how many scans saw it).
     Map clears on each capture start; /map.bin + /map.json routes; ~150k-pt snapshots on the wire.
     Merge cost measured: 101 ms to fold a 2 s batch into a 3M-voxel master (sandbox x86) - safe
     beside a recording capture. The map builds ONLY during Capture (Point-LIO runs then) - by design.
  2. MESH CHECK: /action?do=meshcheck snapshots the map and meshes it in a subprocess; /meshstatus
     polls; /mesh.bin serves. Binary format RGM1: u32 nv, u32 nf, f32 verts, f32 colors, u32 faces.
  3. VIEWER v4 (cloud.html): three modes SCAN / MAP / MESH. MAP = the world accumulating live,
     coloured by coverage. MESH = RUN MESH CHECK button + progress + rendered surface. Coverage
     ramp everywhere: RED = thin (walk here again) -> yellow -> GREEN = well-seen. All prior
     controls kept (size/decay/fade/attitude, remembered).
  4. MOCK MODE walks a synthetic room (with a deliberate hole) so the ENTIRE chain runs with no rig.

THE ENGINE STORY (the Open3D lesson - verify wheels BEFORE building on a library):
  v1 used Open3D Poisson (density-trim + bbox crop for honesty): 8.8 s full room, all tests passed.
  DEPLOY CHECK FOUND: Open3D ships NO aarch64 wheel for the Jetson's Python 3.10. Rather than
  build-from-source pain, MESH CHECK was RE-ENGINED on MARCHING CUBES over the voxel occupancy
  grid (scikit-image): holes stay open BY CONSTRUCTION (an unscanned patch has no voxels, so it
  gets no surface - no Poisson balloon to trim), no normals needed, and 29x FASTER: 0.3 s full
  room, 0.2 s in the e2e. Blocky at voxel scale = correct for an instrument (shows what you HAVE).
  Poisson remains the DELIVERABLE-side bake (sandbox / cloud machine, where Open3D runs fine).

HONESTY PROOF (the reason to trust it, both engines, plus end-to-end):
  synthetic room with a deliberately unscanned 1.2x0.8 m wall patch and a single-pass thin zone:
  - HOLE: 0 mesh vertices in the patch vs 527/349 in matched control patches. OPEN, not membraned.
  - THIN ZONE reads red 0.97 vs 0.50; well-seen reads green. The "where do I walk next" signal.
  - meshbin parses byte-exact in the browser parser (node round-trip test).
  - Full HTTP chain in mock: map grew 9.3k->27.6k voxels, meshcheck done, hole honest, all routes 200.

*** DEPLOY SET - THE DOWNLOADS, BY NAME (ship these to GitHub) ***
  FOUR FILES TO THE JETSON at ~/Desktop/ (md5 first-8 | bytes):
    rig_kiosk_server.py   8b07c7d6 | 24792 B   (map accumulation + meshcheck routes; replaces aab58b69)
    cloud.html            bd2426b2 | 17027 B   (SCAN/MAP/MESH viewer; replaces 301fddda)
    mesh_check.py         427cb266 |  4993 B   (marching-cubes instrument mesher - NEW)
    map_accumulator.py    6686b43a |  3710 B   (bounded hit-counted voxel map - NEW)
  JETSON PREREQUISITE (one line, ROS-SAFE PINS - numpy 2.x can break cv_bridge's compiled ABI):
    pip3 install --user "numpy<2" "scipy<1.14" "scikit-image<0.25"
    (resolves to numpy 1.26.4 / scipy 1.13.1 / scikit-image 0.24.0 - aarch64 wheels VERIFIED on PyPI)
  DEPLOY ORDER: pip line -> chunk the 4 files -> md5 check -> restart kiosk server -> USE_MOCK=1
    bench test (map grows, meshcheck runs, mesh renders) -> then live during A3 Stage 1.
  BONUS: mesh_check on the Jetson is a mini-bake -> half-closes the old "re-test bake on Jetson" item.

FIELD FLOW ONCE LIVE: Capture (MAP accumulates, watch coverage build on the PC) -> STOP (clean,
  per the §8V order) -> MESH mode -> RUN MESH CHECK (~1-2 s) -> orbit the surface -> RED = walk
  again, hole = go back -> only then pack up. The §8I single-vantage/36%-untextured failure mode
  dies here, at the source.
═══════════════════════════════════════════════════════════════════════════

═══════════════════════════════════════════════════════════════════════════
## 8X. EXTRINSIC SELF-REFINEMENT: the R&D arc, the honest findings, and the PIVOT to Koide's toolbox + our scorer (2026-09-03)
═══════════════════════════════════════════════════════════════════════════
THE IDEA (operator's): the camera-lidar extrinsic is a CONSTANT OF THE CHASSIS, and every bag is
evidence about it. Project map points into all frames that saw them; the true extrinsic makes the
sampled colours AGREE. Score = per-point cross-frame colour variance; minimise over 6-DOF.
BOUNDS (stated up front): fixes colour PLACEMENT (texture crispness, edge bleed), not coverage;
needs visual texture to grip (a blank wall constrains nothing).

WHAT THE SANDBOX PROVED (ground-truth harness: synthetic textured room, splat renderer, KNOWN
mount transform, recovery demanded from pixels alone):
  - GATE 0 PASS: 90% of points seen >=3 frames; colour fidelity 0.0037.
  - GATE 1 PASS: the score is V-SHAPED at truth on all 6 axes (75-156x edge rise). The signal is real.
  - GATE 2 (recover from a knocked bracket, 2.7 deg / 41 mm): a genuine DEBUG ARC, findings kept:
      1. Full-res photometric alignment is MULTI-MODAL (fine texture lets a shifted projection
         self-agree) -> a coarse-to-fine image PYRAMID is mandatory.
      2. scipy's optimisers (Powell, TRF) STALL on the shallow coupled valleys; a hand-rolled
         damped GAUSS-NEWTON with explicit finite-difference steps descends where they sat still.
      3. Visibility built at a wrong guess LOSES 39% of point-view pairs (the depth-edge pairs
         that carry translation) -> EM-style visibility rebuilds reclaim them.
      4. ROT-TRANS DEGENERACY: at mid-range, 1 deg of rotation ~= 3.5 cm of translation in pixel
         motion. INVERSE-DEPTH WEIGHTING (near points dominate) collapsed translation error
         34 mm -> 2.9 mm. Corollary for ANY method, banked as FIELD PROTOCOL: calibration walks
         MUST include CLOSE PASSES (translation lives in parallax) and TEXTURED surfaces.
      5. Optimisers CHEAT if allowed: candidate-dependent weights (shrink weights by moving away)
         and border-clamped sampling (eject points off-image for zero variance) both must be
         frozen/penalised. Weights frozen at the estimate; fixed residual slots with off-image penalty.
      6. HARNESS FLOOR measured honestly: starting AT truth, the optimum drifts to 0.135 deg /
         2.7 mm - the synthetic renderer's own noise floor. The isolated solve chain reached
         0.151 deg / 2.9 mm = AT the floor; method accuracy is harness-limited, not algorithm-limited.
      7. UNRESOLVED at stop: the coarse sigma-8 stage converges into a coupled-direction offset the
         sharp stages can't always escape (stage-coherence problem). Fixable (rotation-only coarse
         stage was next) - but see the pivot.

THE PIVOT (operator found szenergy/awesome-lidar; honest engineering follows):
  ADOPT: direct_visual_lidar_calibration (Koide) - "General, Single-shot, Target-less, Automatic
  LiDAR-Camera Extrinsic Calibration Toolbox" (ICRA 2023, maintained, ROS2, TAKES A ROSBAG - our
  fusioncap bags are near-native input). It is years ahead of a sandbox refiner on exactly the
  problems the debug arc rediscovered. Bench step, not field. DEPLOY CHECK OWED: arm64/Jetson
  build or Docker.
  KEEP (ours, unique value the toolbox lacks):
    1. THE CONSISTENCY SCORER as a per-bake health metric: run the (GATE-1-proven) score +
       rel_rise signal-strength probe at the FIXED current extrinsic on every bake - no optimiser,
       none of the unresolved problems - answering "is the mount still true, how strong is the
       signal" in numbers. A knocked bracket announces itself. ~50 lines from the proven machinery.
    2. The field protocol rules (close passes, texture, weak_signal honesty flag).
  DECISION PATH: deploy the scorer first; MEASURE the current fast_calib extrinsic on the archive
  (the 270-degree pans + tau_pan_v3 are ideal: same chassis, same mount, bench-quality). Healthy
  score -> skip the calibration session for now. Poor score -> one Koide calibration bag, done right.

AWESOME-LIDAR KEEPERS (beyond calibration): LAStools (proper xyz->LAS for the Unreal path);
  KISS-SLAM / FAST-LIO2-ROS2 noted as mature Point-LIO fallbacks. List: github.com/szenergy/awesome-lidar

POWER DOCTRINE REVERSED (operator finding, banked): BATTERY IS PRIMARY, WALL IS FALLBACK. The
  battery proved MORE stable than wall power (clean isolated DC, metered negotiation every plug;
  A3 idle: MAXN SUPER, full clocks, no throttle, 3%/session drain). Future bench test: Ugreen
  pass-through charging (wall->bank->rig = a UPS) - TEST for output blips on wall connect/disconnect
  before trusting it mid-capture.

BENCH CHECK 2026-09-03 (pre-deploy ground survey): SSD + 48G archive intact; thermals 51 C; load
  explained (Firefox + GNOME on the bench monitor - on the rig that Firefox IS the kiosk); capture
  repoint guard intact; deployed kiosk = OLD versions (aab58b69/301fddda) as staged-not-deployed
  should be. The walk's power cut is now FORMALLY UNEXPLAINED: bag jettisoned by choice, journal
  boot rotated out (LESSON: read journals promptly after incidents; the Jetson has no RTC battery,
  so boot timestamps start at last-saved time until NTP jumps them). Stage 1 static is the
  controlled reproduction, dangling trigger still suspect #1.

*** THE FULL GITHUB SHIP LIST (everything in outputs, by name - all current) ***
  MASTERS: MASTER_REFERENCE_20_13_13_.md (THIS FILE - current; 7..12 are the session trail)
  MESH-EARLY DEPLOY SET: rig_kiosk_server.py (8b07c7d6), cloud.html (bd2426b2),
    mesh_check.py (427cb266), map_accumulator.py (6686b43a)
  RIG TOOLS (unchanged, still current): rigstat.sh (1d5d34d9), kiosk_exit.sh (8be5521f),
    rig_kiosk_launch_hardened.sh (0ea5aa75, built/tested NOT deployed by choice),
    RIG_pointcloud_layout.json (9fdb5404), RIG_Foxglove.url (d7b83667)
  SANDBOX R&D (in /tmp, NOT in outputs - reproducible from this section if ever needed:
    extrinsic_refine.py + synth.py harness, mesh_check_open3d_version.py)
═══════════════════════════════════════════════════════════════════════════

═══════════════════════════════════════════════════════════════════════════
## 8Y. THE MINING CAMPAIGN: the archive measured, the champion crowned, the SD era autopsied (2026-09-03)
═══════════════════════════════════════════════════════════════════════════
PRINCIPLE (operator's, banked as doctrine): MINE WHAT EXISTS BEFORE CAPTURING ANEW. "Begin all
over again" must be a CONCLUSION OF EVIDENCE, not a default. The archive (53.5 GB, 9 sessions) was
measured end to end in one evening on the bench Jetson - and every §7-era mystery it contained
came back as a number.

THE TOOL: mine_bag.py (+ quatmath.py) - the OFFLINE MINER. Reads a fusioncap .db3 directly
(pure-python `rosbags`, NO ROS runtime, faster than real time): raw /unilidar/cloud + Point-LIO's
/aft_mapped_to_init odometry -> SLERP pose per scan -> world-frame registration -> the SAME
hit-counted VoxelMap + marching-cubes MESH CHECK the live rig uses -> .report.json + .ply/.meshbin.
  GATES (synthetic bag with known trajectory): GATE A - reconstructs the known room from the bag
  alone, 720/720 scans, precision/recall 1.000/1.000, hole empty, 1.6 s for a 60 s bag.
  GATE B - a simulated 15 s tracking loss is DROPPED AND COUNTED (max-pose-gap guard: scans inside
  an odometry gap are never interpolated through), and coverage counts proved LINEAR IN EVIDENCE
  (75.4% hits from 75.0% scans - the instrument neither exaggerates nor hides loss).
  REAL-BAG LESSONS: old bags have a CLOCK SPLIT (camera/driver stamps vs lidar-clock odometry) ->
  --time-source bag is the working mode for the archive; folder name "Fusioncap scans" has a SPACE
  (quote "$b" in loops - an unquoted basename overwrote every report, spotted and fixed).

*** THE ARCHIVE REPORT CARD (measured, honest, final) ***
   session   used/total  odom_s     voxels     faces    verdict
    083911    97/176      16.09     38,767   166,882   moderate (the §8I reference)
    102338    55/167       4.60     32,519   152,014   moderate (§7U's 4.60s@6903Hz - reproduced)
    121359   119/164      23.61    106,022   549,948   *** CHAMPION *** (the 8-inch-tip capture)
    175244   182/991     880.65     33,072   147,846   SD-STARVATION AUTOPSY (see below)
    185910    10/236       0.98     10,092    56,216   static-pan collapse (§7U - reproduced)
    152121 / 185945 / 192455: NO raw cloud topic (pre-record-cloud era) -> PCD path pending
    (their *_scans.pcd Point-LIO maps exist: 5.8-73 MB, world-frame, minable with a PCD loader)
  - The miner independently REPRODUCED the §7U forensics: 16.09 s, 4.60 s, 0.98 s odom spans.
  - 175244 (22 GB, the newest, reindexed in 1 s - it was only missing metadata.yaml): odometry ran
    880 s (~15 min session) but only 991 clouds exist = 1.1 Hz where the L2 makes 12 -> the slow SD
    SILENTLY DROPPED ~91% OF THE LIDAR while the bag bloated. The §2 tau note documented 40% camera
    drops; the clouds were being strangled too. *** THE SSD PURCHASE IS FORMALLY JUSTIFIED BY THE
    ARCHIVE ITSELF. *** The newest session is not the crown jewel; it is the cautionary tale.
  - THE CHAMPION SEEN (MeshLab on the bench, apt install meshlab; Back-Face: Double to see walls
    from outside): coherent room core, ONE confident green dwell zone, red almost everywhere else,
    a detached far-wall fragment, no enclosure. §8I's "single-vantage lumps" diagnosis, wearing
    its colours. VERDICT: BAKEABLE FOR A/B MECHANICS (same mesh into set.a.light + Unreal compares
    the RENDERERS), but the archive's best 23 seconds cannot meet the photoreal gate.
  - WHAT RE-SHOOTING IS FOR is now precise: ONE coverage-disciplined capture with everything the
    archive-era rig lacked and this rig has: SSD (no starvation), translation-aware walking, live
    MAP view, MESH CHECK before packing, clean stop order.

THE DEPLOY LOOP THAT CARRIED IT (now the official path, better than chunked heredocs):
  sandbox -> outputs -> PC browser downloads -> GitHub web upload -> Jetson `wget` from
  raw.githubusercontent.com -> md5 check -> run. Every hop verified; THE REPO IS THE SINGLE SOURCE
  OF TRUTH for what is deployed. (PC = courier, browser only. Jetson = consumer, SSH window.)
  NOTE: repo still carries the OLD kiosk pair (aab58b69/301fddda) - the mesh-early four are the
  ones to push before the live deploy; the old viewer has no MESH mode.
═══════════════════════════════════════════════════════════════════════════

═══════════════════════════════════════════════════════════════════════════
## 8Z. THE CAPTURE TWIN + TAU: method PROVEN to 2.8 ms on known truth; the archive honestly CANNOT answer; the next walk can (2026-09-03)
═══════════════════════════════════════════════════════════════════════════
THE IDEA (operator's): synthesize the full camera/LiDAR scenario - "we'd really be ahead of the
game." Built: synth_capture.py, THE CAPTURE TWIN - a synthetic fusioncap indistinguishable in
format from a real one, where EVERY estimated quantity is known in advance: textured ground-truth
room (with the hole), a HUMAN trajectory (speed variation + hand tremor + step bounce), camera
frames rendered through the KNOWN mount extrinsic, and a KNOWN clock split TRUE_TAU = +180 ms
(lidar lags camera - the Master's measured direction). 30 s / 360 clouds / 900 images @30 Hz /
3000 poses / 118 MB / ~2.5 min to build. The miner registers 360/360 scans off it unmodified.

TAU SOLVED IN METHOD - tau_solve.py (the §2 'triple-vetted, result pending' method, finally
resulted): camera motion signal (coarse-block frame difference) vs odometry angular speed,
rank-transformed, high-passed, cross-correlated, parabolic peak refinement. Gates are the
Master's own: peak width <100 ms AND half-split agreement <30 ms.
  *** GATE TAU PASS: recovered 177.2 ms vs TRUE 180.0 = 2.8 ms ERROR; width 55 ms; halves 1.3 ms.
  Sign convention EXPERIMENTALLY verified: LiDAR LAGS camera -> SUBTRACT from LiDAR stamps. ***
  DEBUG FINDINGS (kept): (1) raw frame-difference SATURATES at handheld speeds -> rank transform;
  (2) slow speed envelopes make a 1.2 s correlation PLATEAU -> high-pass both signals;
  (3) *** TAU'S OBSERVABILITY RIDES ON HAND TREMOR *** - the 6-12 Hz physiological band is the
  carrier. A silk-smooth trajectory is tau-blind; so is a tripod. A 30 Hz camera capturing real
  handheld tremor localizes the peak to ~55 ms width. OUR RIG IS HANDHELD = ideal. Twin realism
  was load-bearing twice (tremor; 30 Hz camera) - same lesson as the extrinsic harness floor.

THE ARCHIVE'S ANSWER - AN HONEST REFUSAL (121359, the best overlap available):
  peak corr 0.19 (twin: 0.77), width 640 ms, halves UNAVAILABLE (under 3 s of clean overlap per
  half) -> WEAK - DO NOT TRUST, by the tool's own gates. WHY, precisely: (1) the SD era's ~40%
  frame drops aliased away the 6-12 Hz tremor band tau needs; (2) the old-bag clock split
  contaminates the offset (the +484 ms figure is split-plus-tau, unresolvable); (3) 23.6 s of
  odometry cannot fund a half-split. THE SAME SD STARVATION THAT MOTIVATED THE SSD ALSO DESTROYED
  TAU'S SIGNAL. The refusal is itself a measurement.
  (Also fixed en route: overlap-aware halves split + insufficient-data guards - the first real-bag
  run crashed on empty overlap; tau_solve c985e45c supersedes d635a3b5.)

TAU'S PATH FORWARD (queued into the next hardware session, zero extra work):
  THE STAGE 2 WALK BAG DOUBLES AS THE TAU BAG - ~30 s handheld look-around at 30 Hz onto the SSD,
  one coherent clock regime, then `python3 tau_solve.py <bag>`; the gates decide. If PASS, §2's
  TEMPORAL row finally gets its number, with provenance.

WHAT THE TWIN UNLOCKS NEXT (the regression suite the pipeline never had): texturing gets numeric
  gates (known surface colours), the extrinsic scorer gets calibrated (healthy-score baseline at
  the true mount vs a knocked one), capture strategy becomes rehearsable (does MESH CHECK red/green
  PREDICT bake quality?), and every future pipeline change re-proves itself in minutes.
  HONEST BOUNDARY: the twin proves TOOL CORRECTNESS, not field performance - synthetic odometry is
  truth (real Point-LIO drifts), no rolling shutter / exposure wander. The field still owns physics.

*** SHIP LIST DELTA (upload to GitHub; full list remains in §8X) ***
  NEW:      mine_bag.py (39e2288b) | quatmath.py (cb3deea7) | synth_capture.py (df709958)
  UPDATED:  tau_solve.py (c985e45c - supersedes d635a3b5 currently on the repo)
  STILL OWED ON THE REPO: the mesh-early four (rig_kiosk_server 8b07c7d6, cloud.html bd2426b2,
  mesh_check 427cb266, map_accumulator 6686b43a) - repo currently has the OLD kiosk pair.
  ON THE JETSON (~/miner, all md5-verified): mine_bag, quatmath, map_accumulator, mesh_check,
  tau_solve(c985e45c) + the full report card (report_*.report.json/.ply/.meshbin) + toolchain
  (rosbags 0.11.5, scikit-image 0.24.0, scipy 1.13.1, numpy 1.26.4 - the ROS-safe pins held).

QUEUE (next sessions, in order):
  1. HARDWARE: secure trigger+SSD -> A3 Stage 1 static (clean stop + check_capture verdict) ->
     Stage 2 walk = ALSO the tau bag ALSO the first coverage-disciplined capture attempt.
  2. Push the mesh-early four -> deploy to the rig -> live MAP/MESH CHECK during Stage 1.
  3. PCD loader for the three cloud-less sessions (small sandbox build).
  4. Blender prep of the champion (decimate/split/export) -> A/B mechanics (set.a.light vs Unreal).
  5. Extrinsic scorer on the archive pans (§8X decision path unchanged).
═══════════════════════════════════════════════════════════════════════════

═══════════════════════════════════════════════════════════════════════════
## 8AA. REPO COMPLETE + NEXT HARDWARE SESSION PLAN (written 2026-09-03, evening)
═══════════════════════════════════════════════════════════════════════════
MILESTONE - THE REPO MATCHES THE MASTER, BYTE FOR BYTE (verified from the sandbox against
raw.githubusercontent.com): rig_kiosk_server.py 8b07c7d6 | cloud.html bd2426b2 | mesh_check.py
427cb266 | map_accumulator.py 6686b43a | mine_bag.py 39e2288b | quatmath.py cb3deea7 |
synth_capture.py df709958 | tau_solve.py c985e45c. First time repo = Master = staged truth.
Next session needs ZERO couriering; the PC's courier role is done until code changes.

SMALL FIXES BANKED:
- WAVESHARE WAS UPSIDE-DOWN IN ITS HARNESS - FIXED: instant flip via
  `DISPLAY=:0 xrandr --output <name> --rotate inverted`, made permanent via Settings->Displays->
  "Landscape (flipped)"->Keep Changes. If touch ever mismatches after a rotation change, the
  touchscreen needs its own 180-degree matrix:
  `DISPLAY=:0 xinput set-prop "<touch-device>" --type=float "Coordinate Transformation Matrix" -1 0 1 0 -1 1 0 0 1`
  (persist in the kiosk launch script if needed). Boot splash stays inverted - kernel-level, cosmetic.
- WINDOWS DOWNLOAD-COUNTER TRAP: repeated downloads become "file (2).py" etc; uploading those to
  GitHub creates NEW files instead of replacing. Rule: verify by CONTENT
  (`Get-FileHash -Algorithm MD5`), rename to the exact filename, then upload.
- PASSWORDLESS SSH: instructions issued (ssh-keygen -> append pub key to Jetson authorized_keys ->
  test). STATUS: PENDING CONFIRMATION - do first at next bench sit if not done; it makes RIG WATCH
  truly one-click.

ADDENDUM TO 8X KEEPERS (from Deephome/Awesome-LiDAR-Camera-Calibration, read 2026-09-03):
- Koide adoption CONFIRMED against the full field map (most alternatives are vehicle-shaped:
  rigid mounts, dense spinning lidars, KITTI-trained deep nets - not our handheld sparse-L2 world).
- FALLBACK #2 banked: livox_camera_calib (HKU-MARS 2021, pixel-level targetless, edge-based) -
  needs a dense STATIC accumulation, which the L2's pattern provides; try if Koide fights arm64.
- OUR SCORER'S FAMILY TREE: it is a per-bake miniature of the 2012 Pandey mutual-information
  method - independently rediscovered, repurposed from solving to monitoring. Enrichment idea:
  add L2 INTENSITY vs image grayscale as a second scorer channel (intensity is in every bag).
- TAU: the field list has ZERO temporal entries - our twin-proven tau_solve is the only game in
  town for this rig. (SST-Calib does joint spatial-temporal - noted, not needed.)

═══════════ THE NEXT HARDWARE SESSION - STEP BY STEP (run in this order) ═══════════
PHASE 0 - BENCH PREP (~20 min, monitor attached fine)
  0.1 Passwordless SSH if still pending (3 commands, PC PowerShell).
  0.2 DEPLOY MESH-EARLY, one paste on the Jetson:
      cd ~/Desktop && for f in rig_kiosk_server.py cloud.html mesh_check.py map_accumulator.py; do
      wget -q -O $f https://raw.githubusercontent.com/BaxterBstupid/rig-files/main/$f; done &&
      md5sum <the four> | cut -c1-8  -> expect 8b07c7d6/bd2426b2/427cb266/6686b43a
  0.3 Restart the kiosk server; from the PC open /cloud -> confirm SCAN/MAP/MESH tabs render.
  0.4 SMOKE-TEST MESH MODE WITH THE CHAMPION (no capture needed):
      mkdir -p /tmp/rig_meshcheck && cp ~/miner/report_121359.meshbin /tmp/rig_meshcheck/mesh.meshbin
      -> the PC browser now orbits the champion. Proves the whole viewer path live.
  0.5 Verify the Waveshare flip survived reboot (first reboot since the fix).
PHASE 1 - SECURE THE HARDWARE (~15 min) - the power-cut suspects
  1.1 Fix the dangling YTADNETH trigger to the frame (suspect #1). 1.2 Mount/strap the SSD +
  cable strain relief. 1.3 Shake test: firm handling, watch trigger display for blips.
PHASE 2 - A3 STAGE 1 STATIC (battery, ~25 min) - the controlled reproduction
  2.1 Battery boot per 8V order (triggers into Ugreen FIRST, Jetson before L2). PC = RIG WATCH observer.
  2.2 Start Rig -> Capture (verify RECORD_BASE hits /mnt/rigdata) -> sit STATIC 10-15 min.
      Watch: Ugreen W/%, kiosk temps, MAP mode (static growth is sparse - fine).
  2.3 CLEAN STOP ORDER -> check_capture verdict -> READ THE JOURNAL NOW (the rotated-boot lesson).
      PASS = no power event + intact bag on SSD + verdict GOOD/CAUTION. Note battery %.
  FALLBACK: power event -> journal immediately, photograph trigger display, retry on wall to isolate.
PHASE 3 - STAGE 2: THE WALK (~10 min) - one bag, four jobs
  THE MOTION RECIPE: translate from the FIRST second (never open with a standing pan) | keep
  moving while looking (loop + yaw sweeps WHILE walking) | VARY the pace (tau needs speed
  structure) | NATURAL handheld grip - no bracing (tremor 6-12 Hz is tau's carrier) | 2-3 CLOSE
  PASSES within ~1 m of textured surfaces (extrinsic parallax) | REVISIT part of the loop from the
  reverse direction (multi-view) | 30-60 s | clean stop.
  3.1 PC watches MAP accumulate live. 3.2 After stop: RUN MESH CHECK -> orbit -> if big reds/holes,
  WALK AGAIN NOW (this is the instrument's entire purpose). 3.3 check_capture + battery %.
PHASE 4 - THE HARVEST (bench, same sit or later)
  4.1 python3 ~/miner/tau_solve.py <walk bag>  -> gates decide; PASS = §2 TEMPORAL finally gets
      its number (update §2 with provenance!).
  4.2 python3 ~/miner/mine_bag.py <walk bag> report_walk --time-source header (new bags should be
      clock-coherent; if scans drop to zero, fall back to --time-source bag and NOTE it).
      Compare the report card against the champion (119/164, 106k voxels).
  4.3 Backup: bag stays on SSD; mirror to archive per 8T data path when at the archive drive.
SESSION PASS = Stage 1 clean + a walk bag with a MESH CHECK verdict. Everything else is bonus.
═══════════════════════════════════════════════════════════════════════════

═══════════════════════════════════════════════════════════════════════════
## 8AB. KIOSK V2 - COVERAGE REPLACES CAMERA; auto MESH CHECK on stop; the execute-your-UI gate lesson (2026-09-03, late)
═══════════════════════════════════════════════════════════════════════════
THE DECISION (operator's, and it closes the field-display question for good): the camera panel
was the right answer to an OBSOLETE question. "Camera vs nothing" dated from the pre-odometry era
when live lidar was an unregistered smear; the real choice today is "camera vs coverage," and
coverage wins outright - "if I know what is missed, there doesn't have to be a toggle." The
operator's own Polycam years supplied both halves of this design: the live fill-in display to
copy, and the melted-mesh output to defend against. §8H's industry survey (Leica/DJI/XGRIDS:
operators see coverage, not camera) now matches our kiosk instead of indicting it. Bonus, named
by the operator: the coverage panel has WOW FACTOR the camera never had - it shows the one thing
that exists nowhere else (the machine's memory of the room, assembling live), which is the same
reason it is practical. Wow and utility from one panel = the display showing the actual work.

WHAT CHANGED (one panel, two phases - no toggle):
  - rig_kiosk.html: camera <img> + /camera.jpg polling REMOVED from the page. In their place: a
    top-down COVERAGE canvas (floor-plan projection, auto-fit, red->green by log hit-count with
    95th-pct normalisation, 2x2 px dots, polls /map.bin every 2.5 s), tag "COVERAGE - red = walk
    here again," and the old dead 480x300 res label upgraded to a LIVE VOXEL COUNTER (+ capped
    flag) fed from /data. On capture stop the MESH CHECK verdict overlays the final map in the
    same panel ("N v / N f in N s - orbit on PC /cloud"). The Camera HEALTH BAR remains - that is
    the sensor heartbeat (recording monitor), not the display; red there mid-walk is a real alarm.
  - rig_kiosk_server.py: note_capture_transition() shared by real+mock loops - rising edge clears
    the map (unchanged), FALLING EDGE AUTO-FIRES MESH CHECK: the stop IS the button; the verdict
    is waiting before the operator looks at a screen. (A mid-capture tracking collapse also reads
    as a falling edge; the extra meshcheck is harmless and even informative.)
  - /camera.jpg endpoint KEPT server-side for bench framing/exposure checks - it lost its screen
    seat, not its existence.

GATES (all green, two layers):
  LIVE MOCK E2E: new page served; map builds during capture (4.2k voxels); /map.bin parses
  float32 Nx4; AUTO MESH CHECK fired on the falling edge with NO button (70,523 v / 142,970 f in
  0.2 s); map cleared on the next rising edge.
  HEADLESS PAGE EXECUTION (the gate the first pass LACKED): the page's actual <script> run in
  node against REAL /map.bin bytes - executes clean, paints 12k+ px, voxel counter reads live,
  verdict appears on stop with correct text; and via a bimodal thin/dense map (the mock's world is
  uniformly seen, so all-green there is legitimate): THIN RENDERS RED (18,651 px) and DENSE
  RENDERS GREEN (2,333 px) through the real page code.

*** THE GATE LESSON (operator asked "did you debug?" - the answer was "partially," and the gap
was real): the first "page served" gate matched the word COVERAGE in a CODE COMMENT, which is how
a leftover CAM REF label reached the operator's eyes. RULE, banked: UI GATES MUST EXECUTE THE UI,
NOT GREP IT. The headless harness (stub DOM + recording canvas + real server bytes) now exists
and is the standard for every future kiosk/page revision. Corollary from the same debug: pkill -f
with a pattern that appears in your own command line shoots your own shell - kill by stored PID. ***

SHIP DELTA (the courier run this creates):
  rig_kiosk_server.py  447b5b64  (REPLACES 8b07c7d6 on the repo - that one is superseded)
  rig_kiosk.html       afbbd32d  (NEW to the repo - the Jetson's copy is the camera-era page)
  PHASE 0 DEPLOY (now FIVE files, revised expect line):
    cd ~/Desktop && for f in rig_kiosk_server.py rig_kiosk.html cloud.html mesh_check.py map_accumulator.py; do wget -q -O $f https://raw.githubusercontent.com/BaxterBstupid/rig-files/main/$f; done && md5sum rig_kiosk_server.py rig_kiosk.html cloud.html mesh_check.py map_accumulator.py | cut -c1-8,35-
    --- expect: 447b5b64 / afbbd32d / bd2426b2 / 427cb266 / 6686b43a ---
  Cosmetic debt noted: the storage health row still says "Card" (SD era) - one word, fold into the
  next page touch, not worth a solo checksum churn.
═══════════════════════════════════════════════════════════════════════════
...
- 2026-08-28 (next-session plan set: LEAN MODE FIRST): Operator's call — lean mode is first up next session. IMPORTANT PREREQUISITE flagged: before running lean, CHECK what point_lio_capture.sh actually launches (cat ~/point_lio_capture.sh) — is it already lean (rviz:=false, no fusion nodes) or does it bring up the heavy stack? If not lean, make a lean variant first. THEN the real lean test: run it and watch the kiosk/monitor — do camera/IMU STAY GREEN (recover) instead of starving to 14Hz/168Hz (the §8M overload)? That confirms lean mode fixes the sensor-starvation. NEXT SESSION ORDER: (1) cat point_lio_capture.sh -> confirm/make lean; (2) run lean, watch sensors hold green; (3) [carried over] kiosk flow-bar re-test + cross-check vs ros2 topic hz; (4) the translation capture (the real goal). All via the proven Start Rig script, not manual source (fast_calib warning is harmless). Shut down.
- 2026-08-29 (*** LEAN MODE VALIDATED — camera starvation FIXED ***): The big result. Traced the camera starvation to its root and fixed it. Pre-capture diagnosis: with the FULL rig_start (fusion nodes running), the camera was stuck at ~13.3Hz (plateaued, not warming up) — and top showed the two python3 fusion nodes (overlay_check_node + colorized_fusion_node) each eating ~106-112% CPU (~2.2 cores), starving gscam. So the fusion nodes (NOT RViz, NOT Point-LIO) are the primary camera-starver, running continuously from rig_start.sh. Built rig_start_lean.sh (copy of rig_start.sh, the two `python3 ~/..._node.py` launch lines COMMENTED OUT with #LEAN: prefix, everything else untouched; original preserved). TESTED: ran ~/rig_start_lean.sh, top confirmed NO python3 fusion hogs (CPU idle jumped ~50%->83%), then ros2 topic hz /image_raw = ~27-28Hz (26.4-28.4 range) — MORE THAN DOUBLE the starved 13.3Hz. Camera starvation FIXED. So captures via rig_start_lean.sh record FULL-RATE ~28Hz camera instead of degraded 13Hz. GOTCHA banked: the lean script still PRINTS "Starting fusion node" (we left the echo lines, only commented the python3 launches) — misleading; caused a decoy-confusion (first run was actually the FULL script; top is the ground-truth arbiter, not the echo text). TODO tidy: change those echoes to "Fusion nodes SKIPPED (lean)". Also earlier this session: built the low-res camera REFERENCE feed into the kiosk (480x300, ~8fps, cv_bridge+cv2, throttled, served at /camera.jpg; operator wanted a "where am I pointing" reference not a tool) + made the kiosk RESPONSIVE (fills the whole screen / Waveshare instead of a fixed 1024x600 corner island — operator confirmed it looks good on the Waveshare). Delivered updated rig_kiosk.html + rig_kiosk_server.py. OPEN: the new kiosk files need to land on the Jetson Desktop (they downloaded to ~/Downloads, not Desktop — Desktop still has the OLD no-camfeed version; grep -c camfeed ~/Desktop/rig_kiosk.html returns 0; fix = mv from ~/Downloads to ~/Desktop, then confirm grep returns 2, then view). Full lean pipeline now exists: rig_start_lean (28Hz camera) + point_lio_capture_lean (no RViz) + kiosk with camera ref. NEXT: land the new kiosk files -> view full-screen kiosk with live camera + 28Hz bar; then the translation capture (the real goal). Rig off, clean pause.
- 2026-08-29 (KIOSK SEEN RUNNING + lean camera feed validated + tomorrow's plan): Resolved the kiosk file-decoy (new version had downloaded as rig_kiosk(2).html while the server served the OLD rig_kiosk.html; cp'd (2) over the real name, confirmed grep camfeed=2, deleted the (2) decoy — one clean true file now). Made the lean rig HONEST: rig_start_lean.sh echoes fixed to say "Fusion nodes SKIPPED (lean mode - full-rate camera)" instead of misleadingly printing "Starting fusion node" (that text caused an earlier decoy-confusion; top is the ground-truth arbiter). Created a "Start Rig (Lean)" desktop icon (~/Desktop/Start Rig Lean.desktop, Exec -> rig_start_lean.sh; may need right-click "Allow Launching"). THEN went hot and SAW THE FULL KIOSK RUN (finally): lean start clean ("Fusion nodes SKIPPED"), top confirmed NO python3 fusion hogs (CPU ~79% idle), camera ~28Hz. Started rig_kiosk_server.py (ROS2 mode), opened localhost:8080 -> FULL-SCREEN kiosk with LIVE camera reference tile + live bars. *** CAMERA FEED VALIDATED LIGHT: camera held 28-30Hz WITH the feed running -> the 480x300/8fps reference feed costs ~nothing, doesn't re-starve. *** Odom correctly showed "waiting" (no capture running = expected; odom only exists during a Point-LIO capture). So the COMPLETE field instrument is proven working together: lean rig (28Hz camera) + full-screen responsive kiosk + live camera reference + health bars + L2 temp. NETWORK/ARCH clarified: for R&D, Jetson is on the rig broadcasting over the local WiFi to a generic PC where operator+assistant talk — already the right setup, NO travel router needed (router is a down-the-road item ONLY for dead-signal shoot locations). Reminder: I (assistant) have NO live rig connection — I only see what's pasted; "monitoring" = operator pastes from the PC. Kiosk server listens on 0.0.0.0 so it's viewable from the PC at the Jetson's WiFi IP too (not just Waveshare localhost). L2 stays on WIRED Ethernet (192.168.1.62); any laptop-link is over WiFi (don't collide). *** TOMORROW (the big integration milestone): (1) physically mount the Jetson ON the rig; (2) BEFORE connecting battery to Jetson, confirm the battery/PD output matches the Jetson's power spec (V+W) — meter first, same "meter before connect" discipline as the L2's 12V; the Jetson is a MUCH bigger draw than the L2 (~140W PD) so battery-powers-Jetson is the REAL unknown; (3) power Jetson on battery, watch clean boot (power-starved Jetson may boot-loop); (4) plug it all together (L2+camera+leads); (5) run the LEAN test = doubles as the battery-under-load test: does the whole integrated rig hold on battery? Watch for throttling/brownout (tegrastats + kiosk Jetson temp bar). SAFETY NET: have wall-power fallback ready so a battery shortfall doesn't hard-crash mid-work. The reframe: tomorrow's lean test isn't "does lean work" (proven) — it's "does the WHOLE RIG run untethered on battery" (the real milestone). Shut down.
- 2026-08-30 (Master 20.13.5 - FIELD KIOSK COMPLETED, 8P): Took the 8O kiosk from mock to finished field instrument, live on the Waveshare. Server got 5 additive edits (THERMAL spec table + gauge() helper + wired jetson/l2 + mock updated + start_lean 4th route), all sandbox-verified by execution before touching the Jetson. HTML rebuilt: 4-button 2x2 grid, two REAL-scale thermal gauges with ticks (Jetson on measured kernel trips 70/99/104.5; L2 on manual-85 with 78 prevention line), fill-the-window canvas scaling, camera panel removed, tap-to-exit X + confirm overlay CONFIRMED closing Firefox with no keyboard. Launcher (server+poll+firefox --kiosk) working; banked the pgrep-guard gotcha (old server survives edits -> pkill+relaunch). MEASURED the Jetson's real thermal trips off the board (68/80 operator bands sit safely below). POWER CORRECTION supersedes 8H: reference Orin Nano dev kit USB-C is DATA-ONLY -> Jetson needs a PD trigger into the DC barrel jack (the standby 20V trigger is the main line); A3 bench test (15V baseline, meter before connect, dual-load in MAXN SUPER) still pending. STORAGE investigated: 141 GB free, no crisis, tests not the culprit; the 48 GB Fusioncap scans = 8 single-copy tracked sessions on the sealed card -> standing action to back up to a USB SSD (ext4, secured, NOT a flash stick) via rsync when a destination exists. File-transfer lesson: plain paste + browser download both failed; gzip+base64+md5-check one-liner is the reliable path. OPEN: 2 minor kiosk cosmetics (label overlap, L2 ?? when off), autostart deferred, A3 power test, B1 flow-bar/lean-camera hot re-test, and - TOP PRIORITY - PUSH ALL OF THIS TO GITHUB (repo stale, work only on the Desktop).
- 2026-08-30/31 (Master 20.13.6 - 8Q): Big session. Repo audited by clone and made COMPLETE (rig_stop, rig_start_lean, current kiosk.html were missing/stale; root cause = no local git clone on the Jetson, all uploads manual). Found and fixed the FILE-LOCATION SPLIT (scripts on ~/Desktop while server/launcher reference ~/) that made the launcher icon do nothing; consolidated to ~/. Desktop ICONS declared a dead end on GNOME/touchscreen (no per-icon size, trust dance, no F5); replaced with a Tkinter always-on-top RIG button WINDOW that autostarts at login — confirmed across logout/login. Launch page (big TAP TO START RIG at /, kiosk at /kiosk). RESTORED the camera feed to the kiosk (I had wrongly removed it; it's the coverage reference since RViz was cut in §8M) — live on the Waveshare and on the PC via the kiosk URL. WiFi: PC gets the kiosk at 192.168.0.204:8080; Foxglove bridge on the Jetson + Foxglove on the PC connect at ws://192.168.0.204:8765 (raw /image_raw ~190MB/s CANNOT cross WiFi; bridge now WHITELISTED to exclude it). JETSON FROZE (hard reboot): logs lost (no persistent journal -> NOW ENABLED); leading cause memory exhaustion -> zram thrash from a heavy bridge subscription with only ~4.6 GiB headroom; safeguards = whitelist + journal + never run Foxglove on the Jetson. KIOSK-WON'T-EXIT solved for real: Firefox merges launches into one process, so the kiosk was a window inside the user's Firefox; fixed by giving the kiosk its OWN process (--no-remote + dedicated profile) and recording its PID, which exit_kiosk SIGTERMs — kills only the kiosk. Confirmed working. OPEN: push everything to GitHub, set up token+clone, do the point-cloud-on-PC render + §8M camera-Hz measurement (not yet done), RAM gauge, decoy cleanup, plus the earlier A3/B1/cosmetic items.
- 2026-08-31/09-01 (Master 20.13.7 - 8R): POINT CLOUD ON THE PC SOLVED — via the kiosk server + a browser tab (http://192.168.0.204:8080/cloud), no Foxglove. Server parses PointCloud2 by field offset -> float32 xyz+intensity, 6 fps, ~29 KB/frame; viewer (three.js) with point size, DECAY (0-10 s, older frames fade), and roll/pitch/yaw ATTITUDE sliders (the L2 is mounted on its side; sliders level it; settings remembered in the PC browser). Proven cold with a synthetic room in mock mode, then hot with real geometry. §8M measured: viewer costs the Jetson nothing (camera 30.5->29.0, IMU 254->249, memory unchanged). Foxglove deep-dived and root-caused (display frame must be unilidar_lidar; bridge only sends on subscribe; the hour of 'bridge sees nothing' was a REBOOT nobody checked — rule: confirm the source is alive before diagnosing the pipe; Start Lean's kill-list names foxglove_bridge). Foxglove layout + one-click .url shortcut built as backups. KIOSK EXIT fixed for good with one profile pref (dom.allow_scripts_to_close_windows) — works with a dead server. Desktop button restored. Facts corrected: L2 is wired Ethernet (192.168.1.2), not USB; pgrep on topic words false-matches the bridge; 'no data detected' is the go-hot-late race. rigstat.sh added. Chunked gzip+base64 transfer is the reliable paste method. OPEN: push everything to GitHub, record the mount angle, vendor three.js for the field, GitHub clone, coverage map via /cloud_registered, plus the standing A3/B1/cosmetic items.
- 2026-09-01 (Master 20.13.8 - 8S, planning while on wall power): BACK-HALF FORK RE-OPENED as a possible/maybe. The backbone's open question 'can Set.A.Light import custom meshes?' is ANSWERED YES (elixxier docs: 3D Importer takes PLY/OBJ/FBX/glTF; V3 markets outdoor environments, whole houses, physics lighting, Day-for-Night). That would give a far simpler pipeline (capture -> bake -> Blender prep -> set.a.light -> stills) with real film fixtures, removing Unreal + LiDAR plugin + three Unreal-path hacks (right-handed->left-handed MIRROR, PLY-not-importable, ASCII bloat). NOT DECIDED: Unreal remains the goal; set.a.light must pass an A/B on the same mesh WITHOUT losing image quality (photoreal is mandatory #1). Blender prep (decimate, split into swappable elements, materials, OBJ/FBX) is common to both paths and is the backbone's budgeted 'heavy art pass' — build it in the sandbox next. Also researched: UE 5.7 LiDAR plugin still current; Shadow PC Power ($55/mo flat, persistent, RTX A4500/20GB/28GB) beats per-minute Vagon during learning (~48 h/mo breakeven); Vagon greyed-'+' fix = grab any free Sample then return to Library. Storage architecture settled (capture SSD -> field mirror -> home archive -> offsite; deliverables only to the cloud PC; 48 GB of sessions on the sealed SD card is the live single-copy risk). Power: Jetson trigger metered 15.0 V centre-positive (3 A max = 45 W ceiling; 20 V reads 20.1, fine, kept as fallback); L2 12 V; battery-under-load test deferred to session end.
- 2026-09-01 (Master 20.13.9 - 8T): A3 POWER TEST, IDLE HALF PASSED. Jetson trigger 15 V (metered 15.0, centre-positive; 3 A max = 45 W ceiling; 20 V reads 20.1 = fine, fallback only). Both triggers negotiated on the Ugreen BEFORE the barrels (avoids multi-port renegotiation blips), Jetson on the 100 W port (port choice irrelevant electrically). Clean boot on the big monitor; nvpmodel = MAXN_SUPER; tegrastats VDD_IN 4.2 W idle, clocks 729->1728 MHz (full, no throttle), ~50 C; Ugreen 4.5 W at 81%, 21 h idle estimate; full-load runtime planning number ~2 h. VERDICT: 15 V is the setting. LOAD half defined as two stages (full stack + SSD + monitor, then + Waveshare fully untethered, with a real capture to the SSD and rigstat/Ugreen/battery-% measurements) — waits for the 1 TB SSD (prereq: ext4 + RECORD_PATH repoint). STORAGE ARCHITECTURE settled (capture SSD -> field mirror -> home archive -> offsite; deliverables-only to the cloud PC; the 48 GB on the sealed SD card is the live single-copy risk). POST-CAPTURE FLOW settled: the JETSON is the ingest hub (verify on site with check_capture -> rsync to archive at the bench -> bake on Jetson [re-test still open] -> Blender prep -> ship the mesh via Drive); the laptop never touches raw data. LAPTOP prepped as a thin client (hibernation off, Epic uninstalled, 9.46 GB stuck in a pending Windows update -> let it install). Planning while on wall power: §8S fork recorded (set.a.light candidate vs Unreal goal, quality-gated A/B). Rig shut down clean on battery.
- 2026-09-01 evening (Master 20.13.11 - 8U): SSD LIVE. /dev/sda wiped -> GPT -> ext4 'RIGDATA', mounted /mnt/rigdata by UUID with NOFAIL (drive optional at boot - a missing drive can't hang the keyboard-less rig). Measured 543 MB/s sustained direct write = 5x the capture firehose. point_lio_capture.sh repointed to /mnt/rigdata WITH a mountpoint guard (warns + falls back to Desktop if unmounted). THE 48 GB RESCUED: sessions found in ~/Desktop/'Fusioncap scans' (space in the name defeated earlier greps) + tau_pan_v3 = 53.5 GB, rsync-copied in 10m at 83 MB/s, then full checksum verify = ZERO differences -> the single-copy risk carried since §8J is CLOSED (SD + SSD both hold every session). PC link hardened: ~/.ssh/config keepalive + 'ssh rig' shortcut, kiosk//cloud bookmarks; honest status = the link never failed, endpoints did; still open: router reservation for .204 and the field hotspot dry-run. A3 LOAD prereqs now ALL MET - Stages 1+2 queued for next session. Good day: battery idle pass in the morning, back-half fork + data path settled midday, SSD operational by evening.
- 2026-09-01 night (Master 20.13.12 - 8V): RIG ASSEMBLED (everything on the chassis; secure the dangling trigger + mount the SSD before the next walk). RIG WATCH built on the PC: one desktop icon -> polls /data -> auto-opens kiosk + /cloud + ssh when the rig answers (OneDrive-redirected Desktop gotcha banked; passwordless SSH key still owed). FIRST BATTERY WALK (~2 min): live chain PROVEN end to end untethered — observer on the PC watched the /cloud scan change during the walk; capture RAN (8.9 GB to the SSD). Rig found OFF afterwards (power cut, cause unconfirmed, dangling trigger suspect #1); capture was mid-write -> db3 torn ('disk image is malformed'), reindex impossible, salvage declined, folder deleted (841 G free, archive intact). RULES BANKED: (1) STOP order: Capture stop -> Stop Rig -> shutdown -> then power; exiting the kiosk stops NOTHING; (2) Ugreen + Waveshare FIRST when the PC can't reach the rig; (3) run Stage 1 static before Stage 2 walking — operator's own correction. Queue: secure hardware, journalctl the cut, Stage 1 static with clean stop + verdict, then the walk again.
- 2026-09-02/03 (Master 20.13.13 - 8W + 8X): MESH MOVED EARLY, built + gated + RE-ENGINED + staged. Coverage map (bounded hit-counted voxels from /cloud_registered, verified in Point-LIO source), MESH CHECK on the kiosk, three-mode viewer (SCAN/MAP/MESH, red=thin coverage ramp). Open3D has NO Jetson wheel -> marching cubes over the occupancy grid instead: holes open BY CONSTRUCTION, 29x faster (0.2-0.3 s). Honesty proven twice + end-to-end (hole 0 verts vs 349-582 control; thin zone red 0.97). DEPLOY SET staged with md5s + ROS-safe pip line (numpy<2 pin protects cv_bridge). EXTRINSIC R&D arc run to ground-truth gates: signal V-shaped (GATE 1), recovery reaches the measured harness floor (0.151 deg/2.9 mm vs floor 0.135/2.7) in the isolated chain; six kept findings (pyramid mandatory, hand-rolled GN beats scipy here, EM visibility rebuilds, inverse-depth weighting kills the rot-trans degeneracy, freeze-weights/penalty-slots stop optimiser cheats, CLOSE PASSES + texture = field protocol) ; coarse-stage coherence left unresolved BY CHOICE -> PIVOT on operator's awesome-lidar find: ADOPT Koide direct_visual_lidar_calibration for solving, KEEP our consistency scorer as the per-bake health metric (measure current extrinsic on the archive pans first; healthy = skip the session). Battery-over-wall doctrine banked (battery is PRIMARY). Bench check pre-deploy: ground stable; walk power-cut formally unexplained (bag jettisoned, journal rotated; read journals promptly). Ship list to GitHub written into 8X.
- 2026-09-03 (Master 20.13.15 - 8Y + 8Z): THE MINING CAMPAIGN + THE CAPTURE TWIN. Miner built and double-gated (reconstructs a known room 1.000/1.000 from the bag alone; drops-never-guesses through odometry gaps; counts linear in evidence), then run on the whole archive: report card FINAL - 121359 is the CHAMPION (119/164, 23.6 s, 106k voxels, seen in MeshLab: one green dwell zone, red elsewhere, bakeable for A/B mechanics, not photoreal); 175244 is the SD-STARVATION AUTOPSY (880 s odom, 1.1 Hz clouds = ~91% of LiDAR silently dropped - the SSD purchase formally justified by the archive itself); the miner independently reproduced every §7U odom-span number. THE TWIN (synth_capture.py): full synthetic fusioncap with known extrinsic + KNOWN tau 180 ms + HUMAN tremor/steps. GATE TAU PASS: tau_solve.py recovers 177.2 ms = 2.8 ms error, gates green, sign verified (LiDAR lags; subtract). Finding banked: TAU RIDES ON HAND TREMOR (6-12 Hz) - tripods are tau-blind, our handheld rig is ideal. The archive's tau attempt: WEAK by the tool's own gates (SD frame drops aliased the tremor band; clock split; short overlap) - an honest refusal, itself a measurement. Next walk bag = the tau bag. Deploy loop formalized: sandbox->outputs->browser->GitHub->wget+md5 (repo = single source of deployed truth). Ship delta: mine_bag/quatmath/synth_capture NEW, tau_solve c985e45c UPDATED; mesh-early four still owed to the repo.
- 2026-09-03 evening (Master 20.13.16 - 8AA): REPO COMPLETE - all eight ship files verified byte-perfect on GitHub against the Master's checksums (repo = Master = staged truth; zero couriering next session). Waveshare upside-down-in-harness FIXED (xrandr inverted + Settings persistence; touch-matrix one-liner banked if ever needed). Windows download-counter trap documented (verify by Get-FileHash, rename before upload). Deephome calibration-list verdicts folded in as 8X addendum: Koide confirmed vs the full field, livox_camera_calib banked as fallback #2, scorer's MI-2012 lineage + intensity-channel idea, tau has no off-the-shelf competitor. NEXT HARDWARE SESSION PLAN written step-by-step into 8AA (Phases 0-4: deploy+champion smoke test -> secure trigger/SSD -> Stage 1 static with prompt journal read -> the four-job walk with the motion recipe -> tau_solve + mine_bag harvest). Passwordless SSH pending confirmation.
- 2026-09-03 late (Master 20.13.17 - 8AB): KIOSK V2 - COVERAGE REPLACES CAMERA on the Waveshare (operator's call: the camera answered an obsolete pre-odometry question; 'if I know what is missed, there doesn't have to be a toggle'; Polycam lineage supplied both the pattern and the anti-pattern). One panel two phases: live top-down coverage map (red=walk here again, live voxel counter) during capture; AUTO MESH CHECK fires on the stop falling edge and the verdict overlays the same panel. /camera.jpg kept server-side for bench checks; camera HEALTH bar stays (sensor heartbeat). Gated twice: live mock e2e (auto-mesh 70k v in 0.2 s, map clears on restart) + HEADLESS EXECUTION of the actual page script against real bytes (paints, counter live, verdict correct, bimodal map proves red AND green render). GATE LESSON banked after the operator caught a CAM REF leftover my grep-gate missed: UI gates must EXECUTE the UI, not grep it - headless harness is now the standard. Ship delta: rig_kiosk_server 447b5b64 (supersedes 8b07c7d6), rig_kiosk.html afbbd32d (new to repo); Phase 0 deploy = five files.
- 2026-09-03 mid-morning (Master 20.13.18 - 8AC): FIRST HOT RUN OF KIOSK V2 + THE DIVERGENCE AUTOPSY + CHAMPION LOST-AND-FOUND. Chat died mid-test; new session cold-read the Master and picked up the ladder. Pre-test debug (from the dead chat, banked here): server shrugged off two SIGTERMs -> restart rule = kill -9 by PID, verify port freed, relaunch (PID 5068 landed; gauge 902.3 GB = the SSD in decimal GB, same disk as df's 841 GiB); no signal handlers in the source = small open mystery, future rev adds clean shutdown. THE LADDER PROVEN HOT: honest cold face -> START RIG greened LiDAR 12 Hz + IMU 250 Hz, L2 temp ??->40 C (FIRST HOT PROOF of the 8N driver mod) -> lean CAPTURE ran -> deliberate stop in banked order -> AUTO MESH CHECK FIRED UNBIDDEN on the stop falling edge (2,814 v / 5,768 f in 0.6 s, verdict in the panel) = v2's headline feature works live, first try. CAMERA 0 Hz all night — the Master-20 gscam-orphan hang RECURRED despite the PATTERNS patch; parked by design (LiDAR-only night), TOP OF NEXT HARDWARE QUEUE. fusioncap_094752 -> SSD, metadata.yaml intact (the anti-8V clean close), 135M / ~30 s (stopped early amid two-monitor confusion; streams full-rate the whole way: cloud 369 / imu 7717 / img 0). check_capture: GOOD TAKE — true for Tier-1 stream health and a LIVE DEMONSTRATION of the 8L Tier-2 gap: the geometry was garbage. inspect_pcd: robust extent 179x915x367 m; robust ~ full = point MASS spread, not a flier-dragged box (this discriminator now banked in §6). VIEWS VERDICT: ISLANDS, NOT A RIBBON — gap-injection hypothesis banked as the candidate answer to §6's open "why does the cluster get injected" (details in the §6 update; testable on the next walk bag). INIT-DISCIPLINE GAP flagged: the kiosk CAPTURE button hides the dead-still init the stone-tablet method made explicit — the static REDO must restore it. CHAMPION OVERWRITE: auto-check clobbered /tmp/rig_meshcheck/mesh.meshbin at 09:53; /tmp volatility means the staged copy was doomed on next reboot anyway; champion FOUND intact in ~/miner/ (staging was a copy), RESTORED 10:28 and verified rendering (246,804 v); diverged outputs preserved in ~/miner/session_094752/; server-rev requirements written into §6. Debris ID'd: "Rig Point Cloud Files" folder = Firefox saved-webpage artifact (three_min = three.min.js, not a capture). Kiosk map's "specks" were HONEST (drew the diverged truth, auto-scaled). NEXT SESSION: (1) DISPLAY SWAP — operator's call: /cloud (Rig Point Cloud, the real view; matches the DJI/Leica field-display standard of 8H) opens on the WAVESHARE via the RigKiosk launcher URL edit; Rig Field Control (/kiosk, the buttons) moves to the PC bookmarks (test later); watch tegrastats once during a hot run for WebGL-during-capture load (8M). (2) STAGE-1 STATIC REDO with explicit dead-still init, expecting a CLEAN small map this time. (3) Camera 0 Hz hunt (orphan check FIRST). (4) Then the four-job walk (8AA plan stands).

- 2026-09-03 afternoon (Master 20.13.19 - 8AD): THE CONFLICT AUDIT + KIOSK V3 — the day's fog burns off and the REAL GOAL lands. MORNING TAIL: launcher edited to /cloud, then the DECOY ZOO struck live — the icon's true script is ~/rig_kiosk_launch.sh (the Desktop copy is a stale draft), and a misclick opened Desktop's rig_kiosk(4).html as file:// = eternal mock (CAPTURING/02:14 are the HTML's SHIPPED DEFAULTS; the uptime clock is decorative, never JS-updated — it read 02:14 in every screenshot all day). Swap then PROVEN from the icon. 'SCAN LIVE' staleness defect banked (label = connected, not fresh; 13 frames = one decay buffer re-served forever). Champion clobbered a SECOND time (mesh check re-meshed the stale in-memory map into the serve path) -> restore deferred to session end by design; RUN MESH CHECK = ARMED, always. BENCH DOCTRINE: icon fullscreen = Waveshare-only; bench = two tabs, KIOSK FIRST (operator's sequence: buttons+health, then the view). THE AUDIT (operator's call: 'debug and look for conflicts before we go farther' — it paid): (1) rig_stop.sh kill list = RTAB-ERA FOSSIL, no pointlio_mapping/rviz2 -> STOP RIG NEVER STOPPED POINT-LIO since the Aug-20 pivot; rig_start.sh identical hole. (2) point_lio_capture.sh launched rviz:=true — 'lean' carried the §8M CPU thief on every capture since the kiosk existed. (3) PROVEN LIVE: zombie pointlio_mapping PID 25121 at 119% CPU (born 11:06 from an accidental CAPTURE mid-confusion, parent = the rviz:=true launch line) AND an OPEN BAG RECORDER mid-write — INT'd politely FIRST (8V lesson), metadata.yaml landed, fusioncap_110644 425M kept. FIXES ON THE JETSON (backups .bak_prezombiefix): both PATTERNS lists += pointlio_mapping+rviz2; capture rviz:=false; wedged server killed (scan-reader thread had frozen while /data + map ingest lived on — partial-wedge failure mode banked); fresh server PID 26898, clean map, honest JSON. Repo copies of all three scripts now STALE — ship list. THE GOAL, FINALLY STATED PLAIN: 'replace the camera feed in the RIG KIOSK with the image from Rig Point Cloud' — the v2 flat top-down map was never what was wanted; the 3D red/green view belongs IN the control screen. KIOSK V3 BUILT COLD: panel = iframe /cloud?embed=1&mode=MAP (auto-verdict overlay + voxel counter KEPT, 2D draw path dead-coded); cloud.html += embed/mode URL params (hud+help hidden in embed; standalone unchanged). AUDIT ON V3 BEFORE DEPLOY (operator's discipline again): server routes via urlparse = query-safe; pollScan mode-gates BEFORE fetching; __lastData verdict chain intact; iframe isolation kills collision risk; node --check syntax gate on every file (caught a REAL const collision in the mock — the 8AB gate lesson, vindicated); AND ONE REAL CONFLICT: cloud.html loaded three.js from cdnjs = INTERNET DEPENDENCY = dead panel in the field -> src now /three.min.js, r128 (603445 B) fetched to ~/Desktop (D0 done). MOCK approved by operator: 'control, image and data all on one screen!' REPO STALE-NAME TRAP MAPPED with md5s (see §6) — canonical kiosk + server names hold OLD versions; real v2s hid as (4)/(2). Download-counter struck AGAIN live during D1 (v3 arrived as rig_kiosk(4).html — rename-before-upload rule re-earned). BREAK at D1. NEXT SESSION: (1) finish deploy D1 repo upload (canonical names!) -> D2 wget pair to ~/Desktop -> D3 hard refresh; (2) HOT V3 TEST — kiosk tab first, L2 on, START RIG, bars, THEN capture; EXPECTATION SET HONESTLY: the v3 MAP panel wakes ON CAPTURE (feeds /cloud_registered) — pre-capture liveness check still uses /cloud SCAN tab; v3.1 idea = SCAN-until-capturing auto-switch; tegrastats glance during (WebGL load, §8M); (3) STAGE-1 STATIC REDO with dead-still init; (4) champion restore at session END (cp ~/miner/report_121359.meshbin /tmp/rig_meshcheck/mesh.meshbin); (5) camera 0 Hz — check the CABLE first (software orphan theory weakened); (6) revert ~/rig_kiosk_launch.sh to :8080 (v3 kiosk IS the Waveshare face now); (7) Desktop decoy sweep into kiosk_v1_backup/.\n
- 2026-09-03 late pm (Master 20.13.20 - 8AE): V3 DEPLOYED + FIELD CHAIN DOCTRINE. Deploy detour: a break-time Desktop sweep (operator's ALL FILES TO SEPT 3/) had taken every load-bearing file with the debris — server, launch.html, three.min.js, both HTML pages; server PID 26898 also gone (likely reboot). Restored ONLY the load-bearing set from the sweep folder (server verified 447b5b64), leaving all decoys buried — the sweep became the housekeeping win. Repo canonical names STILL served fossils on D2 wget (9ec7b1dc kiosk) — third strike; but the v3 pair was found INSIDE the sweep folder ('rig_kiosk v3.html' 8edf3eab, 'cloud(3).html' 325576a0, both byte-perfect) and installed locally under canonical names. Fresh server PID 5484. COLD BROWSER GATE PASSED: v3 kiosk face renders (RIG OFF, red bars, 3D panel alive). Operator confirmed the face IS the Waveshare readout. FIELD CHAIN DOCTRINE ratified and banked as §4B — notably the operator-added step: RIG LEAVES STAND between START RIG and CAPTURE (tau + 7U grounds). NEXT: hot v3 test (the chain, live), icon revert to :8080, repo canonical-name fix, auto-login check.
- 2026-09-03 night (Master 20.13.21 - 8AF): THE FORENSIC NIGHT + THE FINALE, WITNESSED. Operator called the halt ('absolute mess... back to normal state before any Masters') and ordered the full repo audit + all Masters re-read — the discipline that cracked it. AUDIT: repo WHOLE — 32 Masters (20.1->20.13.20 unbroken, today's three byte-perfect: 8e61b0ab/a1238551/ee8816ae), all core scripts checksummed, v3 kiosk in repo (8edf3eab); stale canonical names mapped (known); check_capture.py Jetson-only. THE HUNT: panel blank through a good walk -> five theories eliminated by direct observation (launch gates only rviz; scan_pub_en true; /cloud_registered correctly named at source; QoS plain-depth matched; same-domain) -> two-ear test: CLI heard the replay at 312 Hz while the server heard ZERO -> minimal 10-line rclpy ear heard 3,199 msgs in 8s = SERVER CODE GUILTY -> kiosk.log tail: **ModuleNotFoundError: map_accumulator** — the break sweep had taken it (+ mesh_check.py + check_capture.py); my restore list missed all three. Lazy import on the capture rising edge killed the reader thread mid-loop; the frozen state-dict impersonated every exotic theory (gotcha banked). RESTORED from repo (6686b43a/427cb266), reader PROVEN by full-bag replay: lidar 12 / imu 251 / odom 389 counted, capturing:True. Stale-DDS litter noted (27 fastrtps segs from kill -9s; cleared by reboot; not causal here). REBOOT: SSD auto-mounted, launcher perma-fixed (RECORD_PATH baked in, gauge honest on icon-launch). V3.1 BUILT+GATED (a2c556d5): panel auto-switches MAP->MESH on verdict, back on capture — 'paint while you walk, mesh when you stop'. THE FINALE RUN: doctrine chain, pre-flight retry passed first try, IMU init 100, walk painted 23,883 voxels server-side, stop -> mesh 0.2s -> ON THE PANEL IN HAND. Operator's two questions answered by demonstration: mesh IS mid-pipeline (witnessed twice today); panel DOES integrate it (witnessed). THE WRINKLE, DISSECTED SAME NIGHT: operator double-tapped CAPTURE -> false stop-edge at 18:54 snapshotted the FIRST LEG (7,610 vox) + flipped the panel to that partial mesh MID-WALK = 'static image / champion again' illusion; identity settled by ARITHMETIC (uploaded champion parsed: RGM1 246,804v/549,948f/12,522,684B exact, md5 99a6bf2b — panel mesh 35,125v ≠ champion; same room, honestly meshed twice). Sandbox state-machine gate: 4 traces (clean stop OK; tonight reproduced; §7U gap self-heals with flicker; NEW stale-done race caught). Full map re-meshed by manual check: 78,458v/163,056f. RENDERS BANKED (3-view, coverage-ramped): 171814 = full coherent room; finale pan COMPLETE = 23,883 vox at 7.0x12.2x4.2m — same footprint as leg one, 3x the fill = the 360 pan is IN THE DATA; only the display ever failed. check_capture graded the evening: 170207 GOOD, 171814 GOOD, 185352 RE-SHOOT-flagged for PCD only (bag clean, regenerable). Recorder orphan from the double-tap closed politely (metadata 19:02), ALL QUIET verified. v4 WORK ORDER (courier rig_kiosk_server_9_3.py = deployed 0ce776db handed to the merge chat): five audited conflicts + counters + process-derived capture state + bake-id/fresh-done switch guard + surfaced refusals + persistent per-session mesh dir + protected champion slot + loud reader death + staleness labels. DATED-COURIER CONVENTION ratified (name_M_D.ext for transit; canonical names stable on-rig; md5 arbiter). NEXT SESSION: (1) v4 merge lands (gate in mock, then deploy); (2) single-tap doctrine capture start-to-finish incl. watching LIVE paint in MAP mode (the one still-unwitnessed view — everything under it now proven); (3) camera 0 Hz cable check; (4) icon-zoo + Desktop sweep WITH the load-bearing list in hand; (5) repo canonical-name heal + ship patched scripts + v3 cloud.html; (6) then back to the REAL pipeline: tau bag, texture bridge on a GOOD walk bag (171814 is a candidate), Unreal.
- 2026-09-04 (Master 20.13.22 - 8AG): THE POSE INVESTIGATION — right method, wrong first move (Master not consulted at the decision point, ~an afternoon lost; operator re-ratified 'every new session read the Master, and consult it at every fork'). THE ARC: pursued camera/odom pose-anchoring on fusioncap_121658. Matcher gave '99% anchored' (2598/2620) — but VALIDATION (operator insisted on checking the picked poses before trusting) caught them near-STATIC: full set spans 12x18x34mm. Read straight from the bag (matcher bypassed): confirmed /aft_mapped_to_init is inert (pos 3cm, ROT 0.6deg across a ~360 walk). Checked the two PROCESSED captures 083911 + 102338 (Master-documented as the good/fused ones): BOTH DEAD too (1-3cm). Sensor-id fingerprint banked to rule out L1/L2 confusion: points-per-scan 5,395 ~ L2's 5,392 (L2 online Aug 6; all working bags are L2). Publisher audit (laserMapping.cpp): Point-LIO publishes /path, /odom_corrected, /cloud_registered, /aft_mapped_to_init; capture script (line 48) records only aft_mapped(+image,imu,cloud) — so /cloud_registered & /path never recorded (coverage not replayable; a future capture-script fix, but NOT the pose cause). RE-REGISTRATION built & PROVEN as a capability: run_point_lio.sh with no driver + bag replay of cloud+imu regenerates poses offline (3-terminal choreography, topic_silence_check_9_4.sh gates the clean stage, --topics excludes aft_mapped so fresh poses don't collide). Ran twice: 121658 fast-replay -> 0.63m (collapsed, start->froze); 102338 --rate 1.0 real-rate -> 0.16m. Real-rate came out SMALLER, so the collapse is NOT a firehose/replay artifact — it's genuinely absent translation. LIVE TIP TEST 143849 (on the stand, deliberate ~10deg forward-down tip): 0.25m spread, start≈end -> an IN-PLACE tip is a pivot, not translation (rotates head, doesn't displace it); 64M PCD = more angles not positions. THE REFRAME (which line 151 already banked Aug 24): the poses were never buggy — bench pans/pivots/tips don't translate, so poses are correctly ~0. Odom needs LINEAR DISPLACEMENT; the proven 8in-tip worked by translating the head, not by its angle. OPERATOR'S KEY FACT: essentially ONE real untethered translating walk has ever happened (dubious) — so pose-based camera/odom SYNTHESIS has never been testable; it is blocked on a TRANSLATING capture, not code. Rig stand-committed 'for now' -> translation BENCHED for the session. CAMERA thread: 'did we silence it?' -> NO (checked Master): that memory is lean-mode cutting the FUSION nodes; camera absences are the recurring GSCAM HANG. Diagnosed live (gscam up, /dev/video0 present, lsusb 0c45:0578 present, topic silent = wedge), CLEARED via kill-9-gscam + exact-config relaunch -> 25.9 Hz. TOOLS BANKED (proven-then-saved, per rule): topic_silence_check_9_4.sh (7f265747), camera_unwedge_9_4.sh (7c2bdfd3), rig_kiosk_bench_9_4.html (df117808, serve via localhost:8080 not file://). SUBSTANCE GAINED despite the lost time: re-registration capability, the pose-technique reframe confirmed on live data, sensor-id fingerprint, two tools, camera hang diagnosed+cleared+recipe. NEXT (unchanged from the Master's own Aug plan): TAKE THE RIG OFF THE STAND for one deliberate translating capture (carry it / out-and-back). That single capture unblocks poses -> texture -> the whole synthesis. There is nothing left to build first. *** NEXT SESSION OPENER (operator-set 2026-09-04): (A) REDO THE TIP CAPTURE WITH THE CAMERA ON — first run gscam via Start Rig, and if silent run camera_unwedge_9_4.sh to get it live (~26Hz), verify all FOUR topics at the wire incl /image_raw, THEN tip-capture. Purpose = camera+geometry integration during motion (a fusion rung); EXPECT poses still near-static if the tip is in-place (143849 lesson: in-place tip = pivot, not translation) — read it as an integration check, not a translation win, unless the tip genuinely DISPLACES the head. (B) THEN MOUNT THE JETSON ON THE RIG + SMALL WALK — the rig LEAVES THE STAND, carried through space; THIS is the translating capture that unblocks poses->texture->camera/odom synthesis. Re-run the pose-spread read on that bag: expect METERS of spread for the first time. Pre-flight both with the file-checksum gate + topic_silence_check; single-tap capture, Ctrl-C stop (never double-tap). AND: consult THIS Master at each fork — today's lesson. *** *** STRATEGIC STANCE (operator, 2026-09-04): the CAMERA/ODOM SYNTHESIS, once achieved, likely makes the TIP much MORE important than it looks now. Today the tip was judged by odometry translation (near-static = 'pivot, not translation'). But the synthesis cares about ANGULAR coverage: a tip sweeps the camera through many angles on the SAME locked geometry — ideal for texture (kills specular glare, fills oblique-surface gaps, gives the baker a best-view-per-face choice) in a way a pure walk (new positions) does not. Tip = new angles; walk = new positions; the synthesis may want BOTH. We couldn't see the tip's value today because we measured it by pose-translation, the wrong yardstick for texture. CONSEQUENCE FOR THE ARCHIVE: every bag records raw /unilidar/cloud+/imu+/image_raw, and re-registration (proven 8AG) can regenerate poses offline — so PAST CAPTURES ARE RE-PROCESSABLE as methods improve. The archive is a latent asset, not spent film: when synthesis/angular-coverage/tau are better understood, GO BACK to 143849, 121658, the champion source, etc. and extract value not visible at capture time. This project ACCUMULATES capability + data that compound; it is not converging to a finish. 'This isn't over by a long shot and our data may come back to help us.' *** 
- 2026-09-05 (Master 20.13.23 - 8AH): *** THE TRANSLATION GATE BROKEN + THE MEMORY WALL FOUND ***. New session opened per rule: READ THE MASTER first (canonical = MASTER_REFERENCE_20_13_22_(1).md, md5 7244ba2c; the plain _22_.md is a byte-identical download-counter duplicate — repo naming trap live again, delete the dup). Pre-flight cold gate clean (server 0ce776db, modules present, tools located in Downloads then Desktop, chmod +x needed after browser download). CAMERA SAGA: silent at wire; camera_unwedge (Mode-1 tool) FAILED this time ('Could not get gstreamer sample'/'BLOCKING MODE' in the relaunch log) = a NEW, deeper failure mode (device-state stall, not process wedge). Operator's instinct (physical fix) was right: USB UNPLUG/REPLUG re-enumerated the device (Device 007->008) and CLEARED it; v4l2-ctl raw-grab confirmed 614400B frame = device delivering; gscam then streamed 25Hz standalone AND survived a full Start Rig (28.9Hz). Two-mode camera finding + discriminator banked. Viewed live via rqt_image_view. Operator chose panel-free capture (rqt as the aiming reference). THE CAPTURE: fusioncap_180551, all four topics verified at wire (LiDAR 12/IMU 251/CAMERA 28.9), tip-down/tip-up/~1m-walk, single tap, clean Ctrl-C SUCCESS (56M PCD). POSE READ (sandboxed against LIVETEST first): mag 2.65m, spread 1.57x2.08x0.45m — METERS at last. THE GATE IS BROKEN: first archive capture with real translation. Cold anchoring: matcher self-test PASS, 100% (2200/2200) matched, and VALIDATED not-hollow (poses step 0->-1.77m->back through space; the 121658 hollow-99% trap did NOT recur). posed_180551.npz written. PROJECTION/SYNTHESIS proof attempt: baker engine 4 selftests PASS cold; ran pointlio_to_texture on the full bag -> python3 climbed to 2.3GB/100%CPU/141MB-free/swap -> HARD-LOCKED THE JETSON (reset required; all data survived, read-only). => JETSON TEXTURE-BAKE MEMORY WALL found (permanent header fact; resolves §8I): capture+anchoring fit 8GB, full texture bake does NOT -> station task. Findings banked to outputs (JETSON_BAKE_MEMORY_WALL_9_5.md, STARTUP_UNWEDGE_TODO_9_5.md). SUBSTANCE: the weeks-long translation gate broken + first genuine anchoring + camera 2nd-mode fix + the memory wall answered. NEXT: SAFE synthesis proof (pre-downsample 180551's PCD then project ONE view, OR point-projection no-mesh) — the color-lands-on-geometry test, finally on data that can test it; then full bake on the STATION; then the startup-unwedge cold build. *** SESSION-END CHECKPOINT (9-5): 180551 MESHED safely -> coverage-colored .ply, 345,612v/715,316f in 0.8s (via NEW pcd_to_voxelmap_9_4.py [a7829e33, sandboxed] + mesh_check.py = the memory-SAFE path; NO lockup). Mesh is LARGER than the champion (246k/549k) — the walk swept more volume (extent 18x16x3.5m). Saved to ~/Desktop/mesh_180551.ply for MeshLab. OPERATOR NOTE: put outputs on the DESKTOP, not the hidden ~/anchor_test/ (it was un-findable in the file dialog). SAFE-MESH RECIPE proven+banked: PCD -> pcd_to_voxelmap_9_4.py -> mesh_check.py -> coverage .ply. Paused mid-MeshLab-view. NEXT: (a) finish viewing 180551 mesh vs champion; (b) SAFE textured synthesis proof (downsample + few frames, per JETSON_BAKE_MEMORY_WALL recipe); (c) full bake on STATION; (d) startup-unwedge cold build. ***
- 2026-09-05 (Master 20.13.26 - 8AI): HUD BUILD DAY. Projection foundation proven; readable FORM found = MESHED COVERAGE SURFACE (abstract cloud unreadable; 121219 meshed legibly — mantel identifiable, green=push-in, red=thin). Per-frame density re-confirmed as the WRONG unit (all-green); accumulation-over-capture is the needed judgment, unbuilt. THREE captures: 112949 (dwell, after a battery-unplug fiasco), 121219 (near-static then PUSH-IN, the clean one), + yesterday's 180551 walk. CHECKSUMMED all three to disprove operator's reasonable duplication suspicion — three distinct md5s (b45589bc/7899816a/65b2caba), genuinely different captures that merely LOOK alike (same room). HARD LESSON: Claude repeatedly blurred the three together (discussed 180551's noisy overlays as today's data); operator caught it twice by instinct, checksums resolved both — Claude was the one needing correction, operator's verify-first instinct was right each time. RULE: room is visually identical across captures, so only md5/timestamp distinguishes them; verify before interpreting. OPEN: (a) noisy dumped frames vs clean live rqt = frame/colorspace bug in dump path; (b) build readable HUD on trusted 121219; (c) accumulation judgment. Rig clean all day (camera up without unwedge each start). NEXT: run down the frame-colorspace bug, then build the readable HUD (meshed-coverage / grid-on-photo) on 121219.
- 2026-09-06 (Master 20.13.27 - 8AJ): CAMERA CAPTURE ERROR SOLVED. Priority 1 (camera back online, full capture) = DONE. Static forensics ruled out RViz (deployed rviz:=false; repo stale w/ rviz:=true — drift noted). Hot test w/ tegrastats: killed the two fusion nodes, captured 115614 -> camera survived the FULL 66.1s (1809 frames @27.4Hz, 100% matched across the whole span; first full-duration-camera capture ever). tegrastats proved it was NOT CPU/RAM/GPU starvation (full headroom) -> mechanism CONFIRMED as message_filters.ApproximateTimeSynchronizer in the fusion nodes buffering/blocking and BACK-PRESSURING gscam. Verified fusion nodes are live-only (rqt overlay + unrecorded colorized_cloud) -> killing them costs nothing; colorized product is post-capture. DERIVED the HUD design rule: the live camera+LiDAR HUD is the same operation that killed the camera, so it must subscribe best-effort/depth-1 (drop-old, ~15fps) NOT via synchronizer/queue — operator's 15fps viewing instinct == the non-back-pressure fix. Banked CAMERA_FIXED_9_6.md + §4E. DOWNSTREAM: all prior camera results are first-14s-truncated (re-validate on 115614). NEXT: Priority-2 synthesis discussion on 115614 (first full-multimodal capture) + build the combined camera+LiDAR view on the PROVEN projection path (matcher poses + compose_world_to_cam), NOT the buggy same-instant script (hand-rolled quaternion + unverified extrinsic-only projection).
- 2026-09-06 (cont.): Banked the OPEN 15fps-scope decision into §4E (HUD-only vs project-wide) — to weigh next session. Key bound: 15fps is a proven HUD display drop-rate, NOT a camera-fix requirement (115614 records full 27Hz); Arducam has no native 15fps at 1920x1200 (30/60/80 only). Session also: read the whole repo + all prior project chats (targeted extraction, flagged chats included); ratified rig geometry from photos (camera+L2 share the plate ABOVE the isolator = extrinsic is a chassis constant; L2 spin gives a healthy shared vibration, isolator filters the 6-12Hz tremor band — pre-logged tau caveat); restored the kiosk load-bearing set to the Desktop after the Sept-4-6 sweep (server 0ce776db, cloud.html v3 325576a0, +6 files, one canonical-name fossil trap cleared); confirmed 115614 = 1809/1809 matched; corrected a near-miss — almost re-ran the full Poisson bake the Master forbids (Jetson OOM-lock), caught by consulting §8AH at the decision point. REPO DEBT noted: v3 cloud.html + three.min.js + check_capture.py absent from repo; point_lio_capture.sh in repo is v1 (no lock, rviz:=true) — stale vs Jetson.

- 2026-09-06 EVENING (Master 20.13.30 - THE CAPTURE): The session that broke the camera wall AND landed the seed bag. AUDIT/READ first: re-read rig_start_lean.sh off the Jetson (exists + wired to a "Start Rig (Lean)" desktop icon — the lean bringup already built, NOT to be rebuilt; camera ON + fusion nodes OFF is its whole point). THE CAMERA FIGHT (full ladder, see header): camera would not stream; walked tiers 1->2->3, mis-chased a USB3-downshift theory for ~an hour until operator said "check the Masters" -> line 249 says B0578 is USB2, 480Mbps is CORRECT (my error, logged). Root of tonight's stall = MODE 3 nvmap/hardware-decoder ENOMEM; jpegdec software workaround got 13Hz live, then REBOOT restored the hardware decoder to full rate. Post-reboot verified camera cold + direct (20.5Hz) then re-mounted ON THE RIG via USB and re-verified (25.9Hz, rig cabling clean, no device-number shift). Lean start clean; coexistence gate PASSED 27Hz under full load. THE WALK: single-tap point_lio_capture.sh, 360deg pan + walk into a short hallway, ~70s, clean 4-stage Ctrl-C stop -> fusioncap_202126 (bag 12.8GiB + PCD 9.6M/312,999pts, both fresh). COLD VERIFICATION (the payoff): camera 1968 frames flat 28Hz full duration (survived!); PCD extent 8.77x13.42x3.41m (the 13.4m = real hallway translation). FIRST capture with full-camera + translation + multimodal together. NOTE: PCD is not MeshLab-native (PCD unsupported) -> converted to ASCII PLY (points-only) via numpy, opened fine; operator: "a fair version of the room, I'd like more detail" -> the detail = camera texture overlay = tomorrow.
  *** TOMORROW (first thing) — THE CAMERA OVERLAY on 202126, the thing we are finally after. Use the PROVEN suite, do NOT rebuild: ***
    1. EXTRACT FRAMES -> extract_frames.py on the bag (the v3 with the YUV fix: ROS 'yuv422'=UYVY not YUY2 — avoids the blue-green noise).
    2. MATCH POSES -> pointlio_pose_matcher.py on the bag -> produces posed_202126.npz (convention locked, APPLY_EXTRINSIC=False, don't pre-apply the extrinsic).
    3. OVERLAY/PROJECT -> LiDAR onto the frames, fusion_triptych-style ("does it correspond" — LiDAR lands on the real room). This is the LIGHT no-mesh path the Master BLESSES on the Jetson. Do NOT run the full textured-mesh bake on the Jetson (OOM hard-lock, station-only — Mode-3 territory + the §8AH memory wall). Overlay = projection, not bake.
  Camera OK overnight? L2 powered down for heat. Rig: camera mounted via USB, verified 25.9Hz on rig cable. OPEN (unchanged, deprioritized behind the overlay): the 15fps HUD-only-vs-project-wide decision (§4E); Mode-3 recurrence watch; the camera-hz gate to add to rig_start_lean.sh.

- 2026-09-07 (Master 20.13.31 - TAU SIDE-QUEST, cold): Chose to close the open tau thread while the rig was cold, before the overlay recapture. Read the Tau Pan folder (operator: "sounds familiar") — found tau_pan_v3 (2.9GB, 40s, raw sensors, no odom) + tau_curves v1-v4 CSVs + tau_solve.py, none in prior inventory (different name prefix escaped the fusioncap searches). tau_solve.py can't run it (wants odometry, absent) so built tau_csv.py: correlates the dump_curves CSV series (CAM vs IMU/LID), reusing tau_solve's vetted math. SANDBOX-FIRST DISCIPLINE: self-tested on synthetic known lags — caught a SIGN bug in the selftest injection (not the solver), fixed it (+/- on one interp line), re-proved +175/-90/0ms recovered to <5ms, THEN ran real data. Result honest WEAK (see header) — tau not closed, but per-point correction quantified (~57ms) and the "needs dense+jerky+translating capture" requirement nailed down. The offline solver had 0 chance of "hanging" — a ROS2-thread paste and a traffic-CCTV dissertation were both looked at and honestly set aside as non-applicable. NET: tau needs a better capture, and that capture == the compliant overlay walk. Everything points to the SAME next step: the recapture. 202126 correction banked (header) — its poses are dead by pan-starvation; camera milestone intact.
  *** NEXT (unchanged, now double-motivated): the COMPLIANT TRANSLATING RECAPTURE. Serves BOTH the overlay (camera+odom overlap on a translating walk) AND a future tau solve (dense + jerky + translating). Walk doctrine: hold still for IMU init, then STEP OFF IMMEDIATELY — translate from second one; get 360 coverage by panning WHILE walking / orbiting a loop, never rotation-alone; vary pace (dwell/rush); 60-90s; clean single Ctrl-C. Then cold reads: bag info -> odom histogram (want poses in EVERY window this time) -> matcher (want real matches) -> extract_frames -> overlay -> then the 3D bake (downsampled, memory-safe, the streaming design). Tools staged + proven: tau_csv.py (tau), the pcd_to_voxelmap->mesh_check coverage chain (proven on 202126), extract_frames v3, pointlio_pose_matcher.

- 2026-09-07 LATER (Master 20.13.32 - THE READING FAILURE, banked at operator's direction): Operator directed that this be recorded plainly: the assistant did not read offered material, and did not look at the Aug-24 fusion overlay despite it being handed over repeatedly across the session. Discussed fusion_overlay_f0.png / fusion_triptych_f0.png without opening the code that makes them; built the 180551 overlay on hud_projection_test (crude, 1px speckle, Image 1) when the mature Aug-24 same-viewpoint fusion (Image 3, structured TURBO triptych) was the right tool and was RETRIEVABLE from the LIDAR STAGE 7 chat the whole time. Recovered the Aug-24 script verbatim via conversation_search only AFTER the operator pushed. RATIFIED: fusion path = colorized_fusion_node_v3.py + the Aug-24 same-viewpoint fusion script; hud_projection_test is proof-of-concept only. And the operator's standing verdict, banked: 2D pictures are NOT progress after three weeks — the deliverable is a 3D orbitable textured render; nothing else counts. Today re-proved: 180551 overlay ran clean (50.8% land, matches banked) so the CHAIN works on translating data — but that is still a 2D picture. THE bag for the 3D render is 180551 (only capture with translation+camera+overlapping poses; 202126 poses dead by pan-starvation, 1.15s). Cloud already converted to .npy (1,812,190 pts) this session.
  *** NEXT (the only thing that counts): downsampled MEMORY-SAFE TEXTURED BAKE on 180551 -> colored .ply -> orbit in MeshLab. pointlio_to_texture with both levers cut (downsample cloud + subset ~10-15 frames across the walk), watch free -m, abort <500MB. Inputs on disk: fusioncap_180551_scans.npy, frames_180551 (2200), posed_180551.npz. Full-density rich bake = station (unresolved: Vagon flopped, cloud-GPU still the plan).

- 2026-09-07 (Master 20.13.33 - ENDPOINT OPEN + BAKE READY): Operator corrected a standing error: set.a.light is a TEMPLATE not the endpoint; the endpoint is OPEN and UNCHOOSABLE until a real textured image exists to test candidates against. Banked. This reframes the bake as THE prerequisite to the whole endpoint question, and argues for neutral-.ply output (no tool pre-commit). Then built + sandbox-audited the 3D textured bake (bake_180551.py): recovered Aug-25 chain, retargeted, memory levers cut; audit proved 1.17GB peak (no OOM), fixed 2 real bugs + a reinvented scorer. The bake is the move: it makes the first camera-textured 3D artifact from the one translating+camera+posed bag (180551). Gate before running: confirm Open3D exists on the Jetson (may not — mesh_check avoided it for the missing-aarch64-wheel reason). Tools this session: fusion_180551.py (Aug-24 overlay, retargeted — showed the full-map-cloud smears without occlusion, a real finding), bake_180551.py (audited, ready), pcd_to_npy.py, tau_csv.py.
  *** NEXT: Jetson open3d check -> bake -> first 3D textured render in MeshLab, judged by eye. If open3d absent, fork (install vs marching-cubes rewrite). Then: does the render give us an IMAGE good enough to finally start the endpoint/tool conversation.

- 2026-09-07 (Master 20.13.34 - DEEP CHAT SWEEP): Operator directed a deep sweep of ALL past chats (back to the May L1 origins). Method-honest: searched content across the corpus; the load-bearing findings are in the header. Biggest correction: today's bake seen in true context — the crisp textured room (pointlio_bridge_realproof.png) is an Aug-21 2D render from the PROVEN Poisson bridge on a 161k single-vantage cloud; today's blocky .ply came from an over-large walk-map cloud + marching-cubes fallback after Poisson segfaulted. Also recovered: mesh=settled-endpoint (not "open" as Master 33 wrongly said), the standing GS-maturity re-check, the C3 extrinsic-hardcode landmine, the skipped Jetson-Poisson gate, the extrinsic-translation soft-spot (visually-verified-only, ~1-4cm), the LIV-Eye ~$700 prior-art kit, and portable-router field monitoring. New convention: date-stamp entries.
  *** NEXT (the proven path, operator-approved): (1) verify bake_180551.py extrinsic == extrinsic_20260816.yaml (C3); (2) downsample 180551 cloud HARDER toward ~161k pts (Image-2 size where Poisson worked); (3) run the PROVEN bridge (per_shot_texture + pointlio_to_texture, Poisson) NOT the marching-cubes reconstruction; (4) if Poisson still segfaults at 161k, the crash is cloud-characteristic not size -> then marching-cubes IS the Jetson floor and crisp Poisson waits for the station. Also queued: web-research the GS-maturity re-check; investigate LIV-Eye as a shortcut.

- 2026-09-07 (Master 20.13.35 - JETSON CEILING + CLOUD PLAN): Long session. Established, by exhaustive proof, that the Jetson CANNOT make the deliverable mesh: Open3D 0.18 Poisson segfaults or shreds on walk clouds (tried full/cleaned/downsampled), can't upgrade past 0.18 (no aarch64 wheel), and marching-cubes (GOOD MESH 9 7.ply, 2cm, 2.2M verts, 76% textured, intact) is the Jetson's honest ceiling - operator: "miles away from a deliverable." Deep chat-sweep earlier established the crisp Aug-21 result was single-vantage + 2D render, not a walk mesh - reconciling why it worked. Landed the cloud-station plan: PC is a thin client, cloud service does the work, split into cheap headless mesh-bake (no GPU, do first, ~free) and pricier Unreal relight (GPU+streaming, after a mesh exists). Researched the Vagon-Unreal failure: almost certainly DX12-on-virtualized-GPU; fix is -dx11 (Vagon's own doc). Tools built + sandbox-audited this session: bake_180551.py (marching cubes, the Jetson deliverable-floor), bake_proven_180551.py & bake_clean_180551.py (Poisson attempts, both crash on Jetson 0.18 - keep as cloud-station scripts where Open3D works).
  *** NEXT SESSION (2026-09-08+): (A) CLOUD MESH BAKE FIRST - the actual blocker. Spin up a cheap headless box (Colab free, or ~$0.30/hr VM) with Python+Open3D>=0.19, move 180551 data (npy cloud 58MB + frames_180551 2200 + posed_180551.npz) via scp/repo, run the PROVEN Poisson bridge at FULL density + all frames -> the first crisp orbitable deliverable mesh. bake_proven_180551.py already written (crashes on Jetson 0.18, will WORK on cloud 0.19+). (B) VAGON UNREAL: retry free trial with -dx11 per the 5-step plan above. (C) these are parallel - (A) unblocks having a mesh; (B) unblocks relighting it. Do (A) first.

- 2026-09-07 (Master 20.13.36 - THE CAPTURE-ASSURANCE HUD): Session end. Reframed the whole project around one operator insight: the capture process works end to end; what's missing is KNOWING WHAT YOU HAVE on-site so you can move on. The HUD is the pipeline quality gate - the station/Unreal half is capped by capture completeness, and the HUD is the only place a gap is still fixable. Defined the HUD's bounded mandate (geometry coverage [exists] + camera coverage [new, predicts texture completeness] + odom integrity [built this session]) and the re-anchor-and-continue goal (hard, unproven, uses the coverage map for both operator-guidance AND stitch-overlap). Ratified priority: capture-assurance HUD before more station effort (gate before factory). Built + sandbox-logic-tested the odom-warning server patch (green/amber/red, catches the 202126 death case); deploy + HUD-render + smoke-test is a next-session rig task. Also today: proved the Jetson mesh ceiling (GOOD MESH 9 7, marching cubes, "miles from deliverable"), wrote GOOD_MESH_9_7_PROCESSING_RECIPE.md, scoped the cloud station (cheap headless mesh-bake first, then GPU Unreal via Vagon -dx11), researched the Vagon fix.
  *** NEXT SESSION (2026-09-08+), in priority order: (1) CAPTURE-ASSURANCE HUD - deploy the odom-warning patch to the kiosk, add HUD render (amber/red + WALK + dist_m), live smoke-test; then design camera-coverage (the new signal). This is the gate. (2) Re-anchor/continue - read Point-LIO start/stop, design the reset+stitch, its own experiment. (3) Cloud mesh bake - the deliverable-grade Poisson mesh on a cheap headless cloud box (bake_proven_180551.py, works on Open3D>=0.19), on the 180551 data already made. (4) Vagon Unreal via -dx11 (5-step plan in Master 35). (5) The hot experiment - a clean both-halves-alive capture, ideally AFTER the odom warning is live so it's gauged not blind. Files this session: GOOD_MESH_9_7_PROCESSING_RECIPE.md, bake_180551.py (MC, Jetson ceiling), bake_proven_180551.py + bake_clean_180551.py (Poisson, cloud-only), pcd_to_npy.py, tau_csv.py, the odom-warning server patch (in /tmp/server_work.py - re-derive next session).

- 2026-09-08 (Master 20.13.37 - CAPTURE-ASSURANCE HUD, design+build+coexistence): Task 1 of today done as a complete verified unit. Operator resolved the reset design: odom-warning + coverage-map + reset are ONE recovery loop, not two tiers - the HUD shows the gap (frozen coverage) and the already-covered area, so the operator re-sweeps the overlap that makes the capture continuous (HUD-guided stitch, eye is arbiter). Crucial clarification banked: odom-stop loses live tracking not data (bag records raw throughout -> offline re-process/stitch always possible). Built + sandbox-verified: odom strip integrated into the real v3 HTML (6-state logic all-pass), server patched (health/stage/reset route, compiles), and coexistence PROVEN both ways (map_accumulator 5.6%/core + 147MB; separate-process + isolated-thread architecture; 202126 empirically ran capture+kiosk together at 28Hz camera). Mockups made: odom_scale_mockup.html, kiosk_full_build.html, kiosk_recovery_loop.html (the animated loop). NOT yet: reset_odom.sh, live deploy, smoke test (all need the rig = today's hot test).
  *** NEXT (today, Sept 8, continuing): Task 2 - capture-assurance HUD proper: the "captured-enough-to-move-on" display. Signals: geometry coverage (exists, map_accumulator) + CAMERA coverage (NEW - which surfaces frames saw, predicts texture completeness / the 24% gray) + odom integrity (built today). Ideal look = the solid-shaded MESH surface (mesh_look_demo right panel), NOT points. Eye judges "enough", no fake done-green. Then Task 3 hot test (deploy odom strip live + smoke test + a clean both-halves-alive capture, gauged by the new strip), sync the PC. Task 4 cloud mesh bake if time (bake_proven_180551.py on cheap headless Open3D>=0.19). Files today: rig_kiosk_v3_odom.html, server_odom.py, the 3 mockups, GOOD_MESH_9_7_PROCESSING_RECIPE.md (yesterday).

- 2026-09-08 (Master 20.13.38 - COVERAGE HUD ALREADY EXISTS, conflation resolved): After the foundational-doc sweep (PROJECT_MASTER_RECORD charter [fossil-but-accurate, RTAB/Unreal superseded, principles persist: plane->rotation/holes->translation, Kabsch-hides-bias, intrinsics-first, eye-is-arbiter]; CAPTURE_METHOD(2) = canonical written-in-stone capture method, confirms odom-cutoff monitor + bag-record-time-not-header + Poisson-segfaults-on-Jetson; HUD_AND_EYE = the meter spec), traced the feed confusion to ground: the coverage meter is ALREADY BUILT as kiosk MAP mode (code-verified: /map.bin -> covColor(relative log-normalized hit-count) -> live red->green = the doc spec exactly). The "never correct" feed was a CONFLATION of kiosk MAP mode (works) with separate offline overlay PNGs (the speckle, the wrong-question reinvention). Failure images shown = the PNGs, not the kiosk. So: nothing to build for the feed; remaining items are default-to-MAP, the MAP->MESH switch, retire the overlay PNGs, and validate MAP live on a VARIED walk. Naming-trap confirmed live during the sweep (PROJECT_MASTER_RECORD (1) vs (1)(1) [banner], CAPTURE_METHOD (1) vs (2) [newer], map_accumulator dupes identical) — cleanup sweep overdue.
  *** NEXT (today, Sept 8): the hot test is now sharply scoped - go in testing KIOSK MAP MODE (the real coverage HUD) on a deliberately VARIED-dwell walk, with the odom strip deployed, NOT re-chasing the overlay-PNG dead-end. Small kiosk fixes first: default/auto-switch to MAP on capture start; verify the MAP->MESH stop-switch. Then: deploy odom strip + MAP-default, lean start, gate, varied walk (dwell/rush so red/green differentiate), watch MAP mode fill + the odom strip, process on Jetson (GOOD_MESH recipe). Parked: cloud mesh bake (bake_proven on Open3D>=0.19), Vagon -dx11, naming-trap cleanup.

- 2026-09-08 (Master 20.13.39 - ODOM BAND CONFORMED + DEBUG-PASSED): Built the odometry measure into the deployed kiosk by altering the real files in place. Key discipline wins: (1) RigKiosk.desktop/launcher untouched - odom reaches the kiosk by replacing Desktop files the launcher already reads; (2) reconciled against the AUTHORITATIVE face (df117808 BENCH, not the repo v3 I'd first patched - a stale-base trap avoided); (3) honored the touch-only/no-scroll 1024x600 constraint - built full-width odom band, fixed labels-below-line, trimmed to exactly 600px, RENDER-VERIFIED headless. Deploy set (face + server patch + honest marker-only reset stub) passed 7/7 debug+conflict checks incl. the cross-file contract. reset_odom.sh is deliberately a STUB (no fake restart) because the real re-anchor/stitch is unbuilt. NOT yet deployed/cold-verified/hot-tested (needs rig). Daily-cleanup habit clarified (dated folders = history, Desktop = today).
  *** NEXT (Sept 8+): deploy the odom set to the Desktop + cold-verify (curl /data odom fields) + the HOT TEST - lean start, gate, VARIED-dwell walk, watch the odom band transition live (INIT->TRACKING->CAPTURING; would it catch a drop) AND the coverage MAP mode fill red->green, process on Jetson (GOOD_MESH recipe). The hot test is the live-behavior proof the static checks cannot give. Parked: real re-anchor+stitch (uses coverage-map overlap, its own experiment), cloud mesh bake (bake_proven, Open3D>=0.19), Vagon -dx11, naming-trap cleanup.

- 2026-09-08 EVENING (Master 20.13.40 - RED/GREEN ROOT CAUSE: missing map_accumulator.py): The /regdiag instrumentation cracked the "panel never worked" mystery cold-then-hot: server RECEIVED 288 /cloud_registered messages but accumulated 0, erroring ModuleNotFoundError(map_accumulator) on every one - the module was swept off the Desktop by daily cleanup, so reg_cb's import failed and (old code) silently swallowed it. Fixed by copying map_accumulator.py to the Desktop. NOT yet confirmed rendering (L2 killed before the post-fix walk) - but the full chain is now verified: publish OK (288 recvd), format OK (cloud_to_bin offset-based), accumulate now possible (module present). Cold work this session eliminated QoS/scan_publish_en/odom_only/format with certainty (saved wrong fixes). The systemic enemy named: daily-cleanup sweeps 6 required kiosk files off the Desktop; map_accumulator was the 3rd to silently break things (after three.min.js, launch.html).
  *** NEXT SESSION: (1) restore all 6 kiosk files to Desktop (write restore_kiosk.sh). (2) Confirm red/green: lean -> kiosk fresh (wait for LiDAR discovery ~6s) -> load /kiosk, MAP tab -> capture + WALK 30-60s -> /regdiag should show added>0, no err -> MAP fills red/green. The fix is in; this run confirms it. (3) nvmap camera error: reboot clears it if it recurs (LiDAR-path/red-green doesn't need camera). Files: REGDIAG server 6a901f2c, REGDIAG capture b34ca627, REDGREEN_HUD_STEPS_v2.md.

- 2026-09-08 EVENING (Master 20.13.41 - COVERAGE HUD CONFIRMED LIVE): The win. After the map_accumulator.py fix (Master 40), ran the confirmation: restore_kiosk.sh (all 6 files COMPLETE) -> lean -> kiosk fresh -> /kiosk MAP tab -> capture + WALK. Result: the MAP panel FILLED with ~14,485 voxels of live coverage, all gauges green, Odom tracking at 120Hz (walking gave it translation), PCD saved 8.1M. The red/green coverage HUD works, confirmed on screen. This is the first complete usable capture through the working kiosk (fusioncap_170751: BAG+PCD ok). The whole multi-session blocker was a swept module file, caught by the /regdiag instrumentation, fixed by restore-to-Desktop. Prevention: restore_kiosk.sh at session start.
  *** STATUS: the capture-assurance coverage HUD is FUNCTIONAL. Next refinements (not blockers): (a) dwell-vs-rush to see red/green differentiate (varied motion; uniform walk = mostly yellow-mid-ramp); (b) the odom band + coverage together as the full operator instrument; (c) the nvmap camera-decoder error 12 still recurs (reboot clears; camera-path only, doesn't affect coverage). The kiosk deploy discipline: run restore_kiosk.sh (6 files) every session start.

- 2026-09-09 10:01 (Master 20.13.42 - PRE-FLIGHT ARCHITECTURE reasoned, think-deeply): Following the coverage-HUD win, reasoned (did NOT build) the pre-flight/Trouble-Box architecture. Header holds the full design. Essence: the kiosk IS the live diagnostic (rely on it); the missing piece is a COLD pre-flight gate (6-files+md5s) that guarantees it launches whole — the map_accumulator saga was the absence of exactly this. The Trouble Box SHOWS truth + NAMES the fix, never auto-heals (eye-is-arbiter; auto-heal = the silent-except:pass anti-pattern). Checks tiered cold/warm/hot (warm+hot ARE the running kiosk; doctor's unique job is the cold tier). PC-link = cold advisory (warns, never gates). Film-set role model: operator (walk+coverage-glance) vs 1st-AC (PC: pre-flight/start-stop/diagnostics/offload); two humans, two arbiters; SOLO is the floor, two-person the ceiling; reliable pre-flight is what makes solo viable (front-loads verification before the walk). One shared Jetson truth-script, read from PC-SSH (cockpit) or Waveshare-touch (fallback). Dev setup: bench Jetson + PC + Waveshare-via-PC-HDMI as a live preview display = build both faces cold at the bench, no rig round-trips.
  *** NEXT (build, when ready — all COLD, bench-testable): first brick = the shared cold check-script (6 kiosk files present + md5s correct + server launchable -> green/red + names the fix). Then surface it two ways (PC-SSH cockpit; Waveshare touch board). Then the 4-box handoff (cold-green -> tap -> launch kiosk). PC-link advisory last. Reuse restore_kiosk.sh's verify logic, elevated to report-state. Confirmed-working assets to protect: COVERAGE_HUD_HOWTO.md, restore_kiosk.sh (6-file restore), REGDIAG server 6a901f2c, REGDIAG capture b34ca627, map_accumulator.py on Desktop.

- 2026-09-09 (Master 20.13.43 - RIG WATCH EXISTS; pre-flight = extend it): Looked up RIG WATCH in the repo (operator suggestion, "don't reinvent the wheel again"). Read rig_watch.ps1 on the PC: it's the built PC-cockpit — polls /data till 200, then opens /kiosk + /cloud + `ssh rig` (the hands-full auto-launcher, from Master 8V, 2026-09-01). The Master-42 pre-flight design is therefore NOT build-new; it's EXTEND RIG WATCH with the ONE thing it lacks: a wholeness check (it verifies the server ANSWERS, not that it's WHOLE — so it'd open onto broken-empty coverage silently, exactly the 9-08 map_accumulator failure). Fix folds into RIG WATCH's wait-loop: server /health (or /regdiag) endpoint reporting stack-completeness -> RIG WATCH checks it after 200, before opening tabs -> prints truth + names fix if broken, does not open a broken kiosk, does not auto-heal. Eye-is-arbiter preserved.
  *** NEXT (build, cold, at the bench — small): (1) add /health to the server (6 files + md5 + map_accumulator importable -> ready/broken JSON); (2) ~5 lines in rig_watch.ps1: after the 200, curl /health; if broken -> print the named fix + do NOT open tabs; if ready -> open as now. (3) Clear the banked RIG WATCH owed-items when convenient: passwordless SSH key, .204 router reservation. Reuse restore_kiosk.sh verify logic server-side for /health. Confirmed assets protected: COVERAGE_HUD_HOWTO.md, restore_kiosk.sh, REGDIAG server 6a901f2c, REGDIAG capture b34ca627, map_accumulator.py on Desktop, rig_watch.ps1 on the PC.

- 2026-09-09 PM (Master 20.13.44 - PRE-FLIGHT LAUNCHER + PASSWORDLESS COCKPIT): A good, easy, all-additive day after the 9-08 grind. Built and proved, all cold, nothing risked, nothing existing edited: (1) the sweep-proof originals VAULT (~/rig_originals + md5 manifest of 10 known-good files) = the provable restore point + axiomatic source; (2) rig_check.sh = cold wholeness diagnostic (deployed-vs-vault md5, green/red, names the fix, catches the 9-08 silent-missing-file class before launch) - sandbox+real proven; (3) rig_launch.sh = launcher that GATES on the check (RED stops, GREEN proceeds), works on Jetson or from PC - both cases proven; (4) PASSWORDLESS PC COCKPIT - ssh rig ed25519 key installed, PC now runs the pre-flight on the Jetson with output on the PC screen. Protection protocol ratified (clone/archive-first, sweep-proof vault, md5-gate-before-edit, additive-only). RIG WATCH: Big-Fat-No as a frame; salvaged only its SSH link + .204 target.
  *** NEXT (TOMORROW, 2026-09-10): CLOUD / UNREAL ENGINE / PC-SYNC - now founded on a WORKING passwordless PC<->Jetson link (the offload/sync leg starts from a proven connection). This is the fork toward the deliverable half (the mesh->relight pipeline). Carry-in: the cloud mesh-bake still wants a headless Open3D>=0.19 box (bake_proven_180551.py ready); Unreal-cloud via Vagon -dx11 (or iRender) per Master 35. Also queued (capture-side, not blocking): fold rig_check into a Waveshare 4-box board (field face); the true gap-mapping "aim next"; .204 router reservation. Today's new assets in the vault-protected set: rig_check.sh, rig_launch.sh, ~/rig_originals/ + manifest, PC ed25519 key.

- 2026-09-09 LATE (Master 20.13.45 - KIOSK FIXES + PC COCKPIT + 3-BUTTON PRE-FLIGHT): Fixed both kiosk problems (all-additive, protocol-protected, live-confirmed): #1 the RIG KIOSK button opened an old no-IMU face = a DEPLOY GAP (the ODOM face built 9-08 was never copied over rig_kiosk.html) -> deployed it (360a1f1f); #2 the L2 RIG VIEW box vanished = cloud.html deliberately hid .hud in embed mode -> removed the hud-hide (f9cdda3f), controls now stay + buttons work. Built the PC one-click cockpit (rig_cockpit.ps1 + RIG COCKPIT desktop icon: verify-on-Jetson -> start server -> open kiosk on PC; hit + fixed OneDrive-desktop-redirect and two PowerShell parse bugs). Built three big Waveshare touch buttons (RIG KIOSK amber / RIG CHECK green / RIG PRE-FLIGHT blue). Vault now holds the FULL deployed system, md5-manifested. Read-the-real-Jetson-files (not stale repo) was the key discipline that found problem #1's true cause.
  *** NEXT (TOMORROW 2026-09-10 as planned): CLOUD / UNREAL / PC-SYNC — the deliverable half, founded on the working passwordless PC<->Jetson link + one-click cockpit. Carry-in: cloud mesh-bake wants headless Open3D>=0.19 (bake_proven_180551.py ready); Unreal-cloud via Vagon -dx11. Also queued (capture-side, minor): extend rig_check.sh LIVE map to verify the new tools; .204 router reservation; the full hot run to "see it all come together" (RIG CHECK green -> L2 -> lean -> RIG KIOSK IMU/odom face + L2 RIG VIEW -> capture + WALK -> coverage) — not yet done this session (rig stayed mostly down; all fixes deployed + vaulted and individually confirmed).

- 2026-09-09 NIGHT (Master 20.13.46 - IT ALL CAME TOGETHER, LIVE, ON BOTH MACHINES): THE milestone. The full staged flow ran end-to-end clean and the live LiDAR cloud rendered beautifully ON THE PC (and Jetson). RIG CHECK (green) -> power L2 -> RIG PRE-FLIGHT (=lean start, L2 off-ramp) -> RIG KIOSK (fresh server, NO deaf-0Hz) -> IMU/odom face + L2 RIG VIEW + live gauges -> opened /kiosk on the PC -> SCAN mode -> live cloud = "WOOOOW". Every fix from today live at once: fresh-server launcher (48d24185, closes the daemon-deaf bug), PRE-FLIGHT=lean-start repoint, IMU/odom face, L2-RIG-VIEW-visible. Three-step staged flow with off-ramps, buttoned on both machines, all vaulted. Gotcha banked: edit a button script -> must relaunch the button window (holds old code in memory).
  *** NEXT: (a) the red/green COVERAGE on the PC - MAP tab + start a capture + WALK (SCAN already proven; MAP needs Point-LIO running). (b) TOMORROW as planned: cloud / Unreal / PC-sync - the deliverable half, now launched from the working one-click-ish PC flow over passwordless SSH. (c) minor: the premature "LiDAR no data" verify warning (fires if L2 responds slow); extend rig_check.sh LIVE-map to verify the new tools (they show "skipped"); .204 router reservation. Everything deployed + vaulted; nothing owed but the coverage-on-PC victory lap and tomorrow's cloud work.

- 2026-09-10 (Master 20.13.47 - STRATEGY DAY, cloud/Unreal fork): No hot rig work; planning + research + cleanup plan. (1) Memory audit: tight disk = eMMC / at 47%; the 54G Desktop hog is "ALL FILES TO SEPT 3", 48G of it old pre-Sept-3 bags in "Fusioncap scans" - PLAN to RELOCATE (rsync->SSD, verify, remove) not delete, since unique + maybe Unreal keepers. NOT yet executed. (2) Vagon: the twice-failed UE install was UNDER-PROVISIONING (free Planet tier: T4/16GB RAM/75GB disk vs UE5 needing 32GB RAM + 80-120GB install). ONE configured retry (Flame + 175GB disk + preinstalled UE + US region), hard-stop-and-switch if it fails again. (3) Alternatives verified: iRender (RTX4090 $8.20/hr but Vietnam ~150-250ms US latency), Xesktop (batch-only, unsuitable), Shadow (US flat-fee), Paperspace CORE (US interactive). (4) Scale: point clouds scale fine (LAZ + LiDAR plugin); meshes choke on FBX/OBJ -> USD+Nanite; convert on the cloud box not weak local machines; de-light before relighting. (5) Data movement: PC = staging vault not relay; 2TB SSD field holding area (capture->SSD local, upload->cloud later) dissolves the router problem. Two research reports produced (both partly limited by search outages - Vagon tier specs solid, iRender latency inferred-not-measured).
  *** NEXT (the keystone): get ONE capture through Unreal end-to-end - it answers all the open pipeline questions (data path, router, convert-where, beam speed) at once. Path: one configured Vagon Flame attempt (or iRender if it fails) -> convert one mesh (CloudCompare PLY->FBX/OBJ or USD; point cloud PCD->LAZ) -> import -> try relight. Before/independent: execute the 48G relocate (frees the eMMC); verify iRender US latency with a real ping if Vagon strikes out. Then: full house+yard rig sample; then Blackout/Set.A.Light unified previz table (the endpoint - the physical console is built).

- 2026-09-10 NIGHT (Master 20.13.48 - INTO UNREAL): Vagon abandoned after 3 walls (Big-Fat-No). Pivoted to Shadow PC Power (persistent, RTX A4500, the record's own learning-phase pick). UE5.8 INSTALLED + RUNNING on Shadow (cleared the 12GB-install stage that froze Vagon twice), accessed browser-thin-client from the HP PC. Converted the real GOOD MESH (bake_180551_textured_mesh.ply, 89MB, 2.2M verts/4.6M faces, vertex color) -> GOOD_MESH_9_7.glb (112MB, 44181f59): scaled to cm, normals, color preserved, 18x16x3.5m real room. Gotcha: Open3D GLB writer is buggy -> use trimesh. GLB is on the Jetson ~/Downloads. 
  *** TOMORROW STEP 1 (do FIRST): BRIDGE Jetson->PC->Shadow. PC PowerShell: scp rig:~/Downloads/GOOD_MESH_9_7.glb "$env:USERPROFILE\Downloads\" -> drag PC Downloads into Shadow window -> UE Content Drawer Import -> drag into viewport. = first sight of our capture in Unreal. Then: enable Nanite (4.6M tris), check color/scale, try relighting. Expect blocky + baked-daylight (Jetson mesh) - it is a proof-of-path, not final quality; de-lighting + better meshing come after. Strategic fork to hold: off-set-prep vs on-set-live (Blackout console) vs hybrid - the first test informs it. Assets: GOOD_MESH_9_7.glb (44181f59) + .obj backup in chat outputs and on Jetson ~/Downloads.

- 2026-09-11 to 09-13 (Master 20.13.49 - INTO UNREAL + FUSION-MESH QUESTION + SHADOW STOCKED): Imported GOOD_MESH into UE5.8 on Shadow, rendered our captured space (first time). It's the weak per-vertex-color mesh (dotty, "nothing new vs MeshLab") - but VERIFIED the camera+LiDAR fusion is correct (1:1, intermixed, no offset). Core realization: the FUSION is the edge and it's DONE; the gap is turning it into a photoreal/walkable/relightable UV-TEXTURED mesh. PATH A chosen: keep OUR fusion, extend to output a mesh, run on Shadow (which is the processing-station the Jetson never had - pointlio_to_texture hard-locked the 8GB Jetson). UV-bake is plumbing (Unreal/Blender can do it); fusion is ours. DWELL insight banked: L2 accuracy is dwell-dependent (single-shot spec misleads); longer looks avg down noise; unexploited lever; need one finished model to measure it. Fixed UE viewport nav (Win+Alt+M relative-mouse + camera speed 128; use desktop app not browser). SHADOW STOCKED: tools+calib+mesh+cloud+poses (57MB, extracted) + 2200 frames (6.6GB via Drive relay). Free from Jetson now (shut down). Transfer too slow (4 hops) -> BANK TAILSCALE for direct Jetson<->Shadow.
  *** NEXT (all on SHADOW, Jetson can rest): FINISH ONE MODEL = extend the fusion to output a UV-textured relightable mesh (Path A), OR fastest-finish = add a light in Unreal to the already-imported mesh + relight it (proves the day->night mechanism today, rough quality). Then: measure dwell (short-burst vs long-take) against the finished model. Set up Tailscale for transfers. Assets on Shadow: shadow_bundle (per_shot_texture.py, pointlio_to_texture.py, pointlio_pose_matcher.py, bake_180551_textured_mesh.ply, fusioncap_180551_scans.pcd, posed_images.npz, calib_intrinsics_20260813.yaml, extrinsic_20260816.yaml) + frames_180551/ (2200 frames). Calibration: verify per_shot_texture.py hardcoded values match the yamls before running.

- 2026-09-14 (Master 20.13.50 - PIPELINE LOSS FOUND + UNDONE, ~20x texture recovery): Set up the full fusion pipeline on Shadow (conda 'fusion' env, py3.12, open3d/opencv/scipy/trimesh; Miniconda after raw-Python-install hell). Ran OUR fusion -> colored mesh (bake_shadow.py) - the bake that hard-locked the Jetson runs fine on Shadow. Proved operator right: the weak result was PIPELINE LOSS (161k/1.8M pts kept, 210/2200 frames used), not weak capture. Recovered it: extract full odometry from the 16GB bag (extract_odom_and_stamps.py -> odom_and_stamps.npz), re-pose ALL 2200 frames (rematch_all.py -> posed_images_ALL.npz), re-run full-cloud fusion (bake_shadow_full.py) -> 28.8% -> 81.4% textured, same capture. Fixed shredding via correct normals (bake_clean.py: consistent-tangent-plane not toward-origin) -> clean 2:1 mesh, 83.4%; but neighbor-fill overshot into blobs. Tailscale set up (direct Jetson<->Shadow, IP 100.85.175.10).
  *** NEXT: (a) re-run bake_clean WITHOUT neighbor-fill + harder trim = honest clean 83% surface, no blobs. (b) The remaining ~17% is UNSEEN by the camera -> the real next lever is BETTER CAPTURE (dwell/coverage tests): static timed captures, measure dwell curve, more camera sweep. (c) reprocess OTHER captures the same way (all were pipeline-starved). (d) upload today's scripts to the repo. (e) UV-texture-bake + de-light for true Unreal relighting (still pending; the CLEAN mesh is the input). Scripts all in chat outputs + Shadow shadow_bundle. Bag lives at Jetson /mnt/rigdata/fusioncap_180551/.

- 2026-09-15 (Master 20.13.51 - POSE BUG FIXED + PROCESS-FAILURE POSTMORTEM): A full day lost to a debugging spiral, resolved at the end. ROOT CAUSE: our rematch_all.py re-derivation produced DEGENERATE yaw-only poses (lost the 27.6deg pitch) -> camera projected 90deg off -> 2% coverage. THE GOOD POSES existed all along in anchor_test/posed_180551.npz (2200 frames, real pitch, PROVEN 76% in GOOD_MESH_9_7_PROCESSING_RECIPE.md). Proven on Shadow: 82% coverage with the good poses vs 2% with ours. FIX = use posed_180551.npz (bridged to Shadow as posed_180551_GOOD.npz), never our rematch. The ~90px color offset is the KNOWN never-closed extrinsic translation (banked Sept 7), not a bug.
  *** POSTMORTEM (operator-directed): three compounding failures - (1) did NOT read the repo (the proven recipe + poses were there); (2) did NOT read the Masters (the 90px offset, the extrinsic soft-spot, and 180551's validated-good poses were all banked; the Master explicitly warns to read it when surprised, and I built an investigation instead); (3) failed to ISOLATE the pipeline and IGNORED the visual evidence - Unreal/Open3D renders SHOWED the mesh correctly colored where the camera saw it, and my OWN verification checks were buggy (reported 2% while the mesh was visibly 76% colored) yet I trusted the broken numbers over the visual ground truth. Re-ratified: trust the visual artifact over a fresh check; isolate the failing stage before theorizing; when a new check contradicts a known-good result, the CHECK is the suspect.
  *** NEXT (poses now work, 82%): pick one clean path - (A) colorize the cloud with the good poses -> colored cloud to RealityScan (auto-registers, sidesteps the sparse-intensity GCP problem); (B) RealityScan trajectory import of good poses (Registration=Exact) + LiDAR mesh + photo texture; (C) re-bake UV atlas with good poses. Real progress today under the mess: Shadow RealityScan env built, LiDAR->PLY-with-intensity solved (use o3d.t.io to preserve intensity), 2200 frames confirmed unique (RealityScan scrub preview glitches - trust files). capture_check.py built (real future-capture QC, was NOT 180551's problem). Assets: anchor_test/posed_180551.npz (Jetson) = posed_180551_GOOD.npz (Shadow), the trusted poses.
