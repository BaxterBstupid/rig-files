<!-- ============================================================================ -->
<!-- ***** READ THIS FIRST — TOP PRIORITY / THE ACTIVE MARCHING ORDER ***** -->
<!-- Master 20.13.55 (2026-09-16) — THE COLMAP EXPORT is the NEXT BUILD. This is the -->
<!-- re-plumb the conflict-check (20.13.54) proved we need: STOP feeding RealityScan the -->
<!-- colored voxel map; feed it undistorted IMAGES + Point-LIO POSES (COLMAP format) + the -->
<!-- RAW un-voxelized LiDAR cloud; let RS TEXTURE FROM THE IMAGES (F9 -> UV atlas for Unreal). -->
<!-- -->
<!-- HOW TO BUILD THE COLMAP EXPORT (researched best-practices, do NOT hand-roll): -->
<!--  1. REUSE COLMAP's own scripts/python/read_write_model.py as the writer (Camera/Image/ -->
<!--     Point3D namedtuples + write_cameras_text/write_images_text/write_points3D_text + -->
<!--     rotmat2qvec). Nerfstudio vendored this verbatim as colmap_parsing_utils.py. Don't -->
<!--     reinvent the format. -->
<!--  2. THE #1 BUG = the WORLD-TO-CAMERA INVERSION. images.txt stores WORLD->CAMERA; Point-LIO -->
<!--     gives CAMERA-IN-MAP (c2w). Invert: R_wc = R_cw.T ; t_wc = -R_wc @ C (C=camera center -->
<!--     in map). Quaternion is w-FIRST (QW QX QY QZ), Hamilton. scipy as_quat() = xyzw -> -->
<!--     REORDER to wxyz. (SLAM gives camera center C, so translation = -R*C, NOT the raw t.) -->
<!--  3. COMPOSE the camera pose: T_map_cam = T_map_lidar @ inv(T_lidar_cam) using the VETTED -->
<!--     extrinsic (extrinsic_20260816.yaml; be explicit whether it is cam->LiDAR or LiDAR->cam -->
<!--     - a wrong inversion here silently offsets every camera). Our proven compose_world_to_cam -->
<!--     in per_shot_texture.py already does the map->cam math (gave 82% projection); reuse it. -->
<!--  4. cameras.txt = ONE PINHOLE camera (images are UNDISTORTED): "1 PINHOLE W H fx fy cx cy". -->
<!--     Do NOT emit FULL_OPENCV-with-zeros (breaks downstream tools). -->
<!--  5. images.txt = per image: line1 "IMG_ID QW QX QY QZ TX TY TZ CAM_ID NAME", line2 BLANK -->
<!--     (empty 2D-point line ok). NAME must match the image filename exactly. -->
<!--  6. points3D.txt = *** TRIAL AND ERROR (no formal Epic spec; community reports CONFLICT) *** -->
<!--     TEST BOTH: (a) full points3D from the LiDAR cloud (ID X Y Z R G B ERROR TRACK[], dummy -->
<!--     ERROR, empty TRACK) vs (b) EMPTY points3D body + import the raw cloud SEPARATELY as -->
<!--     .las/.ply. One forum thread only loaded after EMPTYING points3D + adding cloud as .las; -->
<!--     one guide rebuilds POINTS2D/TRACK. KEEP WHICHEVER RS ACTUALLY MESHES FROM. Build the -->
<!--     export so this is a TOGGLE. -->
<!--  7. STRIP trailing whitespace + blank end-lines from all 3 .txt files -> the documented RS -->
<!--     import failure ("physical end of the tape has been reached") is caused by that. -->
<!--  8. metric scale: keep Point-LIO metric; do NOT normalize/recenter poses (Nerfstudio-style) -->
<!--     - RS wants raw metric so LiDAR mesh + cameras share scale. -->
<!--  9. VALIDATE BEFORE RS: load the model back in the COLMAP GUI (frustums trace the trajectory, -->
<!--     point the right way) AND project a handful of points3D into an image with K[R_wc|t_wc] -->
<!--     and confirm they land on the right pixels. This ONE test catches the inversion + quat- -->
<!--     order bugs. (Cameras mirrored/180-off = quat-order or c2w/w2c error.) -->
<!-- 10. RS IMPORT: put undistorted images + the 3 .txt in ONE folder; Open -> "Colmap Text -->
<!--     Format" -> select cameras.txt; DO NOT press Align (imported poses = pre-aligned -->
<!--     component, "alignment engine = imported"); go straight to mesh (Preview->Normal->High) -->
<!--     -> Texture (F9) FROM IMAGES -> confirm UV ATLAS (not per-vertex). RS 2.1.1 added -->
<!--     FULL_OPENCV import + fixed loadColmap CLI (loadColmap replaces importColmap). -->
<!-- REFERENCE REPOS to learn from: COLMAP read_write_model.py; Nerfstudio colmap_parsing_utils.py; -->
<!-- naver/kapture (export_colmap); hloc; Colmap-PCD (LiDAR-map+images, closest analog). -->
<!-- CAVEAT: Epic publishes NO formal spec for the minimum COLMAP fields RS needs -> the points3D -->
<!-- question (step 6) is settled ONLY by trying both on OUR data. Full research artifact this session. -->
<!-- ============================================================================ -->
