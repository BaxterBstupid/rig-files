...
SCRIPTS = {
    ...
    "reset_odom": os.path.expanduser("~/reset_odom.sh"),
}
...
_REG_DIAG = {"msgs": 0, "added": 0, "err": "", "fields": ""}  # /cloud_registered intake diagnostics
...
def ros_reader():
    ...
    odom_pose = {"xyz": None, "t_msg": 0.0, "t_moved": 0.0, "dist": 0.0, "origin": None}
    _last_odom = [0.0]
    def make_cb(kk):
        def cb(msg):
            stamps[kk].append(time.time()); ever[kk] = True
            if kk == "odom":
                # THROTTLE: odom can publish ~1000Hz; the band only needs ~10Hz. Doing pose math on
                # every message chokes the spin thread (this regressed the kiosk 2026-09-08). Gate it.
                nowo = time.time()
                if nowo - _last_odom[0] >= 0.1:      # ~10Hz max
                    _last_odom[0] = nowo
                    try:
                        pp = msg.pose.pose.position
                        x, y, z = pp.x, pp.y, pp.z
                        if odom_pose["origin"] is None: odom_pose["origin"] = (x, y, z)
                        px, py, pz = odom_pose["xyz"] if odom_pose["xyz"] else (x, y, z)
                        if (odom_pose["xyz"] is None or
                            ((x-px)**2+(y-py)**2+(z-pz)**2) ** 0.5 > 0.01):
                            odom_pose["t_moved"] = nowo
                        ox, oy, oz = odom_pose["origin"]
                        odom_pose["xyz"] = (x, y, z); odom_pose["t_msg"] = nowo
                        odom_pose["dist"] = ((x-ox)**2+(y-oy)**2+(z-oz)**2) ** 0.5
                    except Exception: pass
    ...
    def reg_cb(msg):
        _REG_DIAG["msgs"] += 1                      # counts EVERY /cloud_registered received
        try:
            if not _REG_DIAG["fields"]:
                _REG_DIAG["fields"] = ",".join(fl.name for fl in msg.fields)  # record the point format once
            flds = [(fl.name, fl.offset, fl.datatype) for fl in msg.fields]
            b, n = cloud_to_bin(bytes(msg.data), msg.point_step, msg.width * msg.height, flds)
            ...
            _REG_DIAG["err"] = repr(e)[:200]        # LOG the error instead of swallowing it
    ...
    while rclpy.ok():
        ...
        # ODOM POSE-AWARE HEALTH (stage scale RESET->INIT->TRACKING->CAPTURING + warn/lost)
        try:
            rig_on = rate("lidar", now) > STREAMS["lidar"]["hz"]*0.3
            if not rig_on:
                stage, od_state, od_msg = 0, "ok", "Idle."
            elif not capturing:
                # rig on, not recording: init (odom not yet live) vs tracking (odom live)
                if odom_pose["t_msg"] and (now - odom_pose["t_msg"]) < 0.5:
                    stage, od_state, od_msg = 2, "ok", "Tracking — ready to capture."
                else:
                    stage, od_state, od_msg = 1, "ok", "Hold still — IMU init."
            else:
                alive = odom_pose["t_msg"] and (now - odom_pose["t_msg"]) < 0.5
                translating = odom_pose["t_moved"] and (now - odom_pose["t_moved"]) < 2.0
                if not alive:      stage, od_state, od_msg = 3, "lost", "Hold still, tap RESET to re-anchor & continue."
                elif not translating: stage, od_state, od_msg = 3, "warn", "NOT MOVING — WALK to keep tracking."
                else:              stage, od_state, od_msg = 3, "ok",  "Recording — odometry healthy."
            d.setdefault("odom", {})
            d["odom"]["stage"] = stage; d["odom"]["state_health"] = od_state
            d["odom"]["state"] = od_state if od_state in ("warn","lost") else d["odom"].get("state","ok")
            d["odom"]["msg"] = od_msg
            d["odom"]["dist_m"] = round(odom_pose["dist"], 2)
        except Exception: pass
        # card
        try:
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if u.path in ("/", "/index.html", "/launch"):
        ...
        elif u.path == "/three.min.js":
            try:
                with open(os.path.join(HERE, "three.min.js"), "rb") as f:
                    body = f.read()
                ...
                self._send(404, b"three.min.js not found next to the server", "text/plain")
        elif u.path == "/data":
            with STATE["lock"]:
            ...
            self._send(200, json.dumps(_REG_DIAG))   # msgs=received, added=accepted, err=last error, fields=point format
