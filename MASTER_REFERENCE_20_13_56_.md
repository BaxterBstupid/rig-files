<!-- ============================================================================ -->
<!-- ***** READ THIS FIRST - TOP PRIORITY / ACTIVE STATE (Master 20.13.56, 2026-09-17) ***** -->
<!-- *** THE XMP BRIDGE WORKS. TWO DAYS OF COLMAP 'END OF TAPE' HELL IS OVER. *** -->
<!-- PROVEN THIS SESSION (3-image gate + LiDAR merge, all on Shadow): -->
<!--  1. XMP exact-priors sidecars LOAD into RealityScan 2.1 - poses read (pin icons), focal read -->
<!--     correct (f:15.9mm = FocalLength35mm=15.914). NO 'end of tape'. -->
<!--  2. Align Images -> 3/3 cams, ONE component, #pts 4352/4052/1835 (COLMAP gave 9/6/14 -> XMP -->
<!--     ~300x better; rich tie-points prove pose ORIENTATION correct, NO axis-flip needed). -->
<!--  3. LiDAR cloud imported + CO-REGISTERED into the SAME 1 COMPONENT as cameras (the research -->
<!--     make-or-break merge step - PASSED). 33 intensity virtual-cams generated. -->
<!-- capture -> pose-match -> XMP -> RealityScan -> aligned+merged into 1 component. THE BRIDGE HOLDS. -->
<!-- -->
<!-- NEXT STEP (start here tomorrow): SCALE 3 -> 422, build the real mesh+texture. -->
<!--  Command (Shadow, fusion env, shadow_bundle dir): -->
<!--    python make_xmp_v2.py --poses posed_163005.npz --extr extrinsic_20260816.yaml -->
<!--        --intr calib_intrinsics_20260813.yaml --images frames_163005_key -->
<!--    -> writes 422 xmp sidecars next to the 422 frames. -->
<!--  Then RealityScan: New project -> WORKFLOW>Folder -> frames_163005_key (422 imgs, xmp auto-load) -->
<!--    -> Align Images -> import LiDAR fusioncap_163005_gray.ply (Registration=Exact, Features=Color, -->
<!--    Local) -> confirm 1 component -> Calculate Model (mesh from LiDAR) -> Texture (from 422 photos) -->
<!--    -> LOOK at the render. -->
<!-- -->
<!-- KEY FILES (all on Shadow, Downloads/shadow_bundle): make_xmp_v2.py (THE tool, 92 lines, SYNTAX OK; -->
<!--  --flip none/flip_yz/flip_xy/flip_xz fallback, default none = correct), posed_163005.npz (2106 -->
<!--  poses 100% ok), frames_163005_key (422 keyframes), extrinsic_20260816.yaml, -->
<!--  calib_intrinsics_20260813.yaml, fusioncap_163005.ply (raw intensity), fusioncap_163005_gray.ply -->
<!--  (1.75M pts, intensity->grayscale RGB so RS accepts it). -->
<!-- -->
<!-- GOTCHAS BANKED: -->
<!--  * RS LiDAR import REJECTS intensity-only ply ('no known color channels') -> needs colors field. -->
<!--    Fix: intensity->grayscale RGB; import Features=Color. -->
<!--  * make_xmp_v2.py imports pyyaml but reads via cv2.FileStorage; Shadow fusion env needed pip -->
<!--    install pyyaml (done). -->
<!--  * RS 'Add Images/Folder' (WORKFLOW>Inputs) != 'RS logo>Open' (opens PROJECT/COLMAP=wrong dialog, -->
<!--    'name not valid'). Import via Inputs>Folder. -->
<!--  * RS dialog may not list a file dir proves exists -> type FULL PATH in File name box. -->
<!--  * Imported XMP poses only show in 3D AFTER Align (empty before). -->
<!--  * TRANSFER LESSON (cost hours): JETSON terminal MANGLES large multi-line pastes. SHADOW (Windows -->
<!--    Anaconda) handles pastes fine. Move scripts via Shadow small-base64-chunks or the repo. NEVER -->
<!--    large-paste to the Jetson. -->
<!-- -->
<!-- DEAD ENDS (do NOT revisit): COLMAP pose-only import -> RS needs a populated observation graph -->
<!--  (points3D+POINTS2D); empty=header-only = the DOCUMENTED 'end of tape'. validate_colmap.py + -->
<!--  fix_images_txt.py encode the WRONG hypothesis (blank-line formatting), would PASS a file RS -->
<!--  rejects - RETIRE them. poses_to_colmap_v2.py = dead. Desktop imgs First_RS_Render_9_16 (thin -->
<!--  2.7% .lsp) + GOOD_MESH_IN_UNREAL (rough per-vertex mesh) are NOT finished renders (that mesh is -->
<!--  the ceiling XMP->RS->UV-texture breaks). No prior success overlooked. -->
<!-- -->
<!-- OPEN TOMORROW (after 422 mesh): (a) grayscale registration strong enough or COLORIZE cloud from -->
<!--  422 photos first (real-RGB, research-recommended)? Low intensity tie-points (10-21/view) hint -->
<!--  grayscale weak - but 422 REAL photos (4000+ pts each) carry texture regardless. (b) tau -->
<!--  (~150-200ms, unmeasured) may ghost texture; less for slow capture; tau_solve if so. (c) does -->
<!--  cam+LiDAR stay 1 component at 422 scale. -->
<!-- ============================================================================ -->
