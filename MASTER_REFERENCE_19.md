...
<!-- Last touched 2026-08-24 (Master 19): THE CLEAN CAPTURE + FIRST SAME-VIEWPOINT FUSION — both PROVEN. fusioncap_083911 captured via the hardened one-click method (334,030 pts, coherent 4.5x5x2.1m room + neighbor house THROUGH window glass at 5-8m, dimensionally true, first try; operator: "as close to the real image as can be imagined"). Camera frames extracted (192 matched, rgb8, 1920x1200 — calib matches exactly), and the cloud projected through the camera exact eye onto real frames: CHECKERBOARD LANDS ON CHECKERBOARD, through-glass red points land INSIDE the window frame, 62.4% of cloud in one view. The calibration holds on live data. ODOM CUTOFF CONFIRMED STILL ALIVE (odom died 16.09s into a 32.78s capture, gaps up to 5.6s) — not self-resolved by the clock fix. Capture method written in stone (CAPTURE_METHOD.md). NEXT: full bake (mesh+192-frame texture) + Unreal export (.xyz-RGB or .las — NOT .ply, corrected per UE5.8 docs). -->
...
>>> NEXT SESSION STARTS HERE: **FULL BAKE + UNREAL EXPORT** (2026-08-24, Master 19).
