<!-- ============================================================================ -->
<!-- ***** READ THIS FIRST - ACTIVE STATE (Master 20.13.57, 2026-09-18) ***** -->
<!-- *** FIRST TEXTURED 3D RENDER EVER FROM THE PIPELINE. End-to-end PROVEN. *** -->
<!-- capture(163005) -> pose-match -> make_xmp_v2 -> RealityScan: 422/422 cams aligned (~2800 -->
<!--  tie-pts each, 1 component 'Component 0') -> Calculate Model Normal -> 83.6K-tri MESH -> -->
<!--  MESH&COLOR>Texture -> TEXTURED render (real photo color projected on geometry: walls, wood, -->
<!--  the calib checkerboard visible). This is the first time photos->geometry has ever worked here. -->
<!-- -->
<!-- HONEST QUALITY READ: geometry is ROUGH (jagged tris, torn surfaces, room shape distorted) -->
<!--  because this was PHOTOGRAMMETRY-ONLY (Path B - no LiDAR). Photos give great TEXTURE but rough -->
<!--  GEOMETRY (indoor, some texture-poor walls). Proves WHY we need the LiDAR: clean geometry. -->
<!-- -->
<!-- WHAT WORKED / BANKED THIS SESSION (scale 3->422): -->
<!--  * Generated all 422 XMP sidecars (python make_xmp_v2.py --poses posed_163005.npz --extr -->
<!--    extrinsic_20260816.yaml --intr calib_intrinsics_20260813.yaml --images frames_163005_key). -->
<!--  * RealityScan: WORKFLOW>Folder -> frames_163005_key (422 imgs, xmp auto-load, all pin+15.9mm) -->
<!--    -> Align -> 422/422 cams, Component 0, ~2800 tie-pts each (huge, healthy). -->
<!--  * MESH via [2]Process>Calculate Model>Normal Quality -> Model 1, 83.6K tris. -->
<!--  * TEXTURE via MESH & COLOR tab > Color&Texture group > Texture button (NOT Colorize=per-vertex; -->
<!--    NOT the WORKFLOW 'Start' button = runs everything). Model 1 selected first. -->
<!-- -->
<!-- THE LiDAR MERGE PROBLEM (still open, = Path A next): imported fusioncap_163005_gray.ply -->
<!--  (Registration=Exact, Features=Color, Local) -> it landed in a SEPARATE 'Laserscan component' -->
<!--  (33/455 cams), did NOT merge with the 422-cam Component 0. Merge Components DUPLICATED the -->
<!--  components instead of fusing (got Component 0, Laserscan, Component 0(1), Component 1). -->
<!--  ROOT CAUSE (research-predicted): grayscale-from-intensity cloud has WEAK tie-points (9-279, -->
<!--  mostly <60) - RS can't cross-link photos to intensity renders. -->
<!-- -->
<!-- PATH A (NEXT - the quality fix): COLORIZE the LiDAR cloud with the REAL 422 photos (project -->
<!--  images onto the 1.75M points via poses+extrinsic -> real RGB cloud) -> import THAT (Features= -->
<!--  Color) -> it shares actual color/features with the photos so it should register+merge -> mesh -->
<!--  from LiDAR geometry (clean) + texture from photos = the good render. If merge still fails: -->
<!--  control points (>=3 shared photo<->cloud points). We have colorize logic from earlier sessions; -->
<!--  reuse per_shot_texture compose_world_to_cam projection (the proven 82% math). -->
<!-- -->
<!-- KEY FILES (Shadow, Downloads/shadow_bundle): make_xmp_v2.py (WORKS, 92 lines), posed_163005.npz, -->
<!--  frames_163005_key (422 imgs + 422 xmp now), extrinsic_20260816.yaml, calib_intrinsics_20260813 -->
<!--  .yaml, fusioncap_163005.ply (raw intensity), fusioncap_163005_gray.ply (grayscale - WEAK for -->
<!--  merge, that's the lesson). Project saved as fusion_163005 / colmap_163005_key. -->
<!-- -->
<!-- GOTCHAS (still true): RS rejects no-color ply ('no known color channels') -> needs colors field. -->
<!--  Add photos via WORKFLOW>Inputs>Folder NOT RS-logo>Open (=COLMAP/project dialog). XMP poses only -->
<!--  show in 3D AFTER Align. make_xmp_v2 needs pyyaml (pip install, done on Shadow). NEVER large- -->
<!--  paste to Jetson terminal (mangles); Shadow Windows paste is fine; move files via repo or -->
<!--  Shadow-base64-chunks or Tailscale scp (Jetson 100.85.175.10, Shadow 100.109.61.120). -->
<!-- DEAD END: COLMAP pose-only import (RS needs populated observation graph = 'end of tape'). -->
<!--  RETIRE validate_colmap.py, fix_images_txt.py, poses_to_colmap_v2.py. -->
<!-- ============================================================================ -->
