<!-- ============================================================================ -->
<!-- ***** READ THIS FIRST - ACTIVE STATE (Master 20.13.64, 2026-09-21) ***** -->
<!-- *** STATUS: automatic merge RULED OUT (all 3 registration modes fail - see below). *** -->
<!-- *** PLAN = programmatic control points. This is a PLAN TO TRY, NOT A PROVEN SUCCESS. *** -->
<!-- *** We have NOT yet merged photos+LiDAR. The script below is UNBUILT and UNTESTED. *** -->
<!-- -->
<!-- ALL THREE LiDAR REGISTRATION MODES NOW TESTED + FAILED (modality mismatch: RGB photos -->
<!--   don't feature-match LiDAR-intensity renders): -->
<!--   * EXACT -> 4 components.  * DRAFT -> 5+ components.  * UNREGISTERED -> duplicates+split. -->
<!--   Also ruled out: Merge georeferenced components (LiDAR not georef), LiDAR Classification, -->
<!--   Ground Plane, Reconstruction Region. Docs (quickstart_3 Components) CONFIRM control points -->
<!--   are the sanctioned fix for multiple components ('artificial tie points between images'). -->
<!-- -->
<!-- >>> CONFIRMED TODAY: the RS control-point IMPORT/EXPORT format (from ALIGNMENT>Export> -->
<!--   Control Points, exported fusion_163005_cp_sample.csv). Format is DEAD SIMPLE CSV, one line -->
<!--   per (control-point x image), comma-separated: -->
<!--     <full_image_path>, <point name>, <x_pixel>, <y_pixel> -->
<!--   Real example lines: -->
<!--     C:\...\frames_163005_key\img_00730.png, point 0, 905.27, 682.03 -->
<!--     C:\...\frames_163005_key\img_00060.png, point 1, 931.02, 382.64 -->
<!--   It is 2D IMAGE-MEASUREMENTS only (NO 3D coords in the file) - RS triangulates the 3D -->
<!--   position itself from multiple 2D observations of the same-named point. A point seen in -->
<!--   3 images = 3 lines w/ same point name. Import via ALIGNMENT>Import>Control Points. -->
<!-- -->
<!-- >>> THE SCRIPT TO BUILD (UNBUILT - next session) - programmatic control points: <<< -->
<!--   GOAL: replace manual clicking by COMPUTING where LiDAR 3D points project into each image -->
<!--   (photos AND LiDAR .lsp views), using poses we already have, and writing the CSV above. -->
<!--   1. Pick N (~20-50) 3D points spread across the LiDAR cloud (distinct spots/corners). -->
<!--   2. For each 3D point, project into every PHOTO that sees it (photo camera pose + extrinsic -->
<!--      + intrinsics = the PROVEN colorize_cloud.py / make_xmp_v2.py projection math) -> (x,y)px. -->
<!--   3. ALSO project the SAME 3D point into the LiDAR .lsp INTENSITY views (the LiDAR virtual -->
<!--      cameras) -> (x,y)px. THIS is what bridges the two components: the same named point -->
<!--      appears in BOTH photo images and LiDAR images. -->
<!--   4. Write CSV lines: <full_path_to_image_OR_lsp>, point_N, x_px, y_px (one per point-image). -->
<!--   5. Import CSV (ALIGNMENT>Import>Control Points) -> F6 -> RS triangulates + merges. -->
<!--   6. VERIFY: SCENE 3D>VIEW>Source dropdown shows ONE component with photos + LiDAR. -->
<!-- -->
<!-- >>> THE ONE OPEN UNKNOWN to resolve before/while building: the LiDAR VIRTUAL-CAMERA POSES. -->
<!--   To project points into the .lsp views (step 3) we need where each .lsp virtual camera sits. -->
<!--   Options: (a) export them from RS (Export>Registration? or the COLMAP cameras/images.txt -->
<!--   already in shadow_bundle may contain them), (b) compute them - they're pose-linked to -->
<!--   Component 0, so derivable from posed_163005 + the From-component generation, (c) find the -->
<!--   .lsp file naming maps to source scan poses. RESOLVE THIS FIRST - it's the crux of step 3. -->
<!--   NOTE: shadow_bundle already has cameras.txt / images.txt / points3D.txt (COLMAP export) - -->
<!--   check if images.txt holds the .lsp virtual-camera poses we need. -->
<!-- -->
<!-- HONEST CAVEAT: it's plausible RS needs the SAME point marked in BOTH a photo AND an .lsp for -->
<!--   each control point (not just photos). If we can only reliably project into photos, the -->
<!--   merge may still not take - that's the risk. THIS IS A TRY. Test on a SMALL set first -->
<!--   (e.g. 6 points) before generating 50. If 6 computed CPs merge it -> scale up. If not -> -->
<!--   diagnose (wrong .lsp poses? format? RS wants 3D coords too?) before investing more. -->
<!-- -->
<!-- CLEAN SAVE-POINTS: fusion_163005_photos_aligned.rsproj = Component 0, 422 photos ALIGNED + -->
<!--   LOCKED, NO LiDAR, NO control points, NO clutter = THE clean base. (fusion_163005_cp_sample -->
<!--   = throwaway w/ 2 test CPs, used only to get the export format - can delete.) -->
<!--   ASSETS on hand: posed_163005*.npz (poses), calib intrinsics+extr YAMLs, fusioncap_163005.ply -->
<!--   (LiDAR pts), frames_163005_key+XMPs, colorize_cloud.py (proven projection math to reuse). -->
<!--   PASS LINE only when photos+LiDAR actually MERGE into ONE component: -->
<!--   'ACTIVE PASS - STAGE 3 - mint Master 20.13.65'. -->
<!-- ============================================================================ -->
