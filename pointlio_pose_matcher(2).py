...
def dump_frames(bag_path, image_topic, out_dir):
    ...
    with AnyReader([Path(bag_path)], default_typestore=_ts) as reader:
        ...
        for con, _, raw in reader.messages(connections=conns):
            ...
            if img is None:
                raise RuntimeError("frame %d failed to decode (%s) — aborting to preserve "
                                   "pose<->image index alignment" % (n, con.msgtype))
...
def main():
    ...
    ap.add_argument("--max-hole", type=float, default=2.0,
                    help="GATE: fail if any in-run odom hole exceeds this (s) — a real blind "
                         "spot. Decoupled from --max-gap so a momentary blip doesn't false-fail "
                         "an otherwise well-covered capture (match%% catches the catastrophic case).")
    ...
    passed, lines = evaluate_coverage(res, image_t, odom_t, args.min_match, args.max_hole)
    print("\n" + "\n".join(lines))
