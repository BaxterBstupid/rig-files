<!-- ============================================================================ -->
<!-- ***** READ THIS FIRST - ACTIVE STATE (Master 20.13.65, 2026-09-21) ***** -->
<!-- *** BREAKTHROUGH (partial): PROGRAMMATIC GCPs ACHIEVED CO-REGISTRATION. The photos are -->
<!-- *** now placed in the LiDAR's coordinate frame via 8 COMPUTED ground control points. -->
<!-- *** First real merge-progress in days. BUT: RS still shows MULTIPLE component LABELS - -->
<!-- *** co-registered (same frame) is proven; single-component FUSE is not yet achieved. -->
<!-- -->
<!-- WHAT WE DID (Option B - the researched winner - and it WORKED for co-registration): -->
<!--   1. Built make_gcp.py (on Shadow, shadow_bundle) - reuses colorize_cloud.py's PROVEN -->
<!--      projection (world->lidar->camera->pixel): Pl=R_ml.T@(P-pos); Pc=R_L2C@Pl+T_L2C; -->
<!--      cv2.projectPoints(Pc,0,0,K,DIST). Sandbox-verified (point straight ahead -> principal -->
<!--      point; offset-right -> x larger; behind -> rejected). -->
<!--   2. Ran make_gcp.py --npoints 8 --minviews 3 --maxviews 8 -> picked 8 well-spread 3D -->
<!--      points (farthest-point sampling), each seen by 8 photos. Wrote: -->
<!--        test_gcp_3d.csv = 'name,X,Y,Z' (8 GCP 3D coords in local frame) -->
<!--        test_gcp_2d.csv = 'full_image_path,name,x_px,y_px' (64 photo measurements) -->
<!--      Validation: gcp_06 projected to ~(921,800) in ALL 8 photos (2px spread) = projection -->
<!--      math is CORRECT (a real 3D pt seen from 8 poses lands at same pixel). -->
<!--   3. In RS (fusion_163005_photos_aligned + LiDAR imported Exact/Intensity/From-component, -->
<!--      Georeferenced=No, clean 2-comp state): -->
<!--      ALIGNMENT>Import>Ground Controls -> test_gcp_3d.csv. Dialog auto-read: File format -->
<!--      'Point X/Lon Y/Lat Z/Alt', separator Comma, Ignore first line No, Coordinate system -->
<!--      'local:1 - Euclidean' (SAME as photos!). OK -> 8 points imported. -->
<!--      ALIGNMENT>Import>Control Points -> test_gcp_2d.csv. Dialog auto-read: File format -->
<!--      'Image Point X Y', Comma, Ignore first line No. OK -> 64 measurements imported. -->
<!--   4. Align Images (F6). -->
<!-- -->
<!-- >>> RESULT (verified in RS): <<< -->
<!--   * Each GCP: Type=Ground control, Weight 10, Coordinate system local:1 Euclidean, bound to -->
<!--     8 IMAGES each, error e:0.00m. -->
<!--   * DECISIVE CHECK: for gcp_06, the 'Actual position' (RS-calculated) == 'Coordinates' (given, -->
<!--     -5.313296, 2.676810, -1.446600). GIVEN == ACTUAL -> the alignment HONORED our GCPs; the -->
<!--     0.00m error means CORRECT, not ignored. Photos aligned exactly to our computed 3D points. -->
<!--   * SO: photos are CO-REGISTERED into the LiDAR frame. The programmatic-GCP approach WORKS. -->
<!--   * CAVEAT: RS still LABELS multiple components (Component 0, Laserscan, Component 0(1), -->
<!--     Component 1, Component 0(2), Component 1(1)). Co-registered (same frame) YES; fused into -->
<!--     ONE component NO. This is exactly the 'co-register vs fuse' split the research flagged: -->
<!--     photo-only GCPs place photos in the frame but don't fuse w/ LiDAR (no shared measurement -->
<!--     in the LiDAR .lsp views). -->
<!-- -->
<!-- >>> NEXT SESSION - the open question: is CO-REGISTRATION ENOUGH, or do we need the FUSE? <<< -->
<!--   PATH 1 (test if co-reg suffices): try to MESH/TEXTURE from this co-registered state - if -->
<!--     everything's in one frame, meshing may work regardless of component labels. If it does, -->
<!--     we're effectively DONE with Stage 3 (the geometry is bound) -> Stage 4. -->
<!--   PATH 2 (force the fuse): extend make_gcp.py to ALSO project the same GCPs into the LiDAR -->
<!--     .lsp INTENSITY views (needs the .lsp virtual-camera poses - the Option A crux), so each -->
<!--     GCP is measured in BOTH photos AND .lsp -> shared measurement -> forces ONE component. -->
<!--   PATH 3 (scale up): more GCPs (20-30) for robustness once the method is confirmed. -->
<!--   RECOMMENDATION: try PATH 1 first (mesh the co-registered state) - cheapest test of whether -->
<!--   the fuse is even needed. Only chase the .lsp fuse (Path 2) if meshing needs one component. -->
<!--   STAGE 3 PASS LINE (only when photos+LiDAR usable together for meshing, fused OR co-reg): -->
<!--   'ACTIVE PASS - STAGE 3 - mint Master 20.13.66'. -->
<!-- -->
<!-- FILES (Shadow/shadow_bundle): make_gcp.py (GCP generator, PROVEN projection, works), -->
<!--   test_gcp_3d.csv + test_gcp_2d.csv (the 8-point import set, imported OK). RS project has the -->
<!--   GCPs + co-registration in it (unsaved fragmented-label state - the CO-REG is the win, the -->
<!--   duplicate labels are cosmetic). Clean base to rebuild from: fusion_163005_photos_aligned. -->
<!--   make_gcp.py knobs: --npoints --minviews --maxviews --imagedir-abs (RS-visible abs path). -->
<!-- ============================================================================ -->
