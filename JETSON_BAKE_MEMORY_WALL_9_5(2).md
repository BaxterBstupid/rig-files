...
## UPDATE — the TWO stacked memory killers (confirmed by reading run(), line 148-170)
Not just the mesh build. TWO killers stacked:
1. pst.mesh_cloud(xyz) on the full ~1M-pt cloud (line 151) — big mesh in RAM.
2. pst.best_image_per_face(V, Tr, images_poses) (line 165) — loops EVERY face x ALL 2200
   images, scoring visibility. faces(100k+) x 2200 imgs = billions of ops holding image
   arrays => THIS is the primary lock. Scales with BOTH face count AND image count.

## EXACT SAFE-PROOF RECIPE for next session (both levers):
1. Downsample the PCD: open3d voxel_down_sample ~1M -> ~150-250K pts (=> far fewer faces).
2. Subset the npz to ~6-10 frames spread ACROSS the 2.65m path (not all 2200) — enough
   viewpoints to test parallax; shrinks best_image_per_face linearly.
3. THEN run pointlio_to_texture on (downsampled pcd, subset npz, subset frames) --render-idx K.
4. Watch memory (free -m in a 2nd terminal); abort if free drops < ~500MB.
Result = a real color-lands-on-geometry synthesis proof, memory-safe, on the Jetson.
Full-density bake still = STATION only.
