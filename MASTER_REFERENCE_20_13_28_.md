<!-- Last touched 2026-09-06 (Master 20.13.28 - 8AK): *** READ-THE-WHOLE-REPO RECKONING + COURSE CORRECTION *** Operator directed a full, careful read of the repo after realizing recent sessions RE-DERIVED solutions that were ALREADY BUILT AND DOCUMENTED. Confirmed by reading the actual tools+runbook (not just the Master narrative): THE TEXTURE BRIDGE IS COMPLETE AND DOCUMENTED (Aug 21-24), proven cold to ~90% photoreal on real data. Suite: TEXTURE_BRIDGE_RUNBOOK.md (gated capture->texture procedure) + capture_pointlio_texture.sh (PURPOSE-BUILT texture capture: lean/no-fusion-node, COMPRESSED topic, gated, stop-and-go) + pointlio_pose_matcher.py (self-test 5e-16) + per_shot_texture.py (K/DIST + extrinsic R_L2C/T_L2C + compose_world_to_cam VERIFIED 7e-16, APPLY_EXTRINSIC=False, mesh_cloud=trimmed Poisson d9, best_image_per_face) + pointlio_to_texture.py (multi-view baker, --occlusion for walked) + colorized_fusion_node_v3.py (LIVE monitor; loads calib from YAML; NOT for capture) + extract_frames.py v3 (correct rgb8->bgr8 + YUV handling) + camera_info_publisher.py + inspect_pcd.py. *** EVERY 'PROBLEM' RECENTLY CHASED WAS ALREADY DOCUMENTED: *** (1) camera dies ~14s = running the FUSION NODE during capture; the capture script header AND runbook HOT-1 say 'lean, NO fusion node' — today's 'discovery' merely re-derived HOT-1. (2) noisy/dropped frames = using RAW /image_raw; capture script auto-selects /camera/image_raw/COMPRESSED and warns raw overruns the SD. (3) my orange-cast overlay = not using extract_frames.py's correct rgb8->bgr8; I double-swapped. (4) tau = stop-and-go pan (zero angular vel during holds) per runbook C2. (5) quaternion convention = already verified 7e-16, documented. LESSON (ratified): READ THE REPO AS SOURCE OF TRUTH — tools+runbooks, not just the Master narrative — BEFORE building; recent flailing came from operating without reference to the documented pipeline. DO NOT rebuild the bridge; RUN it per the runbook. *** THE DIRECTION (runbook 7K, the answer to 'what is this project'): capture->texture is a SOLVED STEPPING STONE. The rig's differentiator is RELIGHT (day->night, Unreal light placement — 'Polycam can't'). NEXT MILESTONE: delight -> PBR -> relight on a good textured mesh. AIM THERE. *** GAP FOUND: the calibration source-of-truth YAMLs (calib_intrinsics_20260813, extrinsic_20260816) live only on the Jetson ~/Desktop, NOT backed up in the repo — if the Desktop is lost, calibration is lost. Back them up. Today's one real contribution: fusioncap_115614 = first full-camera capture (by finally running lean per HOT-1). All my recent hand-rolled scripts (cam_lidar_view, lidar_cam_sameinstant, hud_projection_test) = inferior reimplementations, DISCARD — use the suite. -->
...
- 2026-09-06 (Master 20.13.28 - 8AK): READ-THE-WHOLE-REPO RECKONING. Operator, frustrated that recent sessions re-derived already-solved things and that "the direction of the project" had become unclear, directed a full careful read of the repo. Doing so (reading the actual TOOLS + RUNBOOK, not just the Master narrative) confirmed: the TEXTURE BRIDGE is COMPLETE + DOCUMENTED since Aug 21-24, proven cold ~90% photoreal. Read in full this session: TEXTURE_BRIDGE_RUNBOOK.md, capture_pointlio_texture.sh, per_shot_texture.py, pointlio_to_texture.py, colorized_fusion_node_v3.py, extract_frames.py, camera_info_publisher.py, inspect_pcd.py, RIG_pointcloud_layout.json. FINDINGS: every recent 'problem' was pre-documented — camera-dies=run-lean-no-fusion-node (runbook HOT-1 + capture-script header); frame noise/drops=use COMPRESSED not raw /image_raw (capture script step 2); my orange overlay=didn't use extract_frames.py's correct encoding handling; tau=stop-and-go (runbook C2); quaternion convention=verified 7e-16 already. So today's camera 'fix' was really just executing the runbook's documented HOT-1 the hard way. CLAUDE'S ERROR, owned: treated the Master (narrative) as the whole record and ignored the tools/runbooks that ARE the solved pipeline; kept hand-rolling inferior versions (cam_lidar_view etc — discard). LESSON RATIFIED: read the repo as source of truth before building; run the bridge, don't rebuild it. DIRECTION CLARIFIED (runbook 7K): capture->texture = solved stepping stone; the rig's real differentiator + next milestone = RELIGHT (delight->PBR->day/night in Unreal; the thing Polycam can't do). GAP: calibration YAMLs not backed up in repo (Jetson-only) -> back them up. Real asset gained today: fusioncap_115614 (first full-camera capture). NEXT: run the PROVEN bridge (capture_pointlio_texture.sh w/ compressed+lean+stop-and-go -> matcher -> pointlio_to_texture) to get a clean textured mesh, THEN aim at relight.
