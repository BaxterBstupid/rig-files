> # ⛔ SUPERSEDED / ARCHIVED — DO NOT USE AS CURRENT STATE
>
> **Status:** FOSSIL of the calibration phase. Superseded **2026-08-16**; archived
> **2026-09-03** per the DOCUMENT LINEAGE ruling in the `MASTER_REFERENCE_20_x` line
> (§0-PRE). Authoritative current state lives there — latest
> `MASTER_REFERENCE_20_13_14_.md`. **Read that, not this.** Do not merge this doc's
> "current status" (§10) or "the plan" (§11) into current work.
>
> **What this doc gets wrong — because it predates the fix, not because it was sloppy:**
> it says the camera intrinsics are "in doubt" and that the extrinsic and RTAB-Map work
> are unfinished/future. True when written; no longer true:
>
> - **Intrinsics — SOLVED & vetted numerically, 2026-08-13 (session 6).**
>   `calib_intrinsics_20260813.yaml`: fx 848.76 / fy 849.23 (**fy/fx = 1.0006**, isotropic —
>   exactly as §8 of this doc predicted), RMS 0.297 px, 21-frame big-board set with edge
>   coverage, triple-vet PASS. §8's own prescription ("proper multi-pose
>   `cv2.calibrateCamera` on the big board; do NOT adopt 891 by hand") was **carried out** —
>   that YAML is the result.
> - **Extrinsic — SOLVED, 2026-08-16 (session 8).** `extrinsic_20260816.yaml`: plane-NORMAL
>   (rotation) + CENTROID (translation) — the exact decomposition §7/§11 prescribed,
>   superseding the Kabsch-4-point `extrinsic_solver.py`. Vetted **visually** (colour lands
>   on geometry); the one open extrinsic item is turning that verdict into a **number** via
>   the §8X consistency scorer (run on the archive pans).
> - **RTAB-Map — HAS RUN.** First LiDAR map 2026-08-18 (`milestone_map_20260818.db`). Project
>   has since pivoted to **Point-LIO** (2026-08-20) and moved downstream to texturing /
>   field-readiness. This doc's "no RTAB-Map runs, step 5 future" is stale.
>
> **Why it is KEPT, not deleted:** its diagnosis was *correct* and drove the fix — §8's
> "the anisotropy is spurious; the truth is isotropic" call was right and led directly to the
> 08-13 recalibration. Preserved as accurate HISTORY of the calibration phase.
>
> **The one item here that is still genuinely open:** temporal calibration (tau, ~150–200 ms,
> **colour-only**, sign nailed) — consistent with every later doc. Its SD-dropout blocker is
> now cleared by the 1 TB SSD (543 MB/s).
>
> ---
> *Original document preserved below, unchanged.*
