<!-- ============================================================================ -->
<!-- ***** READ THIS FIRST - ACTIVE STATE (Master 20.13.63, 2026-09-21) ***** -->
<!-- *** BIG FINDING TODAY: the AUTOMATIC photo+LiDAR merge in RealityScan CANNOT WORK on -->
<!-- *** our data. Root cause: RGB photos vs LiDAR-INTENSITY renders do NOT share matchable -->
<!-- *** features (modality mismatch). No registration setting fixes this. TESTED + RULED OUT: -->
<!--   1. 'Merge georeferenced components' (Alignment Settings>Advanced) -> FAILED. Only the -->
<!--      PHOTOS are georeferenced (each PNG has a geo tag from its XMP absolute pose); the -->
<!--      LiDAR .lsp views are NOT georeferenced. Setting needs BOTH georeferenced in the SAME -->
<!--      frame. A raw SLAM .ply canNOT be correctly georeferenced into the photos' local:1 -->
<!--      frame without first solving the correspondence (the very thing we're trying to do). -->
<!--   2. Registration=EXACT + F6 -> split into 4 components (Comp0, Laserscan, Comp0(1), Comp1). -->
<!--      Docs (importlaser_2.htm): Exact = poses LOCKED/unchanged, LiDAR defines coord system; -->
<!--      'for fully-registered inputs you must provide CONSISTENT data' (CPs only fine-tune). -->
<!--      Both photos(locked) AND LiDAR(Exact) fully fixed -> nothing can move -> won't reconcile. -->
<!--   3. Registration=DRAFT + F6 -> split into 5+ components (Comp0, Comp0(1), Comp1, Comp0(2), -->
<!--      Comp1(1)...) - WORSE. Docs: Draft = poses are movable ESTIMATES, fine-tuned; CPs affect -->
<!--      only unregistered/draft inputs. Draft let poses move but there were NO matching features -->
<!--      to anchor to -> scattered into fragments. (Also: Draft import showed no .lsp / odd 'two -->
<!--      sets of img files', 1266 cams = 422 photos + 844 views; first F6 came up blank.) -->
<!--   4. LiDAR Classification - WRONG TOOL (it filters ASPRS classes for MESHING, needs LAS/LAZ; -->
<!--      our .ply has none). Ground Plane / Reconstruction Region - also not merge tools. -->
<!-- -->
<!-- >>> CONCLUSION: automatic/feature-based merge is a DEAD END for photo + mobile-SLAM LiDAR. -->
<!--   The correspondence must be given EXPLICITLY. Two remaining paths: -->
<!--   A. MANUAL control points (tedious clicking; mechanics now known - select-then-click, -->
<!--      interleave photo/lidar, hover-X to delete). -->
<!--   B. *** PROGRAMMATIC control points (THE PATH) *** - we HAVE the Point-LIO camera poses + -->
<!--      camera-to-LiDAR extrinsic, so we can COMPUTE where LiDAR 3D points project into each -->
<!--      photo (their 2D pixel coords) BY MATH, generate control-point measurements, and IMPORT -->
<!--      them (ALIGNMENT tab has Import > Control Points + Ground Controls). Deterministic merge, -->
<!--      NO manual clicking, uses data we already have. This plays to our scripting strength. <<< -->
<!-- -->
<!-- >>> NEXT SESSION: BUILD THE PROGRAMMATIC-CONTROL-POINTS SCRIPT. -->
<!--   Inputs we have on Shadow/Jetson: posed_163005*.npz (camera poses), calib intrinsics+extr -->
<!--   YAMLs, fusioncap_163005.ply (LiDAR points), frames_163005_key + XMPs. -->
<!--   Steps: (1) pick N LiDAR 3D points spread across the scene (e.g. distinct corners), -->
<!--   (2) for each, project into the photos that see it using pose+K+dist (same math as -->
<!--   colorize_cloud.py / make_xmp_v2.py - PROVEN), giving 2D px per image, (3) write RS's -->
<!--   control-points / ground-control IMPORT format (VERIFY exact CSV/XML columns in RS docs: -->
<!--   Import>Control Points), (4) import into the clean project, (5) F6 -> forces merge. -->
<!--   FIRST verify the exact RS 2.1.1 control-point import file format before scripting. -->
<!--   PASS LINE when merged into ONE component: 'ACTIVE PASS - STAGE 3 - mint Master 20.13.64'. <<< -->
<!-- -->
<!-- CLEAN SAVE-POINT (unchanged): fusion_163005_clean_nocp.rsproj = Component 0 (422 photos -->
<!--   LOCKED) + LiDAR (Exact), ZERO control points, no clutter. Reopen for a clean restart. -->
<!--   (Today's Draft/merge experiments all discarded via Don't Save.) -->
<!-- ============================================================================ -->
