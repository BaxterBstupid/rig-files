#!/usr/bin/env python3
# bake_180551 for SHADOW: your fusion (LiDAR geometry + camera texture) -> COLORED MESH (.ply)
# Adapted paths for Shadow; loads PCD (not npy); everything else = the PROVEN bridge.
import numpy as np, cv2, time, os, sys
import per_shot_texture as pst   # same folder on Shadow (shadow_bundle)
import open3d as o3d

# --- SHADOW PATHS (all in shadow_bundle except frames on Desktop) ---
PCD    = 'fusioncap_180551_scans.pcd'
NPZ    = 'posed_images.npz'
FRAMES = os.path.join(os.path.expanduser('~'), 'Desktop', 'frames_180551')
OUT    = 'fusion_180551_MESH.ply'
TARGET = int(sys.argv[1]) if len(sys.argv)>1 else 9999999  # FULL cloud - no Jetson downsample
STRIDE = int(sys.argv[2]) if len(sys.argv)>2 else 1     # use ALL matched frames (stride 1) for best texture
...
T0=time.time()
print("=== LOAD PCD + DOWNSAMPLE to ~%d pts ==="%TARGET)
pts=np.asarray(o3d.io.read_point_cloud(PCD).points, dtype=np.float64); n0=len(pts)
if n0>TARGET:
    idx=np.random.default_rng(0).choice(n0,TARGET,replace=False); pts=pts[idx]
print(f"  {n0:,} -> {len(pts):,} pts  ({time.time()-T0:.1f}s)")

print("=== PROVEN mesh_cloud (trimmed Poisson) ===")
t=time.time()
mesh=pst.mesh_cloud(pts, trim_quantile=0.05)
...
print(f"=== TEXTURE — per-vertex best-facing color, stride {STRIDE} ===")
t=time.time()
