<!-- ============================================================================ -->
<!-- ***** READ THIS FIRST - ACTIVE STATE (Master 20.13.60, 2026-09-19) ***** -->
<!-- *** RUNNING ACTIVE CONTROL CHECKLIST (walkable_checklist.html) -> WALKABLE 3D. *** -->
<!-- Rigid gated stages, composite-first. Each PASS active-verified on screen + mints a Master. -->
<!-- -->
<!-- >>> STAGE 1 (Setup & Ingest) = PASSED. XMP priors bound perfectly (Absolute pose Locked, -->
<!--   focal 15.914228, local Euclidean metric, 422 frames). <<< -->
<!-- -->
<!-- >>> STAGE 2 (Align Photos) = PASSED (active-verified 2026-09-19). <<< -->
<!-- Ran Align Images (F6) with defaults + locked priors (no settings-panel tuning needed - -->
<!--   the locked XMP priors carried it). RESULT (screenshots verified): -->
<!--   * ONE component (Component 0 - RS numbers from 0; 0 models = no mesh yet, correct). -->
<!--   * 422/422 cams aligned = 100%. -->
<!--   * ~2800-2900 tie-points PER IMAGE (strong, healthy). -->
<!--   * Sparse cloud STRUCTURED: dense camera cluster + curved walk-path trajectory (the 360 -->
<!--     walk). Not a scatter. -->
<!--   * ORIENTATION CORRECT: cones point INWARD; selected-camera RGB axis gizmo sane; and the -->
<!--     2800 tie-points independently prove correct orientation (wrong orientation = ~0 tie-pts). -->
<!--   * The KNOWN RS 2.1.1 XMP-orientation regression did NOT bite us - verified by eye. -->
<!-- SIGNIFICANCE: photogrammetry alignment from the locked-pose photos is clean and complete. -->
<!--   This is the camera component the composite will build on. -->
<!-- -->
<!-- >>> NEXT: STAGE 3 - LIDAR-AS-FRAMEWORK COMPOSITE. The 'BOTH' plan, escalating: <<< -->
<!--   3a. WORKFLOW>LiDAR Scan -> fusioncap_163005.ply. Import: Mobile LiDAR, Registration=Exact, -->
<!--       Features=Color, Use camera poses=From component -> Component 0. Then Align Images (F6). -->
<!--       If it fuses into ONE component -> done (LiDAR leads). -->
<!--   3b. If TWO components -> TEST CONTROL POINTS (finally - the tool we kept skipping). Full -->
<!--       vetted procedure in artifact 'Merging Photogrammetry and LiDAR via Control Points': -->
<!--       F3 Add Control Points; select a CP + NAME it; click-hold-zoom-release to place on 3+ -->
<!--       PHOTOS; keep same CP selected; place same point on 2-3 LiDAR .lsp INTENSITY renders -->
<!--       (intensity > color for placement); point must turn BLUE (assigned) in both. Repeat for -->
<!--       4-6 CPs, NON-COLLINEAR, on checkerboard corners + window/mantel edges. Then F6. -->
<!--       Verify: 1 component with photo + .lsp cams; per-image residuals ~<=3px. -->
<!--   3c. If still won't merge -> FALLBACK: photos-lead; LiDAR for SCALE ONLY via Define Distance -->
<!--       (F4) on a known LiDAR length. Reaches walkable without full merge. -->
<!--   Escalate; STOP at first that works. LiDAR-leads needs full merge; photos-lead needs only -->
<!--   scale. We do BOTH in sequence. PASS LINE: 'ACTIVE PASS - STAGE 3 - mint Master 20.13.61'. -->
<!-- -->
<!-- LiDAR-import nuance (banked): our fusioncap_163005.ply is intensity-only; RS may want a -->
<!--   colors field ('no known color channels' error seen before). If import rejects it, use the -->
<!--   gray/colorized variant OR set Features=Intensity. Watch for this at 3a. -->
<!-- HOUSEKEEPING: Fusion_Bundle.bat desktop icon = one-click (fusion) env @ shadow_bundle. -->
<!-- ============================================================================ -->
