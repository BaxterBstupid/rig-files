        ...
        def cb(msg):
            ...
            if kk == "odom":
                try:
                    ...
                    xyz = _np.array([pp.x, pp.y, pp.z]); nowo = time.time()
                    if odom_pose["origin"] is None: odom_pose["origin"] = xyz.copy()
                    prev = odom_pose["xyz"]
                    if prev is None or _np.linalg.norm(xyz - prev) > 0.01: odom_pose["t_moved"] = nowo
                    odom_pose["xyz"] = xyz; odom_pose["t_msg"] = nowo
                    odom_pose["dist"] = float(_np.linalg.norm(xyz - odom_pose["origin"]))
                except Exception: pass
